...
public class DataFetchClient {
    ...
    public DataFetchClient(int dataport, int socketDelayTime) {
        ...
        if (socketDelayTime > 0) this.socketDelayTime = socketDelayTime;
        else this.socketDelayTime = 2000;
    }
                ...
                if (!dedHost.getConfig().isParam("DEDICATED")) {
                    ...
                    int selectedValue =
                            JOptionPane.showOptionDialog(
                                    ...
                                    options,
                                    options[0]);
                    if (selectedValue == 0) System.exit(0); // exit, if they so choose
                }
            ...
            if (!dedHost.isDedicated()) {
                ...
                try {
                    ...
                    if (Integer.parseInt(dedHost.getConfigParam("MAXPLAYERS")) > 0) {
                        ...
                        dedHost.sendChat(
                                MWDedHost.PROTOCOL_PREFIX + "c mm# I need to be updated manually!");
                    }
        ...
        if (localList.exists()) {
            ...
            MWLogger.errLog(
                    "Local OpList: "
                            ...
                            + " Server OpList: "
                            + serverListTimestamp);
            if (localListTimestamp.equals(serverListTimestamp)) timestampMatch = true;
    ...
    public void getServerTraitFiles() throws IOException {
        ...
        try {
            // MMClient.MWDedHostLog.clientErrLog("- opening connection to datafeed. requesting
            // Trait Files");
            ...
            try {
                while (true) {
                    ...
                    PrintStream out = new PrintStream(fops);
                    for (; count > 0; count--) out.println(in.readLine("TraitLine"));
                    out.flush();
                    out.close();
                    fops.flush();
                    fops.close();
        ...
        try {
            ...
            if (localGameOptions.exists()) {
                ...
                try {
                    ...
                    // get the local MD5
                    String localOptionsMD5 =
                            DigestUtils.md5Hex(new FileInputStream(localGameOptions)).toUpperCase();
    ...
    private BinReader openConnection(String cmd, int timeout) throws IOException {
        ...
        out.println(cmd, "cmd");
        if (lastTimestamp == null) out.println("", "lasttimestamp");
        else {
