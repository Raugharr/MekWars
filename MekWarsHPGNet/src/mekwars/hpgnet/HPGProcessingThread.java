...
/**
 * Thread which processes information transmitted to the tracker by MekWarsServers. Servers send
 * simple strings which note the type of information to follow with a header, and use %'s as
 * delimits.
 *
 * <p>Types are as follows: 1) ServerStart - SS% header. Sent when a server boots. Contains vital
 * information like server name and link to homepage. 2) UpdateInfo - Overwrites select information
 * from SS%, including the description and link. Server name may not be changed. Uses UI% as lead.
 * 3) PhoneHome - Most common command. Sent by server on a schedule set by a server operator (eg -
 * every 10 minutes). Contains information re: number of players currently online, the number of
 * running/recently completed games, and more. Uses PH% as lead.
 */
public class HPGProcessingThread extends Thread {
    ...
    public synchronized void run() {
        ...
        tracker.addToLog(
                "- being processing line. block other threads ("
                        + tracker.getProcessingThreads().size()
                        + " record(s) being processed)");
