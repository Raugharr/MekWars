...
/**
 * A class to deserialize HPGSubscriber objects. We need a custom deserializer because JSON doesn't
 * know how to deal with the EvictingQueues used in the object
 *
 * @author Spork
 * @version 1.0
 */
public class HPGSubscriberDeserializer implements JsonDeserializer<HPGSubscriber> {
    ...
    @Override
    public HPGSubscriber deserialize(
            JsonElement json, Type typeOfT, JsonDeserializationContext context)
            throws JsonParseException {
        final JsonObject obj = json.getAsJsonObject();
        final String name =
                (obj.get("name") == null || obj.get("name").isJsonNull())
                        ? ""
                        : obj.get("name").getAsString();
        final String url =
                (obj.get("url") == null || obj.get("url").isJsonNull())
                        ? ""
                        : obj.get("url").getAsString();
        final String desc =
                (obj.get("description") == null || obj.get("description").isJsonNull())
                        ? ""
        ...
        final String ipAddress =
                (obj.get("ipAddress") == null || obj.get("ipAddress").isJsonNull())
                        ? ""
                        : obj.get("ipAddress").getAsString();
        final String domain =
                (obj.get("domain") == null || obj.get("domain").isJsonNull())
                        ? ""
                        : obj.get("domain").getAsString();
        final String version =
                (obj.get("MWVersion") == null || obj.get("MWVersion").isJsonNull())
                        ? ""
                        : obj.get("MWVersion").getAsString();
        final String uuid =
                (obj.get("uuid") == null || obj.get("uuid").isJsonNull())
                        ? ""
                        : obj.get("uuid").getAsString();
        final String password =
                (obj.get("password") == null || obj.get("password").isJsonNull())
                        ? ""
                        : obj.get("password").getAsString();
        final boolean isLegacy =
                obj.get("isLegacy") == null ? true : obj.get("isLegacy").getAsBoolean();
        final String lastUpdated =
                (obj.get("lastUpdated") == null || obj.get("lastUpdated").isJsonNull())
                        ? ""
