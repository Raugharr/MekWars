...
public class MiniCampaignPanel extends JPanel {
    ...
    public MiniCampaignPanel(MWClient mwclient) {
        ...
        String description =
                "<HTML>Mini Campaign allows for a different form of play. Every player has a cycle that limits<br>"
        ...
        panel4b.setBorder(
                BorderFactory.createTitledBorder(
                        "Restock Triggers (what causes the mini campaign to end)"));
        ...
        baseTextField.setToolTipText(
                "<HTML>(-1 to disable) Initiate 'restock' of players hangar when hangar bv drops below this value </HTML>");
        ...
        baseTextField.setToolTipText(
                "<HTML>(-1 to disable) Initiate 'restock' of players hangar when hangar unit count drops below this value </HTML>");
        baseTextField.setName("Unit_HangarRestock");
        panel4b.add(baseTextField);

        panel4c.setBorder(
                BorderFactory.createTitledBorder("Currency Injection (provided during restock)"));

        baseTextField = new JTextField(5);
        panel4c.add(new JLabel(cbName, SwingConstants.TRAILING));
        baseTextField.setToolTipText(
                "<HTML>(-1 to disable) when restock is triggered, inject this amount of "
                        + cbName
                        + ". Note resets amount to zero before injection.</HTML>");
        ...
        baseTextField.setToolTipText(
                "<HTML>(MT = MekTokens used with freebuild limits)(-1 to disable)(PostDefection freebuild w/limit must be enabled)<br> when restock is triggered, inject this amount of Free Mek Tokens. Note resets amount to zero before injection. </HTML>");
        baseTextField.setName("RestockMT_Injection");
        panel4c.add(baseTextField);

        panel4d.setBorder(
                BorderFactory.createTitledBorder(
                        "Required Currency Usage % (to start next MC cycle)"));

        baseTextField = new JTextField(5);
        panel4d.add(new JLabel(cbName, SwingConstants.TRAILING));
        baseTextField.setToolTipText(
                "<HTML>(-1 to disable)<br> % of currency player must use before being able to go active and start the next cycle </HTML>");
        ...
        baseTextField.setToolTipText(
                "<HTML>(MT = MekTokens used with freebuild limits)<br>(-1 to disable)(PostDefection freebuild w/limit must be enabled)<br> % of currency player must use before being able to go active and start the next cycle </HTML>");
        baseTextField.setName("RestockMT_LeewayPercentage");
        panel4d.add(baseTextField);

        panel4e.setBorder(
                BorderFactory.createTitledBorder("Rewards (added at start of restock phase)"));
        panel4e.setLayout(new SpringLayout());

        String rewardsTooltip =
                "<HTML>(0 to disable) Can be set to a negative number to penalize.<br>"
        ...
        baseCheckBox.setToolTipText(
                "<HTML>For a very controled MC cycle, player must buy back up to unit limit for all defined type/weight limits.</HTML>");
        baseCheckBox.setName("AtUnitLimitsMC");
        panel4f.add(baseCheckBox);

        baseCheckBox = new JCheckBox("Enforce Limits, Allow OverLimit");
        baseCheckBox.setToolTipText(
                "<HTML>For a very controled MC cycle, player must buy back up to unit limit for all defined type/weight limits.<br> However, also allow activation if the player is OVER the unit limit (via salvage).</HTML>");
        baseCheckBox.setName("AtOrOverUnitLimitsMC");
        panel4f.add(baseCheckBox);

        baseCheckBox = new JCheckBox("Ignore Aero Unit Limits");
        baseCheckBox.setToolTipText(
                "<HTML>This is in regards to hangar unit limits and the previous two options</HTML>");
        ...
        baseCheckBox.setToolTipText(
                "<HTML>(This feature CAN be used without mini campaigns, use reset% option to manage unlocks.)<br>Enable unit locking, units are locked after use, meaning they can only be used once per mini campaign cycle.</HTML>");
        ...
        baseTextField.setToolTipText(
                "<HTML>(-1 to disable)(Likely to be used when using unit locking w/o mini campaigns)<br> Example: if set to 70 -> if 70 percent of the players units are locked all units unlock without ending the cycle. Injections will NOT occur.  </HTML>");
        ...
        baseCheckBox.setToolTipText(
                "<HTML>Units awarded by salvage after a match are awarded locked</HTML>");
        baseCheckBox.setName("LockSalvagedUnits");
        panel5b.add(baseCheckBox);

        baseCheckBox = new JCheckBox("Remove BV");
        baseCheckBox.setToolTipText(
                "<HTML> With this set, Locked units do NOT count towards hangar BV calculations. Injections WILL occur due to locked units. </HTML>");
        baseCheckBox.setName("LockedUnits_RemoveBV");
        panel5b.add(baseCheckBox);

        baseCheckBox = new JCheckBox("Decrement Count");
        baseCheckBox.setToolTipText(
                "<HTML> With this set, Locked units do NOT count towards hangar unit count. Injections WILL occur due to locked units reducing unit count. </HTML>");
        baseCheckBox.setName("LockedUnits_DecrementUnitCount");
        panel5b.add(baseCheckBox);

        baseCheckBox = new JCheckBox("1 Match Only");
        baseCheckBox.setToolTipText(
                "<HTML> Likely to be used without mini campaigns. With this set, units only stayed locked for one match.</HTML>");
