...
public abstract class GameHost implements GameListener, IGameHost {
    ...
    protected Vector<String> decodeBuffer =
            new Vector<String>(1, 1); // used to buffer incoming data until CMainFrame is built
    ...
    public void gameEntityRemove(GameEntityRemoveEvent e) { // only send if the
        ...
        String toSend =
                SerializeEntity.serializeEntity(removedE, true, false, isUsingAdvanceRepairs());
    ...
    public int getBuildingsLeft() {
        Enumeration<Building> buildings = ((Game) myServer.getGame()).getBoard().getBuildings();
        int buildingCount = 0;
