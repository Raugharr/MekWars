...
public class CArmy extends Army {
    ...
    public void fromString(String s, CPlayer p, String delimiter, MWClient client) {
        ...
        if (locked) playerLockArmy();
        else playerUnlockArmy();
        boolean disabled = TokenReader.readBoolean(ST);
        if (disabled) disableArmy();
        else enableArmy();
    }
    ...
    /**
     * Method that adds a unit to a CArmy, in a specified position. Called from CPlayer in response
     * to a PL|RPU. See CPlayer.java and PL.java for more info.
     */
    ...
    /**
     * Method that returns an amriy's legal operations. Used throughout the client to build GUI
     * elements.
     */
     ...
     *
     * <p>See CPlayer.setArmyData() for usage details.
     */
    ...
    /**
     * Client-side duplication of SArmy's getOperationsBV math. If the client, via tha downloaded
     * server config data, knows that Operation Flashpoint style BV adjustments are in use, the HQ
     * calls this method in order to accurately display BV vs. 4 without forcing the server to
     * recalculate BV every time a unit is added to/removed from an army.
     *
     * <p>See comments in SArmy for more information about how the adjustment is determined, etc.
     */
    /*
    ...
    public float getRawForceSize() {
        ...
        for (Unit u : this.getUnits()) {
            if (u.getType() == Unit.INFANTRY)
                rawForceSize +=
                        Float.parseFloat(mwclient.getServerConfigs("InfantryOperationsBVMod"));
            else if (u.getType() == Unit.VEHICLE)
                rawForceSize +=
                        Float.parseFloat(mwclient.getServerConfigs("VehicleOperationsBVMod"));
    ...
    public int getAverageWalk() {
        ...
        if (this.getUnits().size() < 1) return 0;
