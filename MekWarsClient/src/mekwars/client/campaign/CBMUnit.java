...
/**
 * Client-side market unit. The market uses the filenames and other data from the CBMUnit to
 * generate temporary CUnits (to determine BV, etc).
 *
 * <p>In the past, this class extended CUnit. This is no longer the case. Using minimal data (not
 * sending ammo settings and complete unit strings) saves bandwidth. Instead, we build temporary
 * CUnit and store it w/i the BMUnit.
 */
public class CBMUnit {
    ...
    // CONSTRUCTOR
    /**
     * Constructor which takes a data string from the server. This String is generated in Market2's
     * getAutoMarketStatus() method.
     *
     * <p>Be sure that the token read-in order always matches the market's write-out order.
     */
