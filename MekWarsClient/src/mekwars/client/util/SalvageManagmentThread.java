        ...
        while (true) {
            try {
                ...
                client.systemMessage(
                        "Error proccessing Salvage Management queue. Alert an SO and check your ./logs/error.0 for the error");
            ...
            for (int pos = UnitUtils.TECH_GREEN; pos <= UnitUtils.TECH_PILOT; pos++) {
                ...
                while (workQueue.hasNext()) {
                    ...
                    if (unit == null) {
                        ...
                        client.systemMessage(
                                "Unable to find unit to salvage. removing salvage job");
                        workQueue.remove();
                        continue;
                    }
                    ...
                    synchronized (unit) {
                        if (pos == UnitUtils.TECH_PILOT
                            ...
                            client.systemMessage(
                                    "Work order found for the pilot of "
                                            + unit.getModelName()
                                            + " however the pilot cannot salvage this unit.<br>The work order has been terminated.");
                            workQueue.remove();
                            continue;
                        }
                        ...
                        if (!armor) {
                            ...
                            if (cs.isDamaged()) {
                                client.systemMessage(
                                        UnitUtils.techDescription(pos)
                                                + " tech work order canceled because the critical was damaged.");
                                workQueue.remove();
                                continue;
                            }
                        } else {
                            if (slot == UnitUtils.LOC_FRONT_ARMOR) {
                                ...
                                if (unit.getEntity().getArmor(tempLocation) <= 0) {
                                    client.systemMessage(
                                            UnitUtils.techDescription(pos)
                                                    + " tech work order canceled, that sections armor has already been salvaged.");
                                    workQueue.remove();
                                    continue;
                                }

                            } else if (slot == UnitUtils.LOC_REAR_ARMOR) {
                                if (location >= UnitUtils.LOC_CTR) location -= 7;
                                ...
                                // Check that everything is gone before you try and salvage
                                // internals.
                                if (UnitUtils.getSalvageMessage(
                                                        unit.getEntity(), location, slot, armor)
                                                .length()
                                        > 0) continue;
                                if (unit.getEntity().getInternal(location) <= 0) {
                                    client.systemMessage(
                                            UnitUtils.techDescription(pos)
                                                    + " tech work order canceled, that sections Internal Structure has already been salvaged.");
                                    workQueue.remove();
                                    continue;
                                }
                            }
        ...
        for (int tech = UnitUtils.TECH_GREEN; tech <= UnitUtils.TECH_PILOT; tech++) {
            ...
            while (repairs.hasNext()) {
                ...
                if (repair.indexOf(Integer.toString(unitID)) == 0) {
                    ...
                    if (slotid == UnitUtils.LOC_FRONT_ARMOR)
                        data +=
                                UnitUtils.techDescription(tech)
                                        + " tech queued for external armor salavage "
                        ...
                        data +=
                                UnitUtils.techDescription(tech)
                                        + " tech queued for internal structure salavage "
                                        + unit.getEntity().getLocationAbbr(locationid)
                                        + ".";
                    else {
                    ...
                    data +=
                            " <a href=\"REMOVESALVAGEQUEUEDWORKORDER|"
                                    + tech
                                    + "|"
                                    + repair
