...
public class SplashWindow {
    ...
    public SplashWindow() {
        ...
        versionLabel =
                new JLabel(
                        "<HTML><CENTER><b>Initializing</b></CENTER></HTML>", SwingConstants.CENTER);
    ...
    public synchronized void run() {
        ...
        while (splash.shouldAnimate()) {
            ...
            try {
                ...
                if (currStatus == splash.STATUS_INITIALIZING) this.setLabelText(initializing);
                else if (currStatus == splash.STATUS_FETCHINGDATA) this.setLabelText(fetching);
                else if (currStatus == splash.STATUS_CONSTRUCTINGGUI)
                    this.setLabelText(constructing);
                else if (currStatus == splash.STATUS_CONNECTING) this.setLabelText(connecting);
                else if (currStatus == splash.STATUS_INPUTWAIT)
                    splash.getImageLabel()
                            .setText(
                                    "<HTML><CENTER><b>Connecting to Server<br>[Waiting for Input]</b></CENTER></HTML>");
                else if (currStatus == splash.STATUS_DATAERROR)
                    splash.getImageLabel()
                            .setText(
                                    "<HTML><CENTER><b>Downloading Data<br>[Data Access Error]</b></CENTER></HTML>");
                else if (currStatus == splash.STATUS_CONNECTFAILED)
                    splash.getImageLabel()
                            .setText(
                                    "<HTML><CENTER><b>Connecting to Server<br>[Connection Failed]</b></CENTER></HTML>");
