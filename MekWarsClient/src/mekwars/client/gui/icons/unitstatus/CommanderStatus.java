...
public class CommanderStatus implements UnitStatus {
    private static final int COMMANDER = 0;

    private static final UnitStatusIcon[] ICONS = new UnitStatusIcon[] {
        new UnitStatusIcon("data/images/status/comm.gif") {
            @Override
            public String getToolTipText(ResourceBundle resourceBundle) {
                return resourceBundle.getString("unitStatus.commander.tooltip");
            }
        },
    };

    public UnitStatusIcon apply(CUnit unit, CArmy army) {
        if (army != null && army.isCommander(unit.getId())) {
            return ICONS[COMMANDER];
        }
        return null;
    }
}
