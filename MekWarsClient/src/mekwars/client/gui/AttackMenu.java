...
/**
 * Create an "Attack" menu. Used in Map, CMainFramge, etc. to generate menus which show a player's
 * currently available operation options.
 *
 * <p>Unlike the old Task's dynamic menus, this is an actual class/object, not a static method which
 * returns a contructed menu. Also allows for clearing/updating.
 */
public class AttackMenu extends JMenu implements ActionListener {
    ...
    // private static int OPMINOWNIBD = 15;    //Baruk 2015-11-7 code to grab value in case it is
    // needed by this .java some other time
        ...
        if (armyID < 0) {
            ...
            // else, filter out ops which can't reach/target the world
                ...
                for (String currOpName : tempEligibles) {
                    ...
                    // boolean minOwnIBD = Boolean.parseBoolean(opProps[AttackMenu.OPMINOWNIBD]);
                    // //Baruk 2015-11-7  code to grab value in case it is needed by this .java some
                    // other time
                    ...
                    if (!legalDefenders.startsWith("allFactions")) {
                        ...
                        StringTokenizer legalDefTokenizer =
                                new StringTokenizer(legalDefenders, "$");
                    ...
                    if (tp.getFactoryCount() > 0 && facInfo.equals("none")) continue;
                    else if (tp.getFactoryCount() < 1 && facInfo.equals("only")) continue;
                    ...
                    minOwn *=
                            tp.getConquestPoints()
                                    / 100.0D; // Baruk 2015-11-7 modified to include planet CP in
                    // formula
                    maxOwn *=
                            tp.getConquestPoints()
                                    / 100.0D; // Baruk 2015-11-7 modified to include planet CP in
            ...
            for (String currName : allEligibles) {
                ...
                if (!allOps.containsKey(currName)) {
                    ...
                    if (!allOps.containsKey(currName)) {
                        MWLogger.errLog(
                                "Error in updateMenuItems(): no _" + currName + "_ in allOps.");
                        ...
                        // don't want to stop building because of one bad apple - continue to next
                        // element.
                        continue;
                    }
                }
         ...
         */
            ...
            if (clickArmy != null) {
                ...
                for (String currName : clickArmy.getLegalOperations()) {
                    ...
                    if (!fullMenu)
                        menuItemName =
                                "<html><font color=" + color + ">" + currName + "</font></html>";
                    else menuItemName = " - " + currName;
     ...
     *
     * <p>aid|pid *or* aid|pid|arg0|arg1|arg2...
     *
     * <p>"aid" is the ArmyID to use. If "-1", choice will be given. "pid" is the PlanetID to
     * attack. If "-1", menu will open.
     *
     * <p>cmd is the command and one of those:
     *
     * <p>if arg0 == "multi" arg1: maxattackers arg2: minattackers arg3: maxdefenders arg4:
     * mindefenders
     *
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (name.equals("cmdCheckAccess")) {
            ...
            if (mwclient.getPlayer().getArmies().size() <= 0) {
                String toUser =
                        "CH|CLIENT: It's impossible to check legality when you have no armies.";
        ...
        if (name.equals("cmdAttackFromReserve")) {
            ...
            PlanetNameDialog planetDialog =
                    new PlanetNameDialog(mwclient, "Choose a planet", opProperties);
            ...
            PlayerNameDialog playerDialog =
                    new PlayerNameDialog(
                            mwclient, "Select an opponent", PlayerNameDialog.ANY_PLAYER);
            ...
            new ArmyViewerDialog(
                    ...
                    Op,
                    null,
                    ArmyViewerDialog.AVD_ATTACKFROMRESERVE,
                    planet,
                    target,
                    -1,
                    -1);
        ...
        if (planetName.equals("-1")) {
            ...
            PlanetNameDialog planetdialog =
                    new PlanetNameDialog(mwclient, "Select a planet to attack.", opProperties);
            planetdialog.setVisible(true);
        ...
        if (armyID < 0) {
            ...
            new ArmyViewerDialog(
                    mwclient, name, null, ArmyViewerDialog.AVD_ATTACK, planetName, null, -1, -1);
