...
public class ArmyViewerDialog extends JDialog
    ...
    public ArmyViewerDialog(
            MWClient mwclient,
            String opName,
            StringTokenizer validArmyList,
            int mode,
            String planet,
            String defender,
            int opid,
        ...
        if (teamNumbers > 1) {
            for (int team = 1; team <= teamNumbers; team++) teamBox.addItem("Team #" + team);
        }
            ...
            try {
                ...
                if (armyList.getSelectedIndex() < 0) {
                    JOptionPane.showMessageDialog(
                            this, "Select an army to defend with or click cancel");
                    return;
                }
    ...
    void previewArmy(int armyID) {
        ...
        if (armyID > -1) {
            ...
            for (Unit unit : army.getUnits()) {
                armyText.append(
                        makeLength("#" + unit.getId(), 7)
                                + " "
                                + makeLength(((CUnit) unit).getModelName(), 12)
                                + " ");
                if (unit.getType() == Unit.VEHICLE
                        || unit.getType() == Unit.MEK
                    ...
                    if (((Infantry) ((CUnit) unit).getEntity()).canMakeAntiMekAttacks()) {
                        armyText.append(
                                " ("
                                        ...
                                        + unit.getPilot().getPiloting()
                                        + ")");
                    } else armyText.append(" (" + unit.getPilot().getGunnery() + ")");
                } else armyText.append(" (" + unit.getPilot().getGunnery() + ")");
    ...
    private String makeLength(String s, int nLength) {
        ...
        else if (s.length() > nLength) return s.substring(0, nLength - 2) + "..";
        else return s + SPACES.substring(0, nLength - s.length());
    }
}
