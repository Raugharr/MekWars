...
/**
 * A JPanel containing several JComboBoxes to control what table is displayed in the {@link
 * TablePanel}
 *
 * @author Spork
 */
public class SelectorPanel extends JPanel implements ActionListener {
    ...
    private void prepComponents() {
        ...
        for (int i = 0; i < Unit.MAXBUILD; i++) {
            if ((i == Unit.VEHICLE)
                    && !(Boolean.parseBoolean(client.getServerConfigs("UseVehicle")))) {
                continue;
            }
            if ((i == Unit.INFANTRY)
                    && !(Boolean.parseBoolean(client.getServerConfigs("UseInfantry")))) {
                continue;
            }
            if ((i == Unit.BATTLEARMOR)
                    && !(Boolean.parseBoolean(client.getServerConfigs("UseBattleArmor")))) {
                continue;
            }
            if ((i == Unit.PROTOMEK)
                    && !(Boolean.parseBoolean(client.getServerConfigs("UseProtoMek")))) {
                continue;
            }
    ...
    /**
     * Called by {@link TablePanel} when the tables are first built. Sets the selected table to the
     * user's faction
     *
     * @param house the faction to change to
     */
