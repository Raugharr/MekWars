...
public class SellUnitDialog extends JDialog implements ActionListener {
    ...
    public SellUnitDialog(JFrame parent, MWClient mwclient, Vector<CUnit> toSell) {
                ...
                if (currU.getType() == Unit.MEK
                    ...
                    continue;
                        ...
                        && !Boolean.parseBoolean(
                                mwclient.getServerConfigs("InfantryMayBeSoldOnBM"))) continue;

                if (currU.getStatus() == Unit.STATUS_FORSALE) continue;

                if (Boolean.parseBoolean(mwclient.getServerConfigs("BMNoClan"))
                        && currU.getEntity().isClan()) continue;
        ...
        possibleSaleUnits.setModel(
                new DefaultComboBoxModel(toSell) {
                    /** */
        ...
        newDim.setSize(
                possibleSaleUnits.getMinimumSize().getWidth() * 1.25,
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (command.equals(okayCommand)) {
            ...
            result += mm.getId();
            if (!ticksText.getText().equalsIgnoreCase("")) result += "#" + ticksText.getText();
            else result += "#" + mwclient.getServerConfigs("MinBMSalesTicks");
            if (!minBidText.getText().equalsIgnoreCase("")) result += "#" + minBidText.getText();
            else result += "#" + mwclient.getServerConfigs("MinBMSalesPrice");
