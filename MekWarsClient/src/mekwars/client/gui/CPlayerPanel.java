...
public class CPlayerPanel extends JScrollPane {
    ...
    public CPlayerPanel(MWClient client) {
        ...
        if (!Boolean.parseBoolean(client.getServerConfigs("HideELO"))) InfoPanel.add(lblRating);
        ...
        if (Integer.parseInt(client.getServerConfigs("FreeBuild_Limit"))
                > 0) // @Salient for free build
    ...
    public void refresh() {
        ...
        if (mwclient.getConfig().isParam("LOGO")) {
            try {
                ...
                ((HTMLEditorKit) lblLogo.getEditorKit())
                        .read(
                                new StringReader(mwclient.getPlayer().getLogo()),
                                lblLogo.getDocument(),
                                0);
        ...
        if (player.getMoney() == 0)
            lblMoney.setText(
                    mwclient.moneyOrFluMessage(true, false, -2) + ": " + player.getMoney());
        else
            lblMoney.setText(
                    mwclient.moneyOrFluMessage(true, false, -player.getMoney())
                            + ": "
                            + NumberFormat.getInstance().format(player.getMoney()));
        if (player.getInfluence() == 0)
            lblInfluence.setText(
                    mwclient.moneyOrFluMessage(false, false, -2) + ": " + player.getInfluence());
        else
            lblInfluence.setText(
                    mwclient.moneyOrFluMessage(false, false, -player.getInfluence())
                            + ": "
                            + NumberFormat.getInstance().format(player.getInfluence()));
            ...
            try {
                lblMekbay.setText(
                        PP_BAYS
                                ...
                                + player.getBays()
                                + " ("
                                + mwclient.moneyOrFluMessage(true, true, player.getTechCost())
                                + ")");
                lblTechs.setText(
                        PP_IDLETECHS
                                + " "
                                + player.getAvailableTechs().get(UnitUtils.TECH_GREEN)
                                + "/"
                                + player.getAvailableTechs().get(UnitUtils.TECH_REG)
                                + "/"
                                + player.getAvailableTechs().get(UnitUtils.TECH_VET)
                                + "/"
                                + player.getAvailableTechs().get(UnitUtils.TECH_ELITE));
            } catch (Exception ex) {
            }
        } else {
        ...
        lblRewardPoints.setText(
                PP_REWARD
                        + " "
                        + player.getRewardPoints()
                        + "/"
                        + mwclient.getServerConfigs("XPRewardCap"));
        lblFreeMeks.setText(
                PP_FREEUNITS
                        + " "
                        + (Integer.parseInt(mwclient.getServerConfigs("FreeBuild_Limit"))
                                - mwclient.getPlayer().getMekToken())
                        + " Remain"); // @Salient for free build
    }
    ...
    public void updateClock() {
        lblNextTick.setText(
                "Next Tick: " + ((nextTick - System.currentTimeMillis()) / 1000) + " s");
    }
