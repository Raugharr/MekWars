...
public class CBattlePanel extends JPanel {
	...
	class BattlePopupListener extends MouseAdapter implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (s.startsWith("S|")){
				mwclient.getGUIClient().getMainFrame().stopHost(); mwclient.stopHost();
			}
			
			if (s.startsWith("RESTART|")){
				String currName = s.substring(s.indexOf('|') + 1);
			    int result = JOptionPane.showConfirmDialog(mwclient.getGUIClient().getMainFrame(), "Are you sure you want to restart\n\r"+currName+"?","Restart?", JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
					mwclient.sendChat(GameHost.CAMPAIGN_PREFIX + "mail "+currName+",restart");
			} else if (s.startsWith("RESET|")){
				String currName = s.substring(s.indexOf('|') + 1);
				int result = JOptionPane.showConfirmDialog(mwclient.getGUIClient().getMainFrame(), "Are you sure you want to reset\n\r"+currName+"?","Reset?", JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
					mwclient.sendChat(GameHost.CAMPAIGN_PREFIX + "mail "+currName+",reset");
			} else if (s.startsWith("DIE|")){
				String currName = s.substring(s.indexOf('|') + 1);
				int result = JOptionPane.showConfirmDialog(mwclient.getGUIClient().getMainFrame(), "Are you sure you want to kill\n\r"+currName+"?","Kill?", JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
					mwclient.sendChat(GameHost.CAMPAIGN_PREFIX + "mail "+currName+",die");
			} else if (s.startsWith("START|")){
				String currName = s.substring(s.indexOf('|') + 1);
				int result = JOptionPane.showConfirmDialog(mwclient.getGUIClient().getMainFrame(), "Are you sure you want to start\n\r"+currName+"?","Start?", JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
					mwclient.sendChat(GameHost.CAMPAIGN_PREFIX + "mail "+currName+",start");
			} else if (s.startsWith("STOP|")){
				String currName = s.substring(s.indexOf('|') + 1);
				int result = JOptionPane.showConfirmDialog(mwclient.getGUIClient().getMainFrame(), "Are you sure you want to stop\n\r"+currName+"?","Stop?", JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
				...
				String currName = s.substring(s.indexOf('|') + 1);
				int result = JOptionPane.showConfirmDialog(mwclient.getGUIClient().getMainFrame(), "Are you sure you want to clear the owners of\n\r"+currName+"?","Clear the owners?", JOptionPane.YES_NO_OPTION);
				if (result == JOptionPane.YES_OPTION)
				    mwclient.sendChat(GameHost.CAMPAIGN_PREFIX + "mail "+currName+",clearowners");
			} else if (s.startsWith("ADDOWNERS|")){
				String currName = s.substring(s.indexOf('|') + 1);
				String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter a list of owners you want to add to\n\r"+currName+"\n\r(sperated by $)","Add Owners", JOptionPane.OK_CANCEL_OPTION);
				...
				String currName = s.substring(s.indexOf('|') + 1);
				String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter a new port for\n\r"+currName,"New Port", JOptionPane.OK_CANCEL_OPTION);
				if (result != null && result.length() > 1)
				...
				String currName = s.substring(s.indexOf('|') + 1);
				String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter a new day for\n\r"+currName,"New days out to purge", JOptionPane.OK_CANCEL_OPTION);
				if (result != null && result.length() >= 1)
				...
				String currName = s.substring(s.indexOf('|') + 1);
				String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter a new comment for\n\r"+currName,"New Comment", JOptionPane.OK_CANCEL_OPTION);
				if (result != null && result.length() > 1)
					mwclient.sendChat(GameHost.CAMPAIGN_PREFIX + "mail "+currName+",comment "+result);
			} else if (s.startsWith("SETPLAYERS|")){
				String currName = s.substring(s.indexOf('|') + 1);
				String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter the max number of players for\n\r"+currName,"New Players", JOptionPane.OK_CANCEL_OPTION);
				...
				String currName = s.substring(s.indexOf('|') + 1);
				String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter a new restart count for\n\r"+currName,"New Restart", JOptionPane.OK_CANCEL_OPTION);
				if (result != null && result.length() >= 1)
            	...
            	String currName = s.substring(s.indexOf('|') + 1);
                String result = JOptionPane.showInputDialog(mwclient.getGUIClient().getMainFrame(), "Enter a new update url for\n\r"+currName,"New Update URL", JOptionPane.OK_CANCEL_OPTION);
                if (result != null && result.length() >= 1)
