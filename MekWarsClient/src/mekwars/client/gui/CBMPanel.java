...
public class CBMPanel extends JPanel {
    ...
    public CBMPanel(MWClient client) {
                        ...
                        Integer auctionId =
                                (Integer)
                                        tblMarket
                                                .getModel()
                                                .getValueAt(
                                                        selectedRow, BlackMarketModel.AUCTION_ID);
                        if (auctionId != null) {
        ...
        btnRecallBid.setEnabled(false);
        btnRecallBid.addActionListener(
                new ActionListener() {
                    public void actionPerformed(ActionEvent evt) {
        ...
        btnRecallUnit.setEnabled(false);
        btnRecallUnit.addActionListener(
                new ActionListener() {
                    public void actionPerformed(ActionEvent evt) {
    ...
    public void fireMarketChanged() {
        // here's a problem MyBlackMarket has to be created somehow (by parsing BM or from Player
        // data)
        BlackMarketInfo.refreshModel();

        tblMarket.setPreferredSize(
                new Dimension(
                        tblMarket.getWidth(),
    ...
    /**
     * Called from an action listener. Opens a MechDetailDisplay for the unit at the currently
     * selected row.
     *
     * @param evt
     */
    ...
    public CBMUnit getMarketMechAtRow(int row) {
        mm = null;
        Integer auctionId =
                (Integer) tblMarket.getModel().getValueAt(row, BlackMarketModel.AUCTION_ID);
        if (auctionId != null) {
    ...
    /**
     * Called from an action listener. Opens a dialo for input, checks the input, and places a bid
            ...
            if (auctionID != -1) { // -1 is the default value. indicates null auction.
                ...
                try {
                    ...
                    if (playerBid < mm.getMinBid()) {
                        String toUser =
                                "CH|CLIENT: You tried to bid less than the minimum your contacts are"
                    ...
                    mwclient.sendChat(
                            GameHost.CAMPAIGN_PREFIX + "c bid#" + auctionID + "#" + playerBid);
    ...
    /**
     * Method called from CPlayer after a faction is set. Enables or disables the Sell Unit button,
     * as appropriate for the faction.
     *
     * <p>Also sets a correct CBMPanel.factionBidsAllowed value so that future clicks on BM units
     * give a correct "Place Bid" button.
     */
