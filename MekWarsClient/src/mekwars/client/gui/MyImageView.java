...
public class MyImageView extends View implements ImageObserver, MouseListener, MouseMotionListener {
    ...
    /**
     * Added this guy to make sure an image is loaded - ie no broken images. So far its used only
     * for images loaded off the disk (non-URL). It seems to work marvelously. By the way, it does
     * the same thing as MediaTracker, but you dont need to know the component its being rendered
     * on. Rob
     */
    ...
    float getVerticalAlignment() {
        ...
        if (align != null) {
            ...
            if (align.equals(TOP) || align.equals(TEXTTOP)) return 0.0f;
            else if (align.equals(MyImageView.CENTER)
                    || align.equals(MIDDLE)
    ...
    /**
     * Establishes the parent view for this view. Seize this moment to cache the AWT Container I'm
     * in.
     */
    @Override
     ...
     *
     * @param axis may be either X_AXIS or Y_AXIS
     * @returns the desired alignment. This should be a value between 0.0 and 1.0 where 0 indicates
    ...
    protected void resize(int width, int height) {
        ...
        ((StyledDocument) getDocument())
                .setCharacterAttributes(
                        fElement.getStartOffset(), fElement.getEndOffset(), attr, false);
    }
    ...
    private Icon makeIcon(final String gifFile) throws IOException {
        ...
        if (buffer.length == 0) {
            MWLogger.errLog("warning: " + gifFile + " is zero-length");
            return null;
        }
    ...
    /**
     * Set to true, while the receiver is locked, to indicate the reciever is loading the image.
