    ...
    public void execute(String input) {
        ...
        if (st.hasMoreElements()) {
            ...
            if (!mwclient.isIgnored(name, MWClient.IGNORE_PRIVATE) && st.hasMoreElements()) {
                ...
                if (mwclient.getConfig().isParam("REPLYTOSENDER")) mwclient.setLastQuery(name);
                ...
                if (Boolean.parseBoolean(mwclient.getConfigParam("INVERTCHATCOLOR"))) {
                    factioncolor =
                            StringUtils.color2html(
                                    StringUtils.invertColor(StringUtils.html2Color(factioncolor)));
                    usercolor =
                            StringUtils.color2html(
                ...
                if (mwclient.getConfig().isParam("USEMULTIPLEPM")) {
                    ...
                    if (mailTab == null) {
                        ...
                        if (count >= maxTabs) {
                            mwclient.sendChat(
                                    GameHost.CAMPAIGN_PREFIX
                                            ...
                                            + name
                                            + ", "
                                            + mwclient.getConfigParam("MAXPMMESSAGE"));
                            ...
                            message =
                                    "<font color=\""
                                            ...
                                            + "\"><b>"
                                            + name
                                            + " tried to PM you while you where busy</b></font>";
                        ...
                        mwclient.getMainFrame()
                                .getMainPanel()
                                .getCommPanel()
                                .createMailTab(tabName);
                    }
                }
                // draw a factioncolour from the datafeed
                if (sender.getHouse().length() > 1
                        && mwclient.getData().getHouseByName(sender.getHouse()) != null)
                    factioncolor =
                            mwclient.getData().getHouseByName(sender.getHouse()).getHouseColor();
                ...
                else if (message.indexOf(mwclient.getUsername()) > -1
                        && mwclient.getConfig().isParam("ENABLECALLSOUND"))
                    mwclient.doPlaySound("SOUNDONCALL");
                else if (mwclient.hasKeyWords(message)
                        && mwclient.getConfig().isParam("ENABLEKEYWORDSOUND"))
