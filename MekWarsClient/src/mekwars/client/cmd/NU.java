...
public class NU extends Command {
    ...
    public void execute(String input) {
        ...
        // Read the MMClientInfo from the Stream (It's been put on the Stream with
        // MMClientInfo.toString())
        ...
        if (mwclient.isDedicated()) return;

        if (newUser.isInvis()
                && newUser.getUserlevel()
                        > mwclient.getUser(mwclient.getPlayer().getName()).getUserlevel()) {
            mwclient.refreshGUI(MWClient.REFRESH_USERLIST);
            return;
        }

        if (newUser.getName().startsWith("[Dedicated]")) return;

        // Print an entry message if the information is followed by NEW (NU and UG are used for name
        // changing, too)
        if (st.hasMoreTokens()) {

            String name = newUser.getName();

            if (!newUser.getCountry().equals("unknown")) name += " (" + newUser.getCountry() + ")";
