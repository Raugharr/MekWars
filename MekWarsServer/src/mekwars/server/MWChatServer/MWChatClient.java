...
/**
 * This is the representation of a Client, on the server side. All the IChatClient interface methods
 * are implemented by constructing a message (with the help of CommandMakerRemote), and queuing it
 * up to be sent to the client.
 *
...
public class MWChatClient implements IConnectionListener, ICommands {
     ...
     *
     * <p>The proper way to kill a connection is with MWChatServer.SignOff(). Only the MWChatServer
     * should call this.
     */
    public void die() {
    ...
    public void roomUserList(String room, String[] users) {
        _connectionHandler.queueMessage(
                CommandMakerRemote.constructRoomUserListMessage(room, users));
    }

    /** Construct and queue a message that will be sent back to the client */
    public void userJoinedRoom(String user, String room) {
        _connectionHandler.queueMessage(
                CommandMakerRemote.constructUserJoinedRoomMessage(user, room));
    }

    /** Construct and queue a message that will be sent back to the client */
    public void userPartedRoom(String user, String room, boolean signOff) {
        _connectionHandler.queueMessage(
                CommandMakerRemote.constructUserPartedRoomMessage(user, room, signOff));
    }
    ...
    public void generalRoomMessage(String room, String message) {
        _connectionHandler.queueMessage(
                CommandMakerRemote.constructGeneralRoomMessage(room, message));
    }
    ...
    /**
     * Create an ackkill message and send it to the client. This confirms that this MWChatClient
     * made a kill.
     *
