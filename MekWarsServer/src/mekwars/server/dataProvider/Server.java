...
public class Server extends Thread {
    ...
    public void run() {
        ...
        try {
            ...
            if (IpAddress.equals("-1")) server = new ServerSocket(dataPort, 0, null);
            else server = new ServerSocket(dataPort, 0, InetAddress.getByName(IpAddress));
        ...
        MWLogger.mainLog(
                "DataProvider: server created at port "
                        + dataPort
                        + ". Address "
                        + IpAddress
                        + ". Waiting for calls...");
        ...
        while (true) {
            ...
            try {
                ...
                try {
                    ...
                    MWLogger.errLog(ex);
                    MWLogger.mainLog(
                            "DataProvider: Could not create server socket. Shutting down.");
                    MWLogger.infoLog(
                            "DataProvider: Could not create server socket. Shutting down.");
                    return;
                }
