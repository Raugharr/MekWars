...
/**
 * @author urgru
 *     <p>Private thread. Simple loop that keeps track of player's immunity. All methods that modify
 *     the immunePlayers TreeMap lock the map in order to prevent concurrent modification errors.
 */
public final class ImmunityThread extends Thread { // no extension
    ...
    /**
     * Method that adds a newly immune player to the Thread. The player is informed of his immunity.
     * If he's a newbie, he is also told if/how to reset his units.
     *
    ...
    public void addImmunePlayer(SPlayer p) {
        ...
        if (p.getMyHouse().isNewbieHouse()) {
            ...
            if (numResets > 0) {
                ...
                if (numResets == 1) {
                    ...
                    toSend += numResets + " times";
                }
                CampaignMain.cm.toUser(
                        toSend +=
                                " while immune by selecting \"Reset Units\" in the HQ. You may only reset while in reserve.",
                        p.getName(),
                        true);
            }
        }
        ...
        synchronized (immunePlayers) {
            this.immunePlayers.put(
                    lowername, System.currentTimeMillis() + (immunitySeconds * 1000));
        }
    }
    ...
    /**
     * Remove a player from the Immunity list. This is called when logging in or activating, but
     * shoud NEVER be called when deactivating. If de-activated players were removed using this
     * call, SOL would not be able to reset units.
     */
    ...
    /**
     * Override Thread's run() method to do what we want - periodically wake and poll the immune
     * players hash, bumping anyone whose time has expired back to standard status. Runs forever.
     */
    @Override
            ...
            synchronized (immunePlayers) {
                ...
                for (String currName : toRemove) {
                    ...
                    if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
                        ...
                        // tell the player
                        CampaignMain.cm.toUser(
                                "[!] Your post-game immunity expired!", p.getName(), true);

                        // alert other players
                        OpponentListHelper olh =
                                new OpponentListHelper(p, OpponentListHelper.MODE_ADD);
                        olh.sendInfoToOpponents(
                                " finished an R&R cycle and returned to the front. You may attack it with ");

                        // newbie player handling. if he has resets left, tell him time has expired.
                        // always
                        ...
                        if (p.getMyHouse().isNewbieHouse()) {
                            ...
                            if (currH.getResetsRemaining(p) > 0)
                                CampaignMain.cm.toUser(
                                        "[!] Your post-game reset time expired!",
                                        p.getName(),
                                        true);
