    ...
    private void getOpponentsForAll() {
        ...
        for (House h : CampaignMain.cm.getData().getAllHouses()) {
            ...
            for (SPlayer currPlayer : currHouse.getActivePlayers().values()) {
                ...
                if (MWServ.getInstance().getIThread().isImmune(currPlayer)) continue playersLoop;
                    ...
                    for (SArmy enemyArmy : currPlayer.getArmies()) {
                        if (!searchArmy.isDisabled()) {
                            ...
                            for (String attack : searchArmy.getLegalOperations().keySet()) {
                                ...
                                // continue to next operation if this is our faction and attacks
                                // aren't allowed
                                if (currHouse.equals(searchHouse)
                ...
                for (SArmy currArmy : possDefendArmies) {
                    ...
                    for (int j = 0; j < toStore.size(); j++) {
                        ...
                        // if the army's ID is less than the currently stored ID @ an index, insert
                        // there
