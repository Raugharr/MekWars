...
public class UserActivityComponentsJob implements Job, MWRepeatingJob, JobIdentifiableByUser {
    ...
    public static void submit(
        ...
        JobDetail job =
                newJob(UserActivityComponentsJob.class)
    ...
    /**
     * A method to determine if the player counts for generating components, based on server
     * settings.
     *
     * @param p - the player
    ...
    private boolean playerCountsForProduction(SPlayer p) {
        ...
        if (System.currentTimeMillis()
                < p.getActiveSince()
                        + Long.parseLong(p.getMyHouse().getConfig("InfluenceTimeMin"))) {
            return false;
        }
    ...
    /**
     * A method to reschedule a player's component job at a given frequency This is called when the
     * admin changes the frequency of the jobs, so the users do not have to be deactivated.
     *
