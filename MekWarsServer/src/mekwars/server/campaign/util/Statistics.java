...
public class Statistics {
    ...
    public static String doGetMechStats(int size) {
        ...
        result.append(
                "<table cellpadding=\"3\" cellspacing=\"0\"><tr bgcolor=\"#0066FF\">"
        ...
        while (i.hasNext()) {
            ...
            if (color) result.append("<tr class=\"trcolored\"><td>");
            else result.append("<tr class=\"truncolored\"><td>");
            color = !color;
            // result.append("</td><td>");
            ...
            if (m.getLastTimeUpdated() > 0) {
                result.append(
                        StringUtil.readableTime(
                                System.currentTimeMillis() - m.getLastTimeUpdated()));
        ...
        try {
            ...
            for (int j = v.size() - 1; j >= 0; j--) {
                ...
                try {
                    ...
                    if (color) result.append("<tr class=\"trcolored\">");
                    else result.append("<tr class=\"truncolored\">");
                    color = !color;
                    result.append("<td>" + rank + "</td>");
                    result.append("<td>" + p.getName() + "</td>");
                    if (CampaignMain.cm.getBooleanConfig("HideELO")) result.append("<td> -- </td>");
                    else result.append("<td>" + p.getRatingRounded() + "</td>");
                    result.append("<td>" + p.getMyHouse().getColoredName() + "</td>");
                    if (p.getFluffText().equals("0")) result.append("<td> </td>");
                    else result.append("<td>" + p.getFluffText() + "</td>");
                    result.append("</tr>");
                    rank++;
                } catch (Exception ex) {
    ...
    public static void doEXPRanking() {
        ...
        //  result.append("(Only Players with more than 1000 EXP shown)<p>";
        Iterator<House> e = CampaignMain.cm.getData().getAllHouses().iterator();
        Hashtable<String, EXPRankingContainer> allplayers =
                new Hashtable<String, EXPRankingContainer>();
        // Player DefaultPlayer = null;
        while (e.hasNext()) {
            SHouse h = (SHouse) e.next();
            if (!h.isNewbieHouse()) {
        ...
        try {
            FileOutputStream out =
                    new FileOutputStream(CampaignMain.cm.getConfig("EXPRankingPath"));
            PrintStream p = new PrintStream(out);
            p.println(result.toString());
            p.close();
            out.close();
    ...
    public static String getReadableHouseRanking(boolean useHTML) {
        ...
        String result = "";
        if (useHTML) result += "<b><i>Faction Ranking: </i><br>";
        else result += "Faction Ranking: ";

        TreeSet<HouseRankingHelpContainer> s = CampaignMain.cm.getHouseRanking();
        if (s.size() < 1) return "";

        for (HouseRankingHelpContainer h : s) {
            if (useHTML) result += h.getHouse().getColoredNameAsLink() + " (";
            else result += h.getHouse().getName() + " (";
