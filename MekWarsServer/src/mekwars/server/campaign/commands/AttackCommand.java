 ...
 * type, and then creates the operation and gives attackers their "Special" options.
 */
    ...
    public void process(StringTokenizer command, String username) {
        ...
        HibernateUtil.inTransaction(
                session -> {
                    ...
                    if (ap == null) {
                        CampaignMain.cm.toUser(
                                "AM:Null player. Contact an administrator to report this,"
                                    + " immediately!",
                                username,
                                true);
                        return;
                    ...
                    if (ap.getDutyStatus() < SPlayer.STATUS_ACTIVE) {
                        CampaignMain.cm.toUser(
                                "AM:You aren't on the front lines! (You are currently in Reserve."
                                    + " Activate in order to attack.)",
                                username,
                                true);
                        return;
                    }
                    ...
                    if (altID >= 0) {
                        CampaignMain.cm.toUser(
                                "AM:You're already defending against Attack #" + altID + ".",
                                username,
                                true);
                        return;
                    }
                    ...
                    boolean minActiveMet =
                            (System.currentTimeMillis() - ap.getActiveSince())
                                    >= (Long.parseLong(CampaignMain.cm.getConfig("MinActiveTime"))
                                            * 1000);
                    if (!minActiveMet) {
                        CampaignMain.cm.toUser(
                                "AM:You're still on your way to the frontline. You cannot attack"
                                    + " until you arrive.",
                                username,
                                true);
                        return;
                    ...
                    try {
                        ...
                        CampaignMain.cm.toUser(
                                "AM:No operation name given. Try again.", username, true);
                        return;
                    }
                    ...
                    if (o == null) {
                        CampaignMain.cm.toUser(
                                "AM:Operation Type: " + opName + " does not exist.",
                                username,
                                true);
                        return;
                    }
                    ...
                    try {
                        ...
                        CampaignMain.cm.toUser(
                                "AM:Non-number given for Army ID. Try again.", username, true);
                        return;
                    }
                    ...
                    if (aa == null) {
                        if (MULArmy) {
                            ...
                            CampaignMain.cm.toUser(
                                    "AM:An error occured while creating your Army (The Army was"
                                        + " null. This usually means the army doesn't exist."
                                        + " Example: you tried to use Army 1, but you only have"
                                        + " Armies 0 and 2.)",
                                    username,
                                    true);
                            return;
                    ...
                    try {
                        ...
                        CampaignMain.cm.toUser(
                                "AM:No planet name given. Try again.", username, true);
                        return;
                    }

                    SPlanet target =
                            CampaignMain.cm.getPlanetFromPartialString(planetName, username);
                    if (target == null) {
                    ...
                    if (o.getBooleanValue("TeamOperation") && o.getIntValue("NumberOfTeams") > 1) {
                        ...
                        if (o.getBooleanValue("TeamsMustBeSameFaction")) {
                            ...
                            List<SPlayer> activePlayers =
                                    session.createQuery(
                                                    "FROM SPlayer WHERE myHouse.id = :houseId AND"
                                                        + " status = :status",
                                                    SPlayer.class)
                                            .setParameter("houseId", attackingFaction.getId())
                                            .setParameter("status", SPlayer.STATUS_ACTIVE)
                                            .getResultList();
                            for (SPlayer joiningPlayer : activePlayers) {
                                if (joiningPlayer.getName().equalsIgnoreCase(ap.getName()))
                                    continue;
                                toSend =
                                        new StringBuilder(
                                                ap.getName()
                                                        ...
                                                        + " on "
                                                        + target.getName()
                                                        + " you may join with the following"
                                                        + " armies:");
                                sendCommand = false;
                                for (SArmy army : joiningPlayer.getArmies()) {
                                    String failures =
                                            manager.validateShortAttack(
                                                    joiningPlayer, army, o, target, longID, true);
                                    if (failures != null && s != null && !s.trim().equals(""))
                                        continue;
                                    sendCommand = true;
                                    toSend.append(
                                            "<a href=\"MEKWARS/c joinattack#"
                                                    + ap.getName()
                                                    + "#"
                                                    + army.getId()
                                                    + "\">Army #"
                                ...
                                if (sendCommand)
                                    CampaignMain.cm.toUser(
                                            toSend.toString(), joiningPlayer.getName());
                            }
                        }

                        CampaignMain.cm.toUser("PL|STN|" + teamNumber, username, false);
                        CampaignMain.cm.toUser(
                                "AM:You have been assigned to team #" + teamNumber, username);
                        ap.setTeamNumber(teamNumber);
                    }
                });
    } // end process
