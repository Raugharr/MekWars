...
/**
 * AdminSpoof allows an admin to issue ANY command on a player's behalf. Unlike other commands, this
 * cannot be delegated to lower userlevels. Instead, Spoof is locked IAuthenticator.ADMIN.
 *
 * <p>Format: adminspoof#targetname#command#[command options]
 */
public class AdminSpoofCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (CampaignMain.cm.getPlayer(targetPlayerName) == null) {
            CampaignMain.cm.toUser(
                    "Spoof failed. Could not find player: " + targetPlayerName, Username, true);
            return;
        }
        ...
        if (CampaignMain.cm.getServerCommands().get(targetCommandName.toUpperCase()) == null) {
            CampaignMain.cm.toUser(
                    "Spoof failed. Could not find command: " + targetCommandName, Username, true);
            return;
        }
        ...
        if (CampaignMain.cm
                        ...
                        .getExecutionLevel()
                > MWServ.getInstance().getUserLevel(targetPlayerName)) {
            CampaignMain.cm.toUser(
                    targetPlayerName
        ...
        // build a new string tokenizer to pass to the command, and a commandstring to show in logs,
        // etc.
        ...
        StringTokenizer newCommand =
                new StringTokenizer(issuedCommand.toString(), "#"); // rebuild a tokenizer to pass

        // checks passed. we have a valid player and command name. tell everyone about the spoof ...
        // MWLogger.modLog(Username + " used spoof to send a command as if he were " +
        // targetPlayerName
        // + ": /c " + targetCommandName + "#" + issuedCommand);
        ...
        CampaignMain.cm.toUser(
                ...
                true);
        CampaignMain.cm.toUser(
                "You issued a command as if you were "
                        + targetPlayerName
                        + ": /c "
                        + targetCommandName
                        + "#"
