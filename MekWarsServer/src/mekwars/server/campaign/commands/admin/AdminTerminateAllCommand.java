...
public class AdminTerminateAllCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        for (ShortOperation currO : CampaignMain.cm.getOpsManager().getRunningOps().values()) {
            if (currO.getStatus() != ShortOperation.STATUS_REPORTING
                    && currO.getStatus() != ShortOperation.STATUS_FINISHED) opsToCancel.add(currO);
        }
        ...
        for (ShortOperation currO : opsToCancel) {
            CampaignMain.cm
                    .getOpsManager()
                    .terminateOperation(currO, OperationManager.TERM_TERMCOMMAND, tp);
        }

        // MWLogger.modLog(Username + " terminated all unfinished games.");
        CampaignMain.cm.doSendToAllOnlinePlayers(
                Username + " terminated all unfinished games.", true);
