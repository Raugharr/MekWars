        ...
        try {
            ...
            if (command.hasMoreElements()) lock = Boolean.parseBoolean(command.nextToken());
                ...
                if (toFrom.equalsIgnoreCase("to")) {
                    if (faction.getHouseDefectionTo()) lock = false;
                    else lock = true;
                } else {
                    if (faction.getHouseDefectionFrom()) lock = false;
                    else lock = true;
                }
            }

            if (toFrom.equalsIgnoreCase("to")) faction.setHouseDefectionTo(lock);
            else faction.setHouseDefectionFrom(lock);

            if (!lock) {
                CampaignMain.cm.toUser(
                        "You've blocked defection "
                                + toFrom.toLowerCase()
                                + " "
                                + faction.getName(),
                        Username,
                        true);
                // server.MWLogger.modLog(Username + " has blocked defection
                // "+toFrom.toLowerCase()+" "+
                // faction.getName());
                CampaignMain.cm.doSendModMail(
                        "NOTE",
                        Username
                                + " has blocked defection "
                                + toFrom.toLowerCase()
                                + " "
                                + faction.getName());
            } else {
                CampaignMain.cm.toUser(
                        "You've allowed defection "
                                + toFrom.toLowerCase()
                                + " "
                                + faction.getName(),
                        Username,
                        true);
                // server.MWLogger.modLog(Username + " has allowed defection
                // "+toFrom.toLowerCase()+" "+
                // faction.getName());
                CampaignMain.cm.doSendModMail(
                        "NOTE",
                        Username
                                + " has allowed defections "
                                + toFrom.toLowerCase()
                                + " "
                                + faction.getName());
            }
