 ...
 *
 * <p>Starting at v2016.10.26, added some catches for specific config changes to restart services
 * that would otherwise require a server restart
 *
 * @version 2016.10.26
 */
public class AdminChangeServerConfigCommand implements Command {
    ...
    public void process(StringTokenizer command, String username) {
        ...
        // make setting change
        CampaignData.cd.getCampaignOptions().setProperty(config, arg);
        ...
        if (config.equalsIgnoreCase("Christmas_StartDate")) {
            ...
            if (arg.equalsIgnoreCase("true")) {
                ChristmasHandler.getInstance()
                        .setUnitMethod(ChristmasHandler.UNIT_METHOD_ONEOFEACH);
            }
            ...
            CampaignData.cd
                    .getCampaignOptions()
                    .setProperty("TrackerUUID", UUID.randomUUID().toString());
            CampaignData.cd
                    .getCampaignOptions()
                    .setProperty("TrackerResetUUID", "false");
        }
