...
/**
 * AdminUpdateClientParam allows an admin to set a client param for a single player or all players
 * online. Unlike other commands, this cannot be delegated to lower userlevels. Instead, it is
 * locked IAuthenticator.ADMIN.
 *
 * <p>Format: adminupdateclientparam#targetname[ALL]#param#param value
 */
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (MWServ.getInstance().getUserLevel(Username) < IAuthenticator.ADMIN) {
            CampaignMain.cm.toUser(
                    "Only admins may use the update client param command.", Username, true);
            return;
        }
        ...
        if (!playerName.equalsIgnoreCase("all") && CampaignMain.cm.getPlayer(playerName) == null) {
            CampaignMain.cm.toUser(
                    "update client param failed. Could not find player: " + playerName,
                    Username,
                    true);
            return;
        }
        ...
        // MWLogger.modLog(Username + " used update client param to update " + playerName + "'s " +
        // param + " to " + paramValue);
        CampaignMain.cm.doSendModMail(
                "WARNING",
                Username
                        + " used update client param to update "
                        + playerName
                        + "'s "
                        + param
                        + " to "
                        + paramValue);
        CampaignMain.cm.toUser(
                "you updated " + playerName + "'s " + param + " to " + paramValue, Username, true);
    } // end process()
