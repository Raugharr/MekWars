...
public class AdminRemoveUnitsOnMarketCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (toRemove.equalsIgnoreCase("all")) {
            ...
            MWServ.getInstance().sendChat(Username + " removed all units from the BM.");
            return;
        }
        ...
        if (auctionNumber > -1) {
            ...
            CampaignMain.cm.toUser(
                    "You removed auction #" + auctionNumber + " from the market.", Username, true);
            CampaignMain.cm.doSendModMail(
                    "NOTE", Username + " removed auction #" + auctionNumber + " from the market.");
        } else {

            SPlayer p = CampaignMain.cm.getPlayer(toRemove);
            if (p == null) {
                CampaignMain.cm.toUser(
                        "Couldn't find a player named " + toRemove + ".", Username, true);
                return;
            }

            if (!CampaignMain.cm.getMarket().hasActiveListings(p)) {
                CampaignMain.cm.toUser(
                        p.getName() + " doesn't have any running auctions.", Username, true);
                return;
            }
            ...
            CampaignMain.cm.toUser(
                    "You cancelled all of " + p.getName() + "'s auctions.", Username, true);
            CampaignMain.cm.toUser(
                    Username + " cancelled all of your running auctions.", p.getName(), true);
            CampaignMain.cm.doSendModMail(
                    "NOTE", Username + " cancelled all of " + toRemove + "'s auctions.");
        }
    }
}
