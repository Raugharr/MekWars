...
public class RemovePilotCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (type.equalsIgnoreCase("all")) {
            if (weight.equalsIgnoreCase("all")) {
                ...
                p.getPersonalPilotQueue()
                        .getPilotQueue(Unit.MEK, Unit.getWeightIDForName(weight))
                        .clear();
                p.getPersonalPilotQueue()
                        .getPilotQueue(Unit.PROTOMEK, Unit.getWeightIDForName(weight))
                        .clear();
                p.getPersonalPilotQueue()
                        .getPilotQueue(Unit.AERO, Unit.getWeightIDForName(weight))
                        .clear();
            }
        } else if (weight.equalsIgnoreCase("all")) {
            for (int weightClass = 0; weightClass <= Unit.ASSAULT; weightClass++) {
                p.getPersonalPilotQueue()
                        .getPilotQueue(Unit.getTypeIDForName(type), weightClass)
                        .clear();
            }
        ...
        CampaignMain.cm.doSendModMail(
                "NOTE", Username + " has removed pilots from " + player + "'s PPQ");
