...
public class AdminResetFactionComponentsCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (!command.hasMoreTokens()) {
            CampaignMain.cm.toUser(
                    "Improper command.  Try /adminResetFactionComponents " + syntax, Username);
            return;
        }
        ...
        if (requestedAmount.trim().equalsIgnoreCase("full")) {
            ...
            try {
                ...
                CampaignMain.cm.toUser(
                        "Improper amount.  Could not parse " + requestedAmount + " into Integer.",
                        Username);
                return;
            }
            if (amountToFillTo == 0) {
        ...
        if (fname.trim().equalsIgnoreCase("all")) {
            ...
            for (House faction : CampaignData.cd.getAllHouses()) {
                ...
                if (((SHouse) faction).isNewbieHouse()
                        || faction.getName().equalsIgnoreCase("None")) {
                    continue;
                }
                reset((SHouse) faction, action, amountToFillTo);
                CampaignMain.cm.toUser(
                        "You reset components for " + faction.getName(), Username, true);
                CampaignMain.cm.doSendModMail(
                        "NOTE", Username + " reset components for " + faction.getName());
            }
        } else {
            try {
            ...
            CampaignMain.cm.doSendModMail(
                    "NOTE", Username + " reset components for " + h.getName());
        }
    }
            ...
            for (int type = Unit.MEK; type < Unit.MAXBUILD; type++) {
                if (action == actionFill) {
                    h.addPP(
                            weight,
                            type,
                            h.getMaxAllowedPP(weight, type) - h.getPP(weight, type),
                            true);
                } else {
