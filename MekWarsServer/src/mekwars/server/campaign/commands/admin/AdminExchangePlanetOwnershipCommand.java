...
public class AdminExchangePlanetOwnershipCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (winningHouse == null) {
            CampaignMain.cm.toUser(
                    "Could not find a matching faction for the winner.", Username, true);
            return;
        }

        if (losingHouse == null) {
            CampaignMain.cm.toUser(
                    "Could not find a matching faction for the loser.", Username, true);
            return;
        }
        ...
        // server.MWLogger.modLog(Username + " took " + newAmount + "% of "+ planet.getName() + "
        // from "
        // + losingHouse.getName() + " and gave it to " + winningHouse.getName() + ".");
        CampaignMain.cm.toUser(
                "You took "
                        + newAmount
                        + "% of "
