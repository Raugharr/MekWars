    ...
    public void process(StringTokenizer command, String Username) {
        ...
        for (int weightClass = Unit.LIGHT; weightClass <= Unit.ASSAULT; weightClass++) {
            ...
            LinkedList<Pilot> currList =
                    p.getPersonalPilotQueue().getPilotQueue(Unit.MEK, weightClass);
            if (currList.size() != 0) {
        ...
        for (int weightClass = Unit.LIGHT; weightClass <= Unit.ASSAULT; weightClass++) {
            ...
            LinkedList<Pilot> currList =
                    p.getPersonalPilotQueue().getPilotQueue(Unit.PROTOMEK, weightClass);
            if (currList.size() != 0) {
        ...
        for (int weightClass = Unit.LIGHT; weightClass <= Unit.ASSAULT; weightClass++) {
            ...
            LinkedList<Pilot> currList =
                    p.getPersonalPilotQueue().getPilotQueue(Unit.AERO, weightClass);
            if (currList.size() != 0) {
        ...
        if (hasQueuedPilots) CampaignMain.cm.toUser("SM|" + toReturn.toString(), Username, false);
        else
            CampaignMain.cm.toUser(
                    "SM|" + p.getName() + " doesn't have any reserve pilots at the moment.",
                    Username,
                    false);

        CampaignMain.cm.doSendModMail(
                "NOTE", Username + " has viewed " + p.getName() + "'s pilot queue");
    }
}
