...
public class ModNoPlayCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "Improper format. Try: /c noplay#mode#lister#excludee", Username, true);
            return;
        }
        ...
        if (exList == null) {
            CampaignMain.cm.toUser(
                    "ERROR. " + listerName + "'s no-play list was null.", Username, true);
            return;
        }
        ...
        if (mode.equals("add")) {
            ...
            playerExists =
                    new File("./campaign/players/" + excludeName.toLowerCase() + ".dat").exists();
            ...
            if (listerName.toLowerCase().equals(excludeName)) {
                CampaignMain.cm.toUser(
                        "You can't put someone on his own no-play list. Jackass.", Username, true);
                return;
            }
            ...
            if (exclusionStatus == ExclusionList.ADMIN_EXCLUDED) {
                CampaignMain.cm.toUser(
                        excludeName + " is already on " + listerName + "'s mod/admin no-play list.",
                        Username,
                        true);
                return;
            }
            ...
            if (exclusionStatus == ExclusionList.PLAYER_EXCLUDED) {
                ...
                CampaignMain.cm.toUser(
                        Username + " removed " + excludeName + " from your no-play list.",
                        listerName,
                        true);
                CampaignMain.cm.toUser(
                        "PL|PEU|" + lister.getExclusionList().playerExcludeToString("$"),
                        listerName,
                        false);
            }
            ...
            try {
                ...
                CampaignMain.cm.toUser(
                        ...
                        true);
                CampaignMain.cm.toUser(
                        excludeName + " added to " + listerName + "'s mod/admin no-play list.",
                        Username,
                        true);
                CampaignMain.cm.toUser(
                        "PL|AEU|" + lister.getExclusionList().adminExcludeToString("$"),
                        listerName,
                        false);
                lister.setSave();

                // also inform mod channels
                // server.MWLogger.modLog(Username + " added " + excludeName + " to " + listerName +
                // "'s
                // mod/admin no-play list.");
            ...
            if (exclusionStatus == ExclusionList.NO_EXCLUSION) {
                CampaignMain.cm.toUser(
                        excludeName + " is not on either of " + listerName + "'s no-play lists.",
                        Username,
                        true);
                return;
            }
            ...
            try {
                ...
                if (exclusionStatus == ExclusionList.ADMIN_EXCLUDED) {
                    ...
                    // also inform mod channels
                    // server.MWLogger.modLog(Username + " removed " + excludeName + " from " +
                    // listerName +
                    // "'s Mod/Admin no-play list.");
                    CampaignMain.cm.doSendModMail(
                            "NOTE",
                    ...
                    CampaignMain.cm.toUser(
                            "You removed "
                                    ...
                                    + " from "
                                    + listerName
                                    + "'s standard no-play list.",
                            Username,
                            true);
                    // NOTE: the player removed isn't informed, since this was a standard list entry
                    // and,
                    // thus, supposed to be anonymous.
                    CampaignMain.cm.toUser(
                            "PL|PEU|" + lister.getExclusionList().playerExcludeToString("$"),
                            listerName,
                            false);
