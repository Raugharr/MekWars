...
/**
 * Ban a specific IP for a period of time. Banning by name while a player is online automatically
 * bans the IP he is connected from, but if he is banned while offline an explicit banip must be
 * used because /c ban detects the player's IP as "localhost."
 *
 * <p>Note that banip will NOT disconnect any player's currently online from the targetted address.
 * Always use /c ban for online players.
 *
 * <p>Syntax /c banip#address#duration
 */
public class BanIPCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            if (banTime.equalsIgnoreCase("perm")) {
                ...
                if (banValue.trim().equalsIgnoreCase("h")) {
                    ...
                    if (howlong / hour == 1) timeName = "for 1 hour";
                    else timeName = "for " + Long.toString(howlong / hour) + " hours";
                } else if (banValue.trim().equalsIgnoreCase("d")) {
                    howlong = Long.parseLong(banTime.substring(0, banTime.length() - 1)) * day;
                    if (howlong / day == 1) timeName = "for 1 day";
                    else timeName = "for " + Long.toString(howlong / day) + " days";
                } else if (banValue.trim().equalsIgnoreCase("w")) {
                    howlong = Long.parseLong(banTime.substring(0, banTime.length() - 1)) * week;
                    if (howlong / week == 1) timeName = "for 1 week";
                    else timeName = "for " + Long.toString(howlong / week) + " weeks";
                } else if (banValue.trim().equalsIgnoreCase("m")) {
                    howlong = Long.parseLong(banTime.substring(0, banTime.length() - 1)) * month;
                    if (howlong / month == 1) timeName = "for 1 month";
                    else timeName = "for " + Long.toString(howlong / month) + " months";
                } else if (banValue.trim().equalsIgnoreCase("y")) {
                    howlong = Long.parseLong(banTime.substring(0, banTime.length() - 1)) * year;
                    if (howlong / year == 1) timeName = "for 1 year";
                    else timeName = "for " + Long.toString(howlong / year) + " years";
                } else {
                    howlong = Long.parseLong(banTime) * minute;
                    if (howlong / minute == 1) timeName = "for 1 minute";
                    else timeName = "for " + Long.toString(howlong / minute) + " minutes";
                }
            } // end else(not perm)

        } catch (Exception e) {
            CampaignMain.cm.toUser(
                    "AM:Incorrect Syntax: Syntax is as follows<br>/c banip ip#time"
        ...
        if (ip != null) MWServ.getInstance().getBanIps().put(ip, until);
