...
public class ModDeactivateCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (targetPlayer.getDutyStatus() == SPlayer.STATUS_FIGHTING) {
            CampaignMain.cm.toUser(
                    targetPlayerString + " is fighting. Cancel his game before deactivating.",
                    Username,
                    true);
            return;
        }
        ...
        CampaignMain.cm.toUser(
                Username + " forced you to return to reserve duty.", targetPlayerString, true);
        CampaignMain.cm.toUser(
                "You forced " + targetPlayerString + " into reserve status.", Username, true);
        // server.MWLogger.modLog(Username + " force-deactivated " + targetPlayerString + ".");
        CampaignMain.cm.doSendModMail(
                "NOTE", Username + " force-deactivated " + targetPlayerString + ".'");
        CampaignMain.cm.sendPlayerStatusUpdate(
                targetPlayer, !Boolean.parseBoolean(CampaignMain.cm.getConfig("HideActiveStatus")));
    } // end process()
