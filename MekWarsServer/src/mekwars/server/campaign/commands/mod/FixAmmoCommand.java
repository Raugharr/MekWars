...
public class FixAmmoCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (target == null) {
            CampaignMain.cm.toUser(
                    "AM:Target player could not be found. Try again.", Username, true);
            return;
        }
        ...
        if (m == null) {
            CampaignMain.cm.toUser(
                    "AM:Target player doesn't have a unit with ID# " + unitID + ".",
                    Username,
                    true);
            return;
        }
        ...
        CampaignMain.cm.toUser(
                targetName + "'s " + m.getModelName() + "'s ammo has been fixed.", Username, true);
        CampaignMain.cm.toUser(
                Username
