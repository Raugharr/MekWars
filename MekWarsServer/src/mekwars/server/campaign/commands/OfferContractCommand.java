    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if ((receivingPlayer.getMyHouse()).isMercHouse()) {
            ...
            if (contractType == ContractInfo.CONTRACT_DELAY
                    ...
                    || contractType == ContractInfo.CONTRACT_UNITS) minContract /= 10;
            ...
            if (contractPayment < 2) {
                CampaignMain.cm.toUser(
                        "AM:You must pay a mercenary at least "
                                + CampaignMain.cm.moneyOrFluMessage(true, true, 2)
                                + ".",
                        Username,
                        true);
                ...
                CampaignMain.cm.toUser(
                        "AM:You may offer only one contract at a time.", Username, true);
            ...
            // make sure player can pay contract
                ...
                CampaignMain.cm.toUser(
                        "AM:Mercenaries may not employ other mercenaries", Username, true);
            ...
            // make sure that the merc is online. only loggedin players get contract offers.
                    ...
                    == false) {
                CampaignMain.cm.toUser(
                        "AM:You may only offer contracts to players currently online",
                        Username,
                        true);
            } // end elseif -> player offline
            else if (((MercHouse) (receivingPlayer.getMyHouse())).getContractInfo(receivingPlayer)
                    != null) { // if a contract already exists
                CampaignMain.cm.toUser(
                        "AM:You may not offer contracts to mercenaries with employers",
                        Username,
                        true);
                ...
                CampaignMain.cm.toUser(
                        offeringPlayer.getName()
                                ...
                                + " has offered you "
                                + CampaignMain.cm.moneyOrFluMessage(true, false, contractPayment)
                                + " for "
                                + contractDuration
                                + " "
                                + strContractType
                ...
                CampaignMain.cm
                        .getUnresolvedContracts()
                        .add(newContract); // add newContract to vector of contracts.
                CampaignMain.cm.getUnresolvedContracts().trimToSize();
                CampaignMain.cm.toUser(
                        "AM:Accept this contract by <a href=\"MEKWARS/c acceptcontract#"
                                + offeringPlayer.getName()
                                + "\">clicking here</a>",
                        receivingPlayerName,
                        true);
                CampaignMain.cm.toUser(
                        "AM:Decline this contract by <a href=\"MEKWARS/c refusecontract#"
                                + offeringPlayer.getName()
                                + "\">clicking here</a>",
                        receivingPlayerName,
                        true);
            ...
            CampaignMain.cm.toUser(
                    "AM:You may not offer contracts to non-mercenary players.", Username, true);
        } // end else -> not merc.
