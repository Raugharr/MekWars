...
/**
 * @author Salient for the SolFreeBuild option. This will allow SOL players to remove units they
 *     created from their inventory
 */
public class SolDeleteUnitCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (u == null) {
            CampaignMain.cm.toUser(
                    "Target player doesn't have a unit with ID# " + unitID + ".", Username, true);
            return;
        }

        // tell the player
        CampaignMain.cm.toUser(
                "AM:" + Username + "'s " + u.getModelName() + " was removed.", Username, true);

        p.removeUnit(unitID, true);

        // if the limit is on for SOL players, they have the ability to use this command to delete
        // units
        // So, when they do, we need to subtract one from their free mek counter.
        // though.. i think this may be dead code, pretty sure if there is a limit the del option
        // wont
        // show
