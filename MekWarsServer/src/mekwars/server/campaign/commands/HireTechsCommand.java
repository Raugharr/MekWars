...
public class HireTechsCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (techCost > p.getMoney()) {
            CampaignMain.cm.toUser(
                    "AM:Hiring "
                            + numtohire
                            + " techs will cost you "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, techCost)
                            + ". You only have "
                            + p.getMoney()
                            + " "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, p.getMoney())
                            + ".",
                    Username,
                    true);
            return;
        ...
        if (maxTechs != -1 && (p.getTechnicians() + numtohire > maxTechs)) {
            CampaignMain.cm.toUser(
                    "AM:Sorry but the max number of technicians you can hire is " + maxTechs + ".",
                    Username,
                    true);
            return;
        }
    ...
    private void hireAdvanceTechs(StringTokenizer command, String Username) {
        ...
        if (techType > maxLevelTechHire) {
            CampaignMain.cm.toUser(
                    "AM:Sorry there are no techs of that skill level on the market.",
                    Username,
                    true);
            return;
        }
        ...
        int hireCost =
                Integer.parseInt(
                        house.getConfig(UnitUtils.techDescription(techType) + "TechHireCost"));
        ...
        if (player.getMoney() < hireCost) {
            CampaignMain.cm.toUser(
                    "AM:Hiring "
                            ...
                            + " techs will cost you "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, hireCost)
                            + ". You only have "
                            + player.getMoney()
                            + " "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, player.getMoney())
                            + ".",
                    Username,
                    true);
            return;
