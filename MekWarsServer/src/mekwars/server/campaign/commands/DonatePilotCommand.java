...
public class DonatePilotCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.cm.toUser(
                    "AM:You may not donate any of your pilots while you are on welfare.",
                    Username,
                    true);
        ...
        if (p.getMyHouse().isNewbieHouse()) {
            CampaignMain.cm.toUser(
                    "AM:SOL Players are not allowed to donate pilots, sorry!", Username, true);
            return;
        }
        ...
        if (Boolean.parseBoolean(house.getConfig("DonatingCostsBills"))) {
            ...
            if ((p.getMoney() >= mechdonateprize) && (p.getInfluence() >= infToDonate)) {
                ...
                CampaignMain.cm.toUser(
                        ...
                        Username,
                        true);
                if (p.getMoney() >= mechdonateprize) p.addMoney(-mechdonateprize);
                else {
                ...
                CampaignMain.cm.toUser(
                        "AM:You can't afford to retrain this pilot. You need "
                                + CampaignMain.cm.moneyOrFluMessage(true, true, mechdonateprize)
                                + " and "
                                + CampaignMain.cm.moneyOrFluMessage(false, false, infToDonate)
                                + ".",
                        Username,
                        true);
        ...
        String toUser =
                "AM:You've sent pilot "
                        + pilot.getName()
                        + " back to the faction for more training";
        if (mechdonateprize > 0)
            toUser +=
                    ". Your faction charges you "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, mechdonateprize)
                            + " for the transfer";
        toUser += ".";
        ...
        CampaignMain.cm.toUser(
                "PL|RPPPQ|" + unitType + "|" + weightClass + "|" + pilotLocation, Username, false);
