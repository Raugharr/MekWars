...
public class RefreshFactoryCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (!CampaignMain.cm.getBooleanConfig("AllowFactoryRefreshForRewards") && !useFlu) {
            CampaignMain.cm.toUser(
                    "AM:You may not use "
                            ...
                            + " to refresh a factory on this server.",
                    Username,
                    true);
            return;
        }
        ...
        if (uf == null) {
            CampaignMain.cm.toUser(
                    "AM:Could not find factory: " + factoryName + ".", Username, true);
            return;
        }
        ...
        if (!useFlu) player.addReward(-rpCost);
        else player.addInfluence(-fluCost);
        ...
        CampaignMain.cm.toUser(
                "AM:You refreshed " + uf.getName() + " on planet " + p.getName(), Username, true);
