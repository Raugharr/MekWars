...
public class ListCommandsCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) starter = command.nextToken();

        while (commands.hasMoreElements()) {

            String commandName = commands.nextElement();

            if (commandName.equalsIgnoreCase("SendClientDataCommand")) continue;
            Command commandMethod = commandTable.get(commandName);

            if (starter.trim().length() > 0) {
                if (userLevel < commandMethod.getExecutionLevel()) continue;
                else if (commandName.toLowerCase().indexOf(starter.toLowerCase()) > -1) {
                    String syntax =
                            commandMethod.getSyntax().trim().length() < 1
                                    ? " "
                                    : commandMethod.getSyntax();
                    commandArray[x] =
                            commandName.substring(0, 1)
                                    + commandName.substring(1, commandName.length()).toLowerCase()
                                    + "~"
                                    + syntax
                                    + "~"
        ...
        if (x < commandTable.size()) {
            ...
            for (int count = 0; count < x; count++) tempArray[count] = commandArray[count];
