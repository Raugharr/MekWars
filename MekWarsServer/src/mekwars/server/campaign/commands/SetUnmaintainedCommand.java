...
public class SetUnmaintainedCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
            CampaignMain.cm.toUser(
                    "AM:You may not unmaintain a unit while active.", Username, true);
            return;
        }

        if (p.getDutyStatus() == SPlayer.STATUS_FIGHTING) {
            CampaignMain.cm.toUser(
                    "AM:You may not unmaintain a unit while engaged!.", Username, true);
            return;
        }
        ...
        CampaignMain.cm.toUser(
                "PL|SUS|" + unitToSet.getId() + "#" + Unit.STATUS_UNMAINTAINED, Username, false);
