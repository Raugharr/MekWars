...
public class LinkUnitCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) {
            ...
            if (a.isLocked()) {
                CampaignMain.cm.toUser(
                        "AM:You may not change C3 networks while an Army is in combat.",
                        Username,
                        true);
                return;
            }
            ...
            if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
                CampaignMain.cm.toUser(
                        "AM:You may not change C3 networks while on active duty.", Username, true);
                return;
            }
            ...
            if (slaveUnit == null) {
                CampaignMain.cm.toUser(
                        "AM:Could not find unit #" + slaveid + " in army #" + armyid + ".",
                        Username,
                        true);
                return;
            }
            ...
            if (slaveUnit.hasBeenC3LinkedTo(a) && masterid == -1) {
                ...
                String toReturn =
                        "AM:Unit #"
                                + slaveid
                                + " was removed from its C3 network. New BV: "
                                + a.getBV();
