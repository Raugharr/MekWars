...
public class ActivateCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            // if (!(MWServ.SERVER_VERSION).substring(0,
            // server.MWServ.SERVER_VERSION.lastIndexOf(".")).equals(p.getPlayerClientVersion().substring(0, p.getPlayerClientVersion().lastIndexOf(".")))) {
            if (!MWServ.SERVER_VERSION.is(p.getPlayerClientVersion())) {
                // server.MWLogger.modLog(Username + " failed to activate. Was using version " +
                // p.getPlayerClientVersion()+" Server Version: "+
            ...
            MWLogger.errLog(
                    "Error activating player. User reported client verson: "
                            + p.getPlayerClientVersion()
                            + " --- Stack Trace Follows.");
            // MWLogger.errLog(ex);
            CampaignMain.cm.toUser(
        ...
        if (p.getArmies().size() == 0 && !MULOnlyOps) {
            CampaignMain.cm.toUser(
                    "AM:You must have armies constructed in order to activate.", Username, true);
            return;
        }
        ...
        if (enabledArmies == 0 && !MULOnlyOps) {
            CampaignMain.cm.toUser(
                    "AM:You must have at least 1 enabled army to activate.", Username, true);
            return;
        }
        ...
        for (SArmy currA : p.getArmies()) {
            ...
            if (currA.getAmountOfUnits() == 0 && !MULOnlyOps) {
                CampaignMain.cm.toUser(
                        "AM:You may not activate with empty armies.", Username, true);
                return;
            }
            ...
            for (Unit currU : currA.getUnits()) {
                ...
                if (currU.hasVacantPilot()) {
                    CampaignMain.cm.toUser(
                            "AM:You may not activate with pilotless units!", Username, true);
                    return;
                }
            }
        }
        ...
        if (!CampaignMain.cm.getBooleanConfig("allowGoingActiveWithoutUnitCommanders")
            ...
            CampaignMain.cm.toUser(
                    "AM:You may not activate with armies lacking unit commanders!", Username, true);
            return;
        }
        ...
        if (CampaignMain.cm.isUsingAdvanceRepair()) {
            ...
            if (armiesContainLeggedUnit(p.getArmies())) {
                CampaignMain.cm.toUser(
                        "AM:You may not activate with a legless unit in an army.", Username, true);
                return;
            }
            ...
            if (CampaignMain.cm.getBooleanConfig("UseSimpleRepair")
                    && armiesDamagedUnits(p.getArmies())) {
                CampaignMain.cm.toUser(
                        "AM:You may not activate while units in your armies have damage.",
                        Username,
                        true);
                return;
            }

            if (!CampaignMain.cm.getBooleanConfig("AllowUnitsToActivateWithPartialBins")
            ...
            if ((!CampaignMain.cm.getBooleanConfig("AllowActivationWithDamagedUnits"))
        ...
        if (p.getFreeBays() < 0
                && CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForActivation") > -1
                && ((p.getFreeBays()
                                + CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForActivation"))
                        < 0)) {
            if (CampaignMain.cm.isUsingAdvanceRepair()) {
                CampaignMain.cm.toUser(
                        "AM:You may not activate with more than "
                                + CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForActivation")
                                + " negative bays!",
                        Username,
                        true);
        ...
        if (CampaignMain.cm.getBooleanConfig("DisableActivationIfOverHangarLimits")
        ...
        if (CampaignMain.cm.getBooleanConfig("FreeBuild_LimitGoActive") && p.hasUnusedMekTokens()) {
            CampaignMain.cm.toUser(
                    "AM: To go active you must first finish creating your free meks.",
                    Username,
                    true);
            return;
        }
        ...
        MWScheduler.getInstance()
                .activateUser(
                        p.getName(), p.getWeightedArmyNumber(), p.getHouseFightingFor().getName());

        p.setActive(true);

        CampaignMain.cm.toUser("AM:[!] You're on your way to the front lines.", Username, true);
        CampaignMain.cm.sendPlayerStatusUpdate(
                p, !Boolean.parseBoolean(CampaignMain.cm.getConfig("HideActiveStatus")));
    ...
    /**
     * Method which returns a boolean indicating whether any unit in a set of armies has 3 engine
     * crits.
     *
     * <p>Separate method, instead of inline in process(), so this can be moved into SPlayer easily
     * if other code needs to do engine checks.
     */
    ...
    /**
     * Method which returns a boolean indicating whether any unit in a set of armies is missing a
     * leg.
     *
     * <p>Separate method, instead of inline in process(), so this can be moved into SPlayer easily
     * if other code needs to do leglessness (sp?) checks.
     */
     ...
     *
     * <p>Separate method, instead of inline in process(), so this can be moved into SPlayer easily
     * if other code needs to do damaged unit checks.
     */
    ...
    private int hasNoAttackOptions(SPlayer player, Vector<SArmy> armies) {
        ...
        for (SArmy army : armies) {
            ...
            boolean canAttack = false;
            boolean requireAttackCapable =
                    CampaignMain.cm.getBooleanConfig("RequireAttackCapableArmiesForActivation");
...
/**
 * @author urgru
 *     <p>private thread. simple minactivetime wait which runs a checkattack for the activating
 *     player.
 */
    ...
    public synchronized void run() {
        try {
            ...
            if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
                ...
                CampaignMain.cm.toUser(
                        "<br>You have arrived on the front lines!", p.getName(), true);
