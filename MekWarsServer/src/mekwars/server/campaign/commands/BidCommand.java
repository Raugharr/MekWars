...
public class BidCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p == null) {
            CampaignMain.cm.toUser(
                    "AM:Null SPlayer while bidding. Report immediately!", Username, true);
            return;
        }
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Improper format. Try: /c bid#AuctionID#Amount", Username, true);
            return;
        }
        ...
        if (p.getExperience() < minBMEXP) {
            CampaignMain.cm.toUser(
                    "AM:You are not allowed to buy units from the Market. Required Experience: "
                            + minBMEXP
                            + ".",
                    Username,
                    true);
        ...
        if (auction == null) {
            CampaignMain.cm.toUser(
                    "AM:There is no auction with ID#" + auctionID + ".", Username, true);
            return;
        }
        ...
        if (bidAmount < minBid) {
            if (!CampaignMain.cm.getBooleanConfig("HiddenBMUnits")) {
                CampaignMain.cm.toUser(
                        "AM:Minimum bid for the "
                                + auction.getListedModelName()
                                + " is "
                                + CampaignMain.cm.moneyOrFluMessage(true, false, minBid)
                                + ". Nice try.",
                        Username,
                        true);
            } else {
        ...
        boolean canBuy =
                CampaignMain.cm.getHouseForPlayer(p.getName()).canBuyFromBM(uType, uWeight);
        //		boolean canBuy = p.getMyHouse().canBuyFromBM(uType, uWeight);
        if (!canBuy) {
            CampaignMain.cm.toUser(
                    "AM:Your faction is not allowed to purchase "
                            + Unit.getWeightClassDesc(uWeight)
                            + " "
                            + Unit.getTypeClassDesc(uType)
        ...
        CampaignMain.cm.toUser(
                "AM:You bid "
                        + CampaignMain.cm.moneyOrFluMessage(true, false, bidAmount)
                        + " for the "
