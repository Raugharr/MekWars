...
public class SetMaintainedCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (CampaignMain.cm.isUsingAdvanceRepair()) return;
        ...
        if (unitToSet == null) {
            CampaignMain.cm.toUser(
                    "AM:Invalid id number. Make sure you're using the right unit number.",
                    Username,
                    true);
            return;
        }
        ...
        if (p.getFreeBays() < unitSpace) {
            ...
            String toReturn =
                    "AM:You must hire "
                            ...
                            + ".<br>";
            toReturn +=
                    "AM:<a href=\"MEKWARS/c hireandmaintain#"
                            + numTechs
                            + "#"
                            + numtoset
        ...
        CampaignMain.cm.toUser(
                "PL|SUS|" + unitToSet.getId() + "#" + Unit.STATUS_OK, Username, false);
        CampaignMain.cm.toUser(
                unitToSet.getPilot().getName()
                        + "'s "
