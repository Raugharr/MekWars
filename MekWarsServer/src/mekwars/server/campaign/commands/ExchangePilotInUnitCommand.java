...
public class ExchangePilotInUnitCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (!Boolean.parseBoolean(CampaignMain.cm.getConfig("AllowPersonalPilotQueues"))) return;
        ...
        if (command.hasMoreElements()) {
            ...
            if (m != null) {
                ...
                if (!m.isSinglePilotUnit()) {
                    CampaignMain.cm.toUser(
                            "AM:You may not remove that pilot from this unit.", Username, true);
                    return;
                }
                ...
                int capSize =
                        CampaignMain.cm.getIntegerConfig("MaxAllowedPilotsInQueueToBuyFromHouse");

                if (newPilotId == -1
                        && p.getPersonalPilotQueue()
                                        .getPilotQueue(m.getType(), m.getWeightclass())
                                        .size()
                                >= capSize) {
                    CampaignMain.cm.toUser(
                            "AM:There are no free beds in the barracks "
                                    + pilot.getName()
                                    + " will have to sleep in his unit.",
                            Username);
                    return;
                }
                // issues where protomeks are getting set to the wrong unit type so they become Mek
                // pilots.
                ...
                // m.setExperience(new Integer(0)); -- trying to decide if I want to keep it this
                // way or the
                // old way Torren.

                // CampaignMain.cm.toUser("PL|PPQ|"+p.getPersonalPilotQueue().toString(true),Username,false);
                CampaignMain.cm.toUser(
                        "PL|UU|" + m.getId() + "|" + m.toString(true), Username, false);
