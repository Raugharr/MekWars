 ...
 *
 * <p>Syntax /c Register#Name,Password
 */
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            if (regged && !MWServ.getInstance().isAdmin(Username)) {
                CampaignMain.cm.toUser(
                        "AM:Nickname \"" + regname + "\" is already registered!", Username);
                // MWLogger.modLog(Username + " tried to register the nickname \"" + regname + "\",
                // which
            ...
            if (pw.length() < 3 && pw.length() > 11) {
                CampaignMain.cm.toUser(
                        "AM:Passwords must be between 4 and 10 characters!", Username);
                return;
            }
