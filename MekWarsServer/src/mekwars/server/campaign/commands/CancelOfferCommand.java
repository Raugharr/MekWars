    ...
    public void process(StringTokenizer command, String Username) {
        ...
        for (int i = 0; i < CampaignMain.cm.getUnresolvedContracts().size(); i++) {
            ...
            if (info.getOfferingPlayerName().equalsIgnoreCase(Username)) {
                // if contract belong to offering player, remove the contract and set boolean to
                // true
        ...
        if (contractRemoved == true) {
            CampaignMain.cm.toUser(
                    "AM:You have cancelled your offer to " + receivingPlayerName, Username, true);
            CampaignMain.cm.toUser(
                    Username + " has rescinded his contract offer", receivingPlayerName, true);
        } // end if(a contract was removed)
        else if (contractRemoved == false) {
            CampaignMain.cm.toUser(
                    "AM:There was no outstanding contract to cancel!", Username, true);
