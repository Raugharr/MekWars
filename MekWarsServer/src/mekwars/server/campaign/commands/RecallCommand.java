...
public class RecallCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p == null) {
            CampaignMain.cm.toUser(
                    "AM:Null SPlayer while recalling unit. Report immediately!", Username, true);
            return;
        }
        ...
        if (!auction.getSellerName().equalsIgnoreCase(Username)) {
            CampaignMain.cm.toUser(
                    "AM:Only the selling player may terminate an auction.", Username, true);
            return;
        }
        ...
        if (auction.getAllBids().size() > 0) {
            CampaignMain.cm.toUser(
                    "AM:There are bids on the "
                            + auction.getListedModelName()
                            + ". Sale may not be stopped.",
                    Username,
                    true);
            return;
        }
        ...
        CampaignMain.cm.toUser(
                "The " + auction.getListedModelName() + " is no longer for sale on the Market.",
                Username,
                true);
