...
public class AcceptContractCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        for (int i = 0; i < CampaignMain.cm.getUnresolvedContracts().size(); i++) {
            ...
            if (CampaignMain.cm.getPlayer(receivingPlayerName)
        ...
        if (contractAccepted) {
            CampaignMain.cm.toUser(
                    "AM:You have accepted the contract offered by "
                            + offeringPlayerName
                    ...
                    true);
            CampaignMain.cm.toUser(
                    claimingPlayerName + "has accepted your contract offer.",
                    offeringPlayerName,
                    true);
            // do finances.
            ...
            CampaignMain.cm.toUser(
                    "AM:Received "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, immediatePay)
            ...
            CampaignMain.cm.toUser(
                    "AM:You have spent "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, contractPay)
                            + " to hire "
                            + claimingPlayerName,
                    offeringPlayerName,
                    true);
            CampaignMain.cm.toUser(
                    "PL|SHFF|" + claimingPlayer.getHouseFightingFor().getName(),
            ...
            CampaignMain.cm.toUser(
                    "AM:You have no contract to accept from a player of the given name",
                    Username,
                    true);
