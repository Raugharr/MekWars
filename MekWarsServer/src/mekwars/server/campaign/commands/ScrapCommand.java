...
public class ScrapCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (house.isNewbieHouse()) {
            CampaignMain.cm.toUser(
                    "AM:SOL players may not Sell, Scrap or Donate their units!", Username, true);
            return;
        }

        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.cm.toUser(
                    "AM:You may not scrap any of your units while you are on welfare.",
                    Username,
                    true);
            return;
        }
        ...
        if (m.getStatus() == Unit.STATUS_FORSALE) {
            CampaignMain.cm.toUser(
                    "AM:Units that are for sale on the Market may not be scrapped.",
                    Username,
                    true);
            return;
        }
        ...
        if (p.getAmountOfTimesUnitExistsInArmies(mechid) > 0
            ...
            CampaignMain.cm.toUser(
                    "AM:You may not scrap units which are in active armies.", Username, true);
            return;
        } // end (unit is in armies and player is active)

        if (p.isUnitInLockedArmy(m.getId())) {
            CampaignMain.cm.toUser(
                    "AM:You may not scrap units which are in fighting armies.", Username, true);
            return;
        }

        // If he has not scrapped this tick and Scrapping is allowed, OR if the entity was salvaged
        // recently
        if (p.getScrapsThisTick() >= scrapsAllowed && m.getScrappableFor() <= 0) {
            CampaignMain.cm.toUser(
                    "AM:You may only scrap " + scrapsAllowed + " unit(s) per tick.",
                    Username,
                    true);
            return;
        }
        ...
        if (p.getMoney() < moneyToScrap
                || p.getInfluence() < infToScrap && m.getScrappableFor() < 0) {
            CampaignMain.cm.toUser(
                    "AM:You cannot afford to scrap this unit. You need "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, moneyToScrap)
                            + " and "
                            + CampaignMain.cm.moneyOrFluMessage(false, true, infToScrap)
                            + ".",
                    Username,
                    true);
            return;
        ...
        // Give the player the amount the unit can be scrapped for (post-game), or add/deduct the
        // standard cost
            ...
            CampaignMain.cm.toUser(
                    "AM:You scrapped the "
                            ...
                            + CampaignMain.cm.moneyOrFluMessage(
                                    true, true, m.getScrappableFor(), true)
                            + ").",
                    Username,
                    true);
        } else {
            // it was intentional to allow the post-game scrap to process without a confirm... you
            // don't
            // want the guy getting a message "scrap for +1 cbill" then getting "scrapped for
            // -1800cb"
            // because of the time lag
            if (!strConfirm.equals("CONFIRM")) {
                String result =
                        "AM:Quartermaster command will charge you "
                                + CampaignMain.cm.moneyOrFluMessage(true, false, moneyToScrap)
                                + " to scrap #"
                                + m.getId()
                                + " "
                                + m.getModelName()
                ...
                CampaignMain.cm.toUser(
                        "AM:You scrapped the "
                                + m.getModelName()
                                + " ("
                                + CampaignMain.cm.moneyOrFluMessage(true, true, -moneyToScrap, true)
                                + ", "
                                + CampaignMain.cm.moneyOrFluMessage(false, true, -infToScrap, true)
                                + ").",
                        Username,
                        true);
            }
        ...
        if (p.mayAcquireWelfareUnits())
            CampaignMain.cm.doSendModMail(
                    "NOTE", Username + " scrapped a unit and sent himself into welfare.");
        ...
        OpsScrapThread scrapT =
                CampaignMain.cm.getOpsManager().getScrapThreads().get(p.getName().toLowerCase());
        ...
        if (Boolean.parseBoolean(house.getConfig("AllowPersonalPilotQueues"))
            ...
            CampaignMain.cm.toUser(
                    "PL|AP2PPQ|"
                            ...
                            + m.getWeightclass()
                            + "|"
                            + pilot.toFileFormat("#", true),
                    Username,
                    false);
            CampaignMain.cm.toUser(
                    pilot.getName() + " was moved to your barracks.", Username, true);
