...
public class AcceptAttackFromReserveCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (dp == null) {
            CampaignMain.cm.toUser(
                    "AM:Null player in AcceptAttackFromReserve. Report this immediately!",
                    Username,
                    true);
            return;
        }
        ...
        if (aa == null) {
            CampaignMain.cm.toUser(
                    "AM:Defend failed. Attacker does not have an army with ID #" + armyID + ".",
                    Username,
                    true);
            return;
        }
        ...
        if ((CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForAFR") > -1)
                && ((dp.getFreeBays() + CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForAFR"))
                        < 0)) {
            CampaignMain.cm.toUser(
                    "AM:Defend failed. " + dp.getName() + " has too many negative bays.",
                    ap.getName(),
                    true);
            CampaignMain.cm.toUser(
                    "AM:Defend failed.  You have too many negative bays.", Username, true);
            return;
        }
        ...
        ArrayList<Integer> defensiveFailures =
                manager.getShortValidator().validateShortDefender(dp, da, o, target);
        if (defensiveFailures.size() == 0) {
            ...
            CampaignMain.cm.toUser(
                    "AM:Army #"
                            + da.getID()
                            + "could not defend "
                            + manager.getShortValidator().failuresToString(defensiveFailures),
                    Username,
                    true);
            return;
        }
        ...
        for (Unit currU : da.getUnits()) {
            if (currU.hasVacantPilot()) {
                CampaignMain.cm.toUser(
                        "AM:You may not defend using an army with pilotless units.",
                        Username,
                        true);
                return;
            }
        ...
        ShortOperation so =
                new ShortOperation(o.getName(), target, ap, aa, fullMatches, opID, -1, true);
