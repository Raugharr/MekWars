...
public class ToggleArmyDisabledCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p == null) {
            CampaignMain.cm.toUser(
                    "AM:Null Player while Disabling/Enabling army. Report This!.", Username, true);
            return;
        }
        int aid = -1;
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Improper format. Try: /c togglearmydisabled#ID", Username, true);
            return;
        }
        ...
        CampaignMain.cm.toUser(
                "AM:Army " + army.getID() + (army.isDisabled() ? " disabled." : " enabled."),
                Username,
                true);
    }
}
