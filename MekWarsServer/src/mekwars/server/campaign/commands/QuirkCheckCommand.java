...
public class QuirkCheckCommand implements Command {
    ...
    private boolean checkAccess() {
        if (accessLevel != 0) {
            if (userLevel < getExecutionLevel()) {
                player.toSelf(
                        "AM:Insufficient access level. Level: "
                                + userLevel
                                + ". Required: "
                                + accessLevel
                                + ".");
    ...
    private void compareLengths() {
        ...
        if (serverCanonQuirkLength != clientCanonQuirkLength
                || serverCustomQuirkLength != clientCustomQuirkLength) {
            CampaignMain.cm.doSendModMail(
                    username, " is hosting with quirk files that do not match server!");
            MWLogger.errLog(username + " is hosting with quirk files that do not match server!");
            CampaignMain.cm.doSendErrLog(
                    username + " is hosting with quirk files that do not match server!");
            player.toSelf(
                    "AM: Your files do not match the server, run autoupdate before hosting a match!");
        }

        // player.toSelf("AM: DEBUG: " + serverCanonQuirkLength + clientCanonQuirkLength +
        // serverCustomQuirkLength + clientCustomQuirkLength);
    }
}
