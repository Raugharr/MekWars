...
public class BuyPartsCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (bme == null
                || (!allowTechCrossOver
                        && !UnitUtils.isSameTech(
                                bme.getTechLevel(), player.getMyHouse().getTechLevel()))) {
        ...
        if (numtobuy > bme.getAmount()) {
            CampaignMain.cm.toUser(
                    "AM:There are only " + bme.getAmount() + " crits left select a smaller amount",
                    Username);
            return;
        }
        ...
        if (cost > player.getMoney()) {
            CampaignMain.cm.toUser(
                    "AM:It will cost you "
                            ...
                            + " you only have "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, player.getMoney())
                            + ".",
                    Username);
            return;
        }
