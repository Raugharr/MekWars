...
public class TransferUnitCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Improper format. Try: /c transferunit#TargetPlayer#UnitID", Username, true);
            return;
        }
        ...
        boolean usesTechs =
                CampaignMain.cm.getBooleanConfig("UseTechnicians")
        ...
        if (player.mayAcquireWelfareUnits()) {
            CampaignMain.cm.toUser(
                    "AM:You may not transfer any of your units while you are on welfare.",
                    Username,
                    true);
            return;
        }
        ...
        if (player.getAmountOfTimesUnitExistsInArmies(unitid) > 0
            ...
            CampaignMain.cm.toUser(
                    "AM:You may not tranfer units which are in active armies.", Username, true);
            return;
        }

        // for (SArmy currA : player.getArmies()) {
        //	if (currA.getUnit(unitid) != null && currA.isLocked()) {
        //		CampaignMain.cm.toUser("You may not transfer units which are in fighting armies.",
        // Username,
        // true);
        //		return;
        //	}
        ...
        if (!targetplayer.getMyHouse().equals(player.getMyHouse())
                && !targetplayer
                        .getMyHouse()
                        .getHouseFightingFor(targetplayer)
                        .equals(player.getMyHouse())) {
            CampaignMain.cm.toUser(
                    "AM:"
            ...
            // Target has no room?
                        ...
                        < SUnit.getHangarSpaceRequired(m, targetplayer.getMyHouse())
                && !usesTechs) {
            // on a tech server, can accept units past limit. theyre just marked unmaintained
            CampaignMain.cm.toUser(
                    "AM:" + targetplayer.getName() + " has no room for that unit.", Username, true);
            return;
        ...
        if (senderCost > player.getMoney()) {
            CampaignMain.cm.toUser(
                    "AM:You tried to send "
                            + StringUtils.aOrAn(modName, true)
                            + " to "
                            + targetPlayer
                            + ", but you cannot afford the transfer payment ("
                            + CampaignMain.cm.moneyOrFluMessage(true, true, senderCost)
                            + ").",
                    Username,
                    true);
            return;
        ...
        if (receiverCost > targetplayer.getMoney()) {
            ...
            CampaignMain.cm.toUser(
                    "AM:"
                            ...
                            + CampaignMain.cm.moneyOrFluMessage(true, true, receiverCost)
                            + ").",
                    targetPlayer,
                    true);
            return;
        ...
        if (m.getMaintainanceLevel() <= scrapLevel && !confirmedSend) {
            CampaignMain.cm.toUser(
                    "AM:The unit you are trying to tranfer is not well maintained."
            ...
            if (rnd > m.getMaintainanceLevel()) {
                ...
                if (house.getIntegerConfig("ScrapsAllowed") > 0) {
                    mechscrapprice =
                            (int)
                                    (player.getMyHouse()
                                                    .getPriceForUnit(
                                                            m.getWeightclass(), m.getType())
                                            * Double.parseDouble(
                                                    house.getConfig("ScrapCostMultiplier")));
                ...
                String toSend =
                        "AM:The " + modName + " didn't survive transit intact. HQ is displeased (";
                if (mechscrapprice > 0)
                    toSend +=
                            CampaignMain.cm.moneyOrFluMessage(true, false, -mechscrapprice, true)
                                    + ", ";
        ...
        String toSender =
                "AM:You transferred the " + modName + " to " + targetplayer.getName() + ".";
        if (senderCost > 0) {
            toSender +=
                    " Paid "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, senderCost)
                            + " to your technicians.";
            player.addMoney(-senderCost);
        }

        String toReceiver =
                "AM:" + Username + " sent you " + StringUtils.aOrAn(modName, true) + ".";
        if (receiverCost > 0) {
            toReceiver +=
                    "Paid "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, receiverCost)
                            + " to your technicians.";
