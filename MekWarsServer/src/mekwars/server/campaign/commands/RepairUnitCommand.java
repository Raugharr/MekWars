        ...
        try {
            ...
            int cost =
                    CampaignMain.cm.getRepairCost(
                            entity, location, slot, techType, armor, techWorkMod);
            ...
            if (techType != UnitUtils.TECH_REWARD_POINTS && cost > player.getMoney()) {
                CampaignMain.cm.toUser(
                        "FSM|You do not have enough "
                                + CampaignMain.cm.moneyOrFluMessage(true, false, -cost)
            ...
            if (player.getDutyStatus() == SPlayer.STATUS_ACTIVE
                ...
                CampaignMain.cm.toUser(
                        "FSM|You may not repair that unit while it is in an active army.",
                        Username,
                        false);
                return;
            }
            ...
            if (numberOfTechs <= 0) {
                CampaignMain.cm.toUser(
                        "FSM|You do not have any "
                                + UnitUtils.techDescription(techType)
                                + " techs to do this repair!",
                        Username,
                        false);
                return;
            }
            ...
            if (techType != UnitUtils.TECH_REWARD_POINTS
                ...
                // MWLogger.errLog(crit+" Crits: "+player.getUnitParts().getPartsCritCount(crit)+"
                // Needed:
                // "+damagedCrits);
                if (player.getPartsAmount(crit) < damagedCrits) {
                    ...
                    String critPrettyname =
                            UnitUtils.getCritExternalName(entity, slot, location, armor);
                    CampaignMain.cm.toUser(
                            "FSM|You do not have enough "
                                    + critPrettyname
                                    + " crits to repair this.",
                            Username,
                            false);
                    return;
                }
            ...
            if (armor) {
                ...
                if (slot < UnitUtils.LOC_INTERNAL_ARMOR) {
                    ...
                    if (entity instanceof Tank) rear = false;
                    else {
                        switch (location) {
                            ...
                            case UnitUtils.LOC_RTR:
                                ...
                                break;
                            default:
                                if (slot == UnitUtils.LOC_REAR_ARMOR) rear = true;
                                else rear = false;
                                break;
                        }
                    }
                    ...
                    if (rear)
                        repairMessage =
                                "Repairs have begun on the external armor("
                                        + entity.getLocationAbbr(tabLocation)
                                        + "r) of your "
                                        + entity.getShortNameRaw()
                                        + ".  <b>At a Cost of "
                                        + CampaignMain.cm.moneyOrFluMessage(true, true, cost)
                                        + "</b>";
                    else
                        repairMessage =
                                "Repairs have begun on the external armor("
                    ...
                    repairMessage =
                            "Repairs have begun on the internal structure("
            ...
            if (techType == UnitUtils.TECH_REWARD_POINTS) {
                ...
                if (CampaignMain.cm.getBooleanConfig("UsePartsRepair")) {
                    String crit = UnitUtils.getCritName(entity, slot, location, armor);
                    int damagedCrits =
                            UnitUtils.getNumberOfDamagedCrits(entity, slot, location, armor);
            ...
            if (sendDialogUpdate) CampaignMain.cm.toUser("ARD|" + unitID, Username, false);
        } catch (Exception ex) {
