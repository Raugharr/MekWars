...
/**
 * @author Torren Aug 28, 2004 allows users to redeem award points. they can redeem for techs,
 *     influence, or units syntax for techs and influence: /c
 *     userewardpoints#typeofreward#amountofrewardpointstouse syntax for units /c
 *     userewardpoints#typeofreward#unittype#unitweight#[faction]/[rare] items in brackets are
 *     optional. purchasing a rare unit will cost more rewardpoints
 */
public class UseRewardPointsCommand implements Command {
        ...
        switch (rewardSelection) {
            case 0: // buying techs.
                ...
                if (!(Boolean.parseBoolean(house.getConfig("AllowTechsForRewards")))) {
                    CampaignMain.cm.toUser(
                            "AM:Sorry but you are not allowed to buy techs with "
                                    + CampaignMain.cm.getConfig("RPLongName")
                                    + ".",
                            Username,
                            true);
                ...
                if (CampaignMain.cm.isUsingAdvanceRepair()) {
                    ...
                    CampaignMain.cm.toUser(
                            "AM:You hired "
                                    + StringUtils.aOrAn(
                                            UnitUtils.techDescription(typeOfTechToBuy), true)
                                    + " tech for "
                                    + techCost
                                    + "RP"
                                    + rewards
                    ...
                    int numOfTechBought =
                            (Integer.parseInt(house.getConfig("TechsForARewardPoint")));
            ...
            case 1: // buying influence
                ...
                if (!(Boolean.parseBoolean(house.getConfig("AllowInfluenceForRewards")))) {
                    CampaignMain.cm.toUser(
                            "Sorry but you are not allowed to buy influence with "
                                    + CampaignMain.cm.getConfig("RPLongName")
                                    + ".",
                            Username,
                            true);
                ...
                int amountOfInfluenceBought =
                        (Integer.parseInt(house.getConfig("InfluenceForARewardPoint")));
                amountOfInfluenceBought *= rewardPoints;
                CampaignMain.cm.toUser(
                        "AM:You've bought "
                                + CampaignMain.cm.moneyOrFluMessage(
                                        false, true, amountOfInfluenceBought)
                                + " for "
                                + rewardPoints
                                + " "
                                + CampaignMain.cm.getConfig("RPLongName")
            ...
            case 2: // buying units
                if (!(Boolean.parseBoolean(house.getConfig("AllowUnitsForRewards")))) {
                    CampaignMain.cm.toUser(
                            "AM:Sorry but you are not allowed to buy units with "
                                    + CampaignMain.cm.getConfig("RPLongName")
                                    + ".",
                            Username,
                            true);
                ...
                if (command.hasMoreElements()) {
                    ...
                    if (factionstring.equalsIgnoreCase("rare")) {
                        ...
                        faction =
                                CampaignMain.cm.getHouseFromPartialString(factionstring, Username);
                ...
                if (!player.getHouseFightingFor().equals(faction)) {
                    double nonHouseUnitMod =
                            Double.parseDouble(
                                    house.getConfig(
                                            player.getHouseFightingFor().getName()
                                                    + "To"
                                                    + faction.getName()
                    ...
                    if (nonHouseUnitMod < 0)
                        nonHouseUnitMod =
                                Double.parseDouble(
                                        house.getConfig("RewardPointNonHouseMultiplier"));
                ...
                if (unitTotalRewardPointCost > rewardPointsAvailable) {
                    CampaignMain.cm.toUser(
                            "AM:Sorry. You need more "
                                    + CampaignMain.cm.getConfig("RPLongName")
                                    + " to buy that kind of unit.",
                            Username,
                            true);
                    return;
                }
            ...
            case 3: // repairs
                ...
                if (unit == null) {
                    CampaignMain.cm.toUser(
                            "AM:You don't have a unit with ID# " + unitID + ".", Username, true);
                    return;
                }
                ...
                if (entity.getInternal(Mech.LOC_CT) < 1) {
                    CampaignMain.cm.toUser(
                            "AM:Sorry but cored units cannot be repaired with "
                                    + CampaignMain.cm.getConfig("RPLongName")
                                    + "!",
                            Username);
                    return;
            ...
            case 4: // buying CBills
                ...
                if (!(Boolean.parseBoolean(house.getConfig("AllowCBillsForRewards")))) {
                    CampaignMain.cm.toUser(
                            "Sorry but you are not allowed to buy CBills with "
                                    + CampaignMain.cm.getConfig("RPLongName")
                                    + ".",
                            Username,
                            true);
                ...
                int amountOfCBillsBought =
                        (Integer.parseInt(house.getConfig("CBillsForARewardPoint")));
                amountOfCBillsBought *= rewardPoints;
                CampaignMain.cm.toUser(
                        "AM:You've bought "
                                + CampaignMain.cm.moneyOrFluMessage(
                                        true, false, amountOfCBillsBought)
                                + " for "
                                + rewardPoints
                                + " "
                                + CampaignMain.cm.getConfig("RPLongName")
     ...
     */
    private Vector<SUnit> getUnitProduced(
            int type_id, int weightClass, SPilot pilot, String faction, SHouse house) {
        ...
        if (Boolean.parseBoolean(house.getConfig("UseOnlyOneVehicleSize"))
                && type_id == Unit.VEHICLE)
            unitSize = Unit.getWeightClassDesc(CampaignMain.cm.getRandomNumber(4));

        Filename =
                BuildTable.getUnitFilename(
                        faction, unitSize, type_id, BuildTable.REWARD); // build from rewards dir.
