    ...
    public void process(StringTokenizer command, String Username) {
        ...
        for (int i = 0; i < CampaignMain.cm.getUnresolvedContracts().size(); i++) {
            ...
            if (info.getOfferingPlayerName().equalsIgnoreCase(offeringPlayerName)) {
                ...
                if (CampaignMain.cm.getPlayer(receivingPlayerName)
                        == CampaignMain.cm.getPlayer(Username)) { // player can kill contract offer
                    CampaignMain.cm.getUnresolvedContracts().remove(i);
                    contractCancelled = true;
                    CampaignMain.cm.toUser(
                            "AM:You refused the contract offered by " + offeringPlayerName,
                            Username,
                            true);
                    CampaignMain.cm.toUser(
                            Username + " refused your contract offer", offeringPlayerName, true);
        ...
        if (contractCancelled == false) {
            if (offeringPlayerFound == false) { // not found
                CampaignMain.cm.toUser(
                        "AM:This player has no outstanding contracts", Username, true);
            } else { // contract is for someone else
                CampaignMain.cm.toUser(
                        "AM:This player has not offered you a contract.", Username, true);
            }
