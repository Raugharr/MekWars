...
public class CreateArmyCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
            CampaignMain.cm.toUser(
                    "AM:You may not create new armies while on active duty.", Username, true);
            return;
        }

        if (p.getArmies().size() >= maxlances) {
            CampaignMain.cm.toUser(
                    "AM:You have reached the max number of allowable armies (" + maxlances + ").",
                    Username,
                    true);
            return;
        }
        ...
        if (p.getArmies().size() < newArmy.getID()) p.getArmies().add(newArmy);
        else p.getArmies().add(newArmy.getID(), newArmy);

        // send relevant data to client
        CampaignMain.cm.toUser("PL|SAD|" + p.getArmy(i).toString(true, "%"), Username, false);
        CampaignMain.cm.toUser(
                "AM:Created a new Army (#" + p.getArmy(i).getID() + ").", Username, true);
    } // end process()
