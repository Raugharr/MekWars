    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) {
            ...
            try {
                ...
                CampaignMain.cm.toUser(
                        "AM:Improper format. Try: /c retirepilot#UnitID", Username, true);
                return;
            }
            ...
            if (Boolean.parseBoolean(CampaignMain.cm.getConfig("PilotRetirementAllowed"))
                    == false) {
            ...
            if (m.hasVacantPilot()) {
                CampaignMain.cm.toUser(
                        "AM:This unit does not currently have pilot in it.", Username);
                return;
            }
            ...
            if (totalSkill <= skillForFree) {
                ...
                if (!commandConfirmed) { // send confirm link
                    String toReturn =
                            "AM:<br>Are you sure you want to retire "
                                    + m.getPilot().getName()
                ...
                int costPerLevelNeeded =
                        CampaignMain.cm.getIntegerConfig("CostPerLevelToRetireEarly");
                retirementCost = costPerLevelNeeded * numLevelsNeeded;

                // check to see if early is allowed. if not, break out.
                boolean earlyAllowed =
                        Boolean.parseBoolean(CampaignMain.cm.getConfig("EarlyRetirementAllowed"));
                if (!earlyAllowed) {
                    String toReturn =
                            m.getPilot().getName()
                                    + " may not retire yet. He needs to "
                                    + "level up "
                                    + numLevelsNeeded
                                    + " more time";
                ...
                if (p.getMoney() < retirementCost) {
                    CampaignMain.cm.toUser(
                            "AM:You don't have enough money to dismiss "
                                    + m.getPilot().getName()
                                    + ". Bribing HQ to reassign him would cost "
                                    + CampaignMain.cm.moneyOrFluMessage(true, true, retirementCost)
                                    + ".",
                            Username,
                            true);
                    return;
            ...
            if (!CampaignMain.cm.getBooleanConfig("AllowPersonalPilotQueues")
                    && (CampaignMain.cm.getIntegerConfig("RetiredPilotTakesMechChance") > 0)) {
                if (CampaignMain.cm.getRandomNumber(100)
                        <= CampaignMain.cm.getIntegerConfig("RetiredPilotTakesMechChance")) {
                    retireMechToo = true;
                }
            }
            ...
            if (retirementCost <= 0) {
                toReturn =
                        m.getPilot().getName()
                                ...
                                + (retireMechToo
                                        ? (" Unfortunately, he took his family's "
                                                + unitName
                                                + " with him.")
                                        : "");
            ...
            boolean allowPPQs =
                    Boolean.parseBoolean(CampaignMain.cm.getConfig("AllowPersonalPilotQueues"));
            ...
            while (f.hasMoreElements()) {
                ...
                if (currArmy.getUnit(m.getId()) != null) {
                    ...
                    CampaignMain.cm.toUser(
                            "PL|SAD|" + currArmy.toString(true, "%"), Username, false);
                    CampaignMain.cm
                            .getOpsManager()
