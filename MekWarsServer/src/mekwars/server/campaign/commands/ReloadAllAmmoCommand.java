...
public class ReloadAllAmmoCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (!CampaignMain.cm.getBooleanConfig("UsePartsRepair")) {
            ...
            if (cost > p.getMoney()) {
                CampaignMain.cm.toUser(
                        "AM:You do not have enough to fully reload Unit #"
                                + unit.getId()
                                + ". It would cost "
            ...
            for (Mounted ammo : en.getAmmo()) {
                ...
                // Single shot weapons should only cost 1 short i.e. total shots = 10 then price is
                // 1/10th
                // minium 1.
                if (ammo.getLocation() == Entity.LOC_NONE) {
                    refillShots = 1;
        ...
        // unit.toString() sent's BV to zero and recalculates, so we don't need to do it in this
        // Command
        // class.
        CampaignMain.cm.toUser(
