...
public class CheckArmyLinkCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Improper format. Try: /c checkarmylink#name#armyid", Username, true);
            return;
        }

        if (p.getDutyStatus() != SPlayer.STATUS_FIGHTING
                && MWServ.getInstance().getUserLevel(Username) < IAuthenticator.ADMIN) {
            CampaignMain.cm.toUser(
                    "AM:You may only check links in fighting players' amries.", Username, true);
            return;
        }

        StringBuilder toSend =
                new StringBuilder("Link info for " + p.getName() + "'s Army #" + armyid + ":");
