...
public class SellCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p.getExperience() < minBMEXP) {
            CampaignMain.cm.toUser(
                    "AM:You are not allowed to sell units on the Market. Required Experience: "
                            + minBMEXP
                            + ".",
                    Username,
                    true);
        ...
        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.cm.toUser(
                    "AM:You may not auction any of your units while you are on welfare.",
                    Username,
                    true);
            return;
        }
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Improper format. Try: /c sell#unitid#ticks#minbid", Username, true);
            return;
        }
        SUnit unitToSell = p.getUnit(unitID);
        if (unitToSell == null) {
            CampaignMain.cm.toUser(
                    "AM:You do not have a unit with ID#" + unitToSell + ".", Username, true);
            return;
        }
        ...
        if (unitToSell.getStatus() == Unit.STATUS_UNMAINTAINED) {
            CampaignMain.cm.toUser(
                    "AM:You may not sell unmaintained units on the Market.", Username, true);
            return;
        }
        ...
        if (unitToSell.getStatus() == Unit.STATUS_FORSALE) {
            CampaignMain.cm.toUser(
                    "AM:The " + unitToSell.getModelName() + " is already for sale.",
                    Username,
                    true);
            return;
        }
        ...
        if (!SUnit.mayBeSoldOnMarket(unitToSell)) {
            CampaignMain.cm.toUser(
                    "AM:The " + unitToSell.getModelName() + " may not be sold on the Market.",
                    Username,
                    true);
            return;
        }

        if (unitToSell.isChristmasUnit()
                && !CampaignMain.cm.getBooleanConfig("Christmas_AllowBM")) {
        ...
        sellFluCost =
                sellFluCost
                        + (unitToSell.getWeightclass()) * house.getIntegerConfig("BMFluSizeCost");
        if (p.getInfluence() < sellFluCost) {
            CampaignMain.cm.toUser(
                    "AM:You need "
                            + CampaignMain.cm.moneyOrFluMessage(false, true, sellFluCost)
                            + " to sell the "
        ...
        if (salesTicks < minticks) {
            CampaignMain.cm.toUser(
                    "AM:Units must be offered for at least " + minticks + " ticks.",
                    Username,
                    true);
            return;
        }
        if (minBid < minprice) {
            CampaignMain.cm.toUser(
                    "AM:Units must have a minimum asking price of at least "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, minprice)
                            + ".",
                    Username,
                    true);
            return;
        ...
        if (salesTicks > maxticks && maxticks > 0) {
            CampaignMain.cm.toUser(
                    "AM:Units may not be offered for more than " + maxticks + " ticks.",
                    Username,
                    true);
            return;
        }
        if (minBid > maxprice && maxprice > 0) {
            CampaignMain.cm.toUser(
                    "AM:Units may not have an asking price of more than  "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, maxprice)
                            + ".",
                    Username,
                    true);
            return;
        ...
        if (!Boolean.parseBoolean(house.getConfig("AllowSellingOfDamagedUnits"))
            ...
            CampaignMain.cm.toUser(
                    "AM:You may not sell damaged units on the black market!", Username, true);
            return;
        }
        ...
        CampaignMain.cm.toUser(
                "AM:The "
                        ...
                        + "("
                        + CampaignMain.cm.moneyOrFluMessage(false, false, -sellFluCost, true)
                        + ").",
                Username,
                true);
