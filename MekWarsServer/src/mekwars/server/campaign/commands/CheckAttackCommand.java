...
public class CheckAttackCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        // All the tosses are passed, so check to see if the return should use normal or Operations
        // BVs.
        boolean usingOpRules = CampaignMain.cm.getBooleanConfig("UseOperationsRule");

        String Desc = "<br>"; // output

        // if command has more elements, spit out checkattack for specific armies. no real reason
        // for
        ...
        if (command.hasMoreElements()) {
            ...
            while (targets.hasMoreElements()) {
                ...
                String coloredHouseName =
                        currTargetP.getMyHouse().getHouseFightingFor(currTargetP).getColoredName();
                String defendableOps =
                        listDefendableOperations(
                                arm, currTargetP, currTarget, p.getHouseFightingFor());
                ...
                if (CampaignMain.cm.getBooleanConfig("ShowInfInCheckAttack"))
                    ...
                    Desc +=
                            coloredHouseName
                                    + "("
                                    + currTarget.getAmountOfUnitsWithoutInfantry()
                                    + ")";
            ...
            while (e.hasMoreElements()) {
                ...
                if (arm != null && !arm.isDisabled()) {
                    ...
                    while (targets.hasMoreElements()) {
                        ...
                        if (usingOpRules
                                && arm.getAmountOfUnits() > currTarget.getAmountOfUnits()) {
                            Desc += "(BV Against: " + arm.getOperationsBV(currTarget) + ")";
                        }
