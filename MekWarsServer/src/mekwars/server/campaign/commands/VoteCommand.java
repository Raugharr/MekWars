...
public class VoteCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (type < Vote.ABSTAIN_VOTE || type > Vote.NEGATIVE_VOTE) {
            CampaignMain.cm.toUser(
                    "AM:You tried to use an illegal vote type. Types are:"
                            + "<br> Positive: "
                            + Vote.POSITIVE_VOTE
                            + "<br> Negative: "
                            + Vote.NEGATIVE_VOTE
        ...
        if (votesCast > castingPlayer.getNumberOfVotesAllowed()) {
            CampaignMain.cm.toUser(
                    "AM:You have no votes left to cast. Retract one of your current"
                            + "votes and try again.",
                    Username,
                    true);
            return;
        ...
        if (recipientPlayer == null) {
            CampaignMain.cm.toUser(
                    "AM:The player you tried to vote for doesn't exist.", Username, true);
            return;
        }
        ...
        if (!castingPlayer.getMyHouse().equals(recipientPlayer.getMyHouse())) {
            CampaignMain.cm.toUser(
                    "AM:You may only vote for players in your own faction.", Username, true);
            return;
        }
        ...
        if (voteManager.checkForDuplicate(toCast)) {
            CampaignMain.cm.toUser(
                    "AM:You have already cast a vote for this player.", Username, true);
            return;
        }
