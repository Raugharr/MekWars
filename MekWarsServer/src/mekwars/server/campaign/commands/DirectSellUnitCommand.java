...
public class DirectSellUnitCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.cm.toUser(
                    "AM:You may not sell any of you units while you are on welfare.",
                    Username,
                    true);
            return;
        }
        ...
        if (pSeller == null) {
            CampaignMain.cm.toUser(
                    "AM:Selling player (" + seller + ") could not be found.", Username, true);
            return;
        }

        if (pBuyer == null) {
            CampaignMain.cm.toUser(
                    "AM:Buying player (" + buyer + ") could not be found.", Username, true);
            return;
        }
        ...
        if (m == null) {
            CampaignMain.cm.toUser(
                    "AM:Could not find a unit with ID#" + unitid + ".", Username, true);
            return;
        }

        if (m.getStatus() == Unit.STATUS_FORSALE) {
            CampaignMain.cm.toUser(
                    "AM:You may not directly sell units already on the open market.",
                    Username,
                    true);
            return;
        }
        ...
        // Target has no room?
        if (pBuyer.getFreeBays() < SUnit.getHangarSpaceRequired(m, pBuyer.getMyHouse())
                && !usesTechs) {
            ...
            if (MWServ.getInstance()
                    .getIP(pSeller.getName())
                    .toString()
                    .equals(MWServ.getInstance().getIP(pBuyer.getName()).toString())) {
                CampaignMain.cm.toUser(
                        pBuyer.getName() + " has the same IP as you do. You can't send him units.",
                        Username,
                        true);
                return;
            }
            ...
            return;
                ...
                && pSeller.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
            CampaignMain.cm.toUser(
                    "AM:You may not sell units which are in active armies.", Username, true);
            return;
        ...
        for (SArmy currA : pSeller.getArmies()) {
            if (currA.isLocked() && currA.getUnit(m.getId()) != null) {
                CampaignMain.cm.toUser(
                        "AM:You may not sell units which are in fighting armies.",
                        pSeller.getName(),
                        true);
                return;
            }
        ...
        if (!confirmedSend) {
            if (Username.equals(buyer)) {
                CampaignMain.cm.toUser(
                        "AM:Illegal transaction! The staff was alerted!", buyer, true);
            ...
            CampaignMain.cm.toUser(
                    seller
                            ...
                            + " for "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, costToRecipient)
        ...
        if (costToRecipient - sellPrice > 0) {
            result +=
                    ".  However your contacts took "
                            + CampaignMain.cm.moneyOrFluMessage(
                                    true, true, (costToRecipient - sellPrice))
                            + " for themselves.";
        }
