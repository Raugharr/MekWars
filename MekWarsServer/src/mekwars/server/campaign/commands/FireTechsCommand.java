...
public class FireTechsCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        // dont want a unit being marked unmaintained while its in an active army, so only let
        // reserve
        // players fire techs
        ...
        if (numtofire == 1) CampaignMain.cm.toUser("AM:You fire 1 technician.", Username, true);
        else CampaignMain.cm.toUser("AM:You fired " + numtofire + " technicians.", Username, true);

        if (p.getFreeBays() < 0) {
            int numUnmaintained = p.setRandomUnmaintained();
            String toSend =
                    "There are no longer enough technicians to maintain all of your equipment. ";
            if (numUnmaintained == 1)
                toSend += " 1 unit was pulled from the active rotation (A unit is";
            else
                toSend +=
                        numUnmaintained
                                + " units were pulled from the active rotation ("
                                + numUnmaintained
                                + " units are ";
    ...
    private void fireAdvanceTechs(StringTokenizer command, String Username) {
        ...
        if (numberOfTechs == 1)
            CampaignMain.cm.toUser(
                    "AM:You have fired a " + UnitUtils.techDescription(techType) + " tech.",
                    Username,
                    true);
        else
            CampaignMain.cm.toUser(
