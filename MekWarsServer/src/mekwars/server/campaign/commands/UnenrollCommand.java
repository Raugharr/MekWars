...
public class UnenrollCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (hisfaction == null) {
            CampaignMain.cm.toUser(
                    "AM:Couldn't find faction to unenroll. Contact an admin immediately.",
                    Username,
                    true);
            return;
        }
        ...
        CampaignMain.cm.doSendModMail(
                "NOTE", Username + " unenrolled from the campaign (IP: " + ip + ").");
    } // end process

    private void removeFaction(SHouse faction) {

        if (!CampaignMain.cm.getBooleanConfig("AllowSinglePlayerFactions")) return;

        Enumeration<SPlanet> planets = faction.getPlanets().elements();
        while (planets.hasMoreElements()) {
            SPlanet planet = planets.nextElement();
            planet.doGainInfluence(
                    CampaignMain.cm.getHouseById(-1), faction, Integer.MAX_VALUE, true);
