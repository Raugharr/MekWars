...
public class RequestSubFactionPromotionCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Invalid Syntax: /RequestSubFactionPromotion subFactionname", Username);
            return;
        }

        if (subFaction == null) {
            CampaignMain.cm.toUser(
                    "AM:That SubFaction does not exist for faction " + faction.getName() + ".",
                    Username);
            return;
        }
        ...
        if (player.getExperience() < minEXP || player.getRating() < minELO) {
            CampaignMain.cm.toUser(
                    "AM:Sorry but you are not skilled enough to join that SubFaction.", Username);
            return;
        }
        ...
        if (!player.canBePromoted()) {
            CampaignMain.cm.toUser(
                    "AM:Sorry but you've already been premoted once within a "
        ...
        if (CampaignMain.cm.getBooleanConfig("autoPromoteSubFaction")) {
            ...
            CampaignMain.cm.toUser(
                    player.getMyHouse().getCompleteStatus(), player.getName(), false);

            CampaignMain.cm.toUser(
                    "AM:Congratulations you have been promoted to SubFaction "
                            + subFactionName
                            + ".",
                    Username);
