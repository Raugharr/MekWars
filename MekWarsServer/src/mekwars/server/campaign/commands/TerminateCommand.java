...
/**
 * Terminate command is analagous to the old cancel command used for Tasks. Should be mirrored in
 * the CampaignMain command tree.
 */
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreTokens()) {
            try {
                ...
                CampaignMain.cm.toUser(
                        "AM:Improper format. Try: /c terminate#attack number", Username, true);
                return;
            }
        } else {
        ...
        if (so == null) {
            CampaignMain.cm.toUser(
                    "AM:Terminate failed. Attack #" + opID + " does not exist.", Username, true);
            return;
        }
        ...
        if (so.getStatus() == ShortOperation.STATUS_FINISHED
                || so.getStatus() == ShortOperation.STATUS_REPORTING) {
            CampaignMain.cm.toUser(
                    "AM:Terminate failed. You may not terminate a completed game.", Username, true);
            return;
        }
        ...
        // Check if opponents are offline.  Otherwise, this game cannot be cancelled by a user who
        // has
        // been disconnected on
        for (String currPlayerName : so.getAllPlayerNames()) {
            if (CampaignMain.cm.getPlayer(currPlayerName).getDutyStatus()
        ...
        if (so.getCancelledPlayers().size() >= so.getAllPlayerNames().size()) {
            ...
            CampaignMain.cm
                    .getOpsManager()
                    .terminateOperation(so, OperationManager.TERM_TERMCOMMAND, tp);
        } else {
            CampaignMain.cm.toUser(
