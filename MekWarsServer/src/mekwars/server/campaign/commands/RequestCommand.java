...
public class RequestCommand implements Command {
    ...
    public void process(StringTokenizer command, String username) {
        ...
        if (p.getMyHouse().isNewbieHouse()) {
            ...
            result =
                    CampaignMain.cm.getConfig("NewbieHouseName")
                            + " players may not purchase new units; however, they may reset their"
                            + " units.";
            result +=
                    "<br><a href=\"MEKWARS/c request#resetunits\">Click here to request a reset of"
                        + " your units.</a>";
        ...
        if (p.hasUnmaintainedUnit()) {
            CampaignMain.cm.toUser(
                    "AM:Your faction refuses to assign new units to you force while units in your"
                        + " hangar are unmaintained!",
                    username,
                    true);
            return;
        }
        ...
        if (p.mayAcquireWelfareUnits()) {
            ...
            p.addUnit(unit, true);
            CampaignMain.cm.toUser(
                    "AM:High Command has given you a new unit from its welfare rolls to help you"
        ...
        if (command.hasMoreElements()) {
            ...
            if (planet == null) {
                CampaignMain.cm.toUser(
                        "AM:Could not find planet: " + planetName + ".", username, true);
                return;
            }
            ...
            if (planet.getFactoriesOfWeightClass(weightclass).size() == 0) {
                CampaignMain.cm.toUser(
                        planetName + " does not produce units of the weight class specified.",
                        username,
                        true);
                return;
            }
            ...
            List<SUnitFactory> namedFactories = planet.getFactoriesByName(factoryName);
            if (namedFactories.size() == 0) {
                CampaignMain.cm.toUser(
                        "AM:There is no " + factoryName + " on " + planetName + ".",
                        username,
                        true);
                return;
            }
            ...
            if (CampaignMain.cm.getBooleanConfig("Enforce_Subfaction_Factory_Access")
                ...
                CampaignMain.cm.toUser(
                        "You do not have the correct rank to purchase a unit from "
                                + factoryName
                                + " on "
                                + planetName
                                + ".",
         ...
         */
            ...
            if (CampaignMain.cm.getBooleanConfig("Enforce_Subfaction_Factory_Access")) {
                factory =
                        p.getMyHouse()
                                .getNativeAccessableFactoryForProduction(
                                        type_id, weightclass, p.getSubFactionAccess(), username);
                if (factory != null) {
        ...
        if (planet == null || factory == null) {
            CampaignMain.cm.toUser(
                    "AM:No "
                            ...
                            + " factory is available to fill your order at this time (Click on icon"
                            + " in House Status to use captured factories).",
                    username,
                    true);
            return;
        }
        ...
        if (!factory.getFounder().equalsIgnoreCase(playerHouse.getName())) {
            mechCbills =
                    Math.round(
                            mechCbills
                                    * CampaignMain.cm.getFloatConfig("NonOriginalCBillMultiplier"));
            mechInfluence =
                    Math.round(
                            mechInfluence
                                    * CampaignMain.cm.getFloatConfig(
        ...
        if (hasEnoughMoney && hasEnoughInfluence && factionHasEnoughPP) {
            ...
            if (needsMoreTechs) {
                ...
                if (totalCost > p.getMoney()) {
                    ...
                    toSend.append(
                            "AM:Command will not release a new unit to you unless support is in"
                                + " place; however, you cannot afford to buy the unit *and* ");
                    if (useBays) {
                ...
                toSend.append(
                        "AM:Quartermaster command will not send a new unit to your force until"
                            + " support resources are in place. You will need to ");
                if (useBays) {
                ...
                toSend.append(
                        "<br><a href=\"MEKWARS/c hireandrequestnew#"
                                ...
                                + "#"
                                + factory.getName()
                                + "\">Click here to purchase both the unit and the needed"
                                + " support.</a>");
            ...
            for (SUnit mech : mechs) {
                if (playerHouse.getBooleanConfig("UseCalculatedCosts")) {
                    ...
                    if (unitCost < 1) {
                        unitCost =
                                playerHouse.getPriceForUnit(mech.getWeightclass(), mech.getType());
                    }
            ...
            CampaignMain.cm.doSendToAllOnlinePlayers(
                    playerHouse, "HS|" + hsUpdates.toString(), false);
            ...
            // more PP
            result =
                    "AM:Your faction does not have the components needed to produce such a unit at"
                        + " this time. Wait for your faction to gather more resources.";
            CampaignMain.cm.toUser(result, username, true);
            return; // break out ...
    ...
    /**
     * Private method which builds a welfare unit. Duplicated in RequestCommand. Kept private in
     * these classes in order to ensure that ONLY requests generate welfare units (had previously
     * been a public call in CampaignMain).
     */
