...
public class EmojiCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (!command.hasMoreTokens()) {
            CampaignMain.cm.toUser(
                    "AM:You forgot to specify an emoji to display!. ", username, true);
            return;
    ...
    private boolean checkAccess() {
        ...
        if (!allowEmoji) {
            CampaignMain.cm.toUser(
                    "AM:Emojis have been disabled, the SO does NOT like fun.", username, true);
            return false;
        }
    ...
    private void processEmoji() {
        ...
        //			CampaignMain.cm.doSendToAllOnlinePlayers(coloredName + ": ʕ •ᴥ•ʔ" ,true);
            ...
            CampaignMain.cm.doSendToAllOnlinePlayers(
                    coloredName + ": (•_•) ( •_•)>⌐■-■ (⌐■_■)", true);
        else if (emoji.equalsIgnoreCase("li") || emoji.equalsIgnoreCase("list")) showList();
        else
            CampaignMain.cm.toUser(
                    "AM:That emoji does not exist! Check the list by typing /ec#list ",
                    username,
                    true);
    }
    ...
    private void showList() {
        ...
        String list =
                "Emoji List"
                        + br
                        + "/ec#fl or /ec#flip outputs: (╯°□°)╯︵ ┻━┻ "
                        + br
                        + "/ec#sh or /ec#shrug outputs: ¯\\_(ツ)_/¯"
                        + br
                        + "/ec#fi or /ec#fingers outputs: t(-.-t)"
                        + br
                        + "/ec#ki or /ec#kiss outputs: ( ˘ ³˘)♥"
                        + br
                        + "/ec#sm or /ec#smile outputs: ◉‿◉"
                        + br
                        + "/ec#de or /ec#deal outputs: (•_•) ( •_•)>⌐■-■ (⌐■_■)"
                        + br;
