...
public class BuyBaysCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        int maxBays =
                Integer.parseInt(
                        house.getConfig("MaxBaysToBuy")); // Max number of bays that can be bought
        try {
        ...
        if (maxBays != -1 && p.getBaysOwned() + numtobuy > maxBays) {
            CampaignMain.cm.toUser(
                    "AM:Sorry but the max number of bays you can lease is " + maxBays + ".",
                    Username,
                    true);
            return;
        }
        ...
        if (bayCost > p.getMoney()) {
            CampaignMain.cm.toUser(
                    "AM:Leasing "
                            + numtobuy
                            + " bays will cost you "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, bayCost)
        ...
        if (numtobuy == 1)
            CampaignMain.cm.toUser(
                    "AM:You've leased a bay! After paying the security deposit of "
                            + CampaignMain.cm.moneyOrFluMessage(true, false, bayCost),
                    Username,
                    true);
        else
