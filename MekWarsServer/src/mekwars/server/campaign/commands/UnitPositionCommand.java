    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (CampaignMain.cm.getOpsManager().getShortOpForPlayer(p) != null) {
            CampaignMain.cm.toUser(
                    "AM:You may not change unit positions while you are in a game!",
                    Username,
                    true);
            return;
        }
        ...
        if (command.hasMoreElements()) {
            ...
            try {
                ...
                CampaignMain.cm.toUser(
                        "AM:Improper format. Try: /c unitposition#army#unit#newposition",
                        Username,
                        true);
                return;
            }
            ...
            if (u.getStatus() == Unit.STATUS_UNMAINTAINED) {
                CampaignMain.cm.toUser(
                        "AM:You may not change the position of an unmaintained unit.",
                        Username,
                        true);
                return;
            }
            ...
            if (newposition < 0) {
                CampaignMain.cm.toUser(
                        "AM:You may not place a unit in a negative order/position.",
                        Username,
                        true);
                return;
            }
            ...
            if (newposition > a.getAmountOfUnits()) {
                CampaignMain.cm.toUser(
                        "AM:Highest position available in Army #"
                                + armyid
                                + " is Pos. #"
                                + a.getAmountOfUnits()
                                + ".",
            ...
            // new position
            a.removeUnit(unitid);
            if (newposition >= a.getAmountOfUnits()) a.addUnit(u); // put at end
            else a.addUnit(u, newposition);

            // now, send an update command to the client
            CampaignMain.cm.toUser(
                    "PL|RPU|" + a.getID() + "#" + u.getId() + "#" + newposition, Username, false);
