...
public class DonateCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p.getMyHouse().isNewbieHouse()) {
            CampaignMain.cm.toUser(
                    "AM:SOL Players are not allowed to donate units, sorry!", Username, true);
            return;
        }

        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.cm.toUser(
                    "AM:You may not donate any of your units while you are on welfare.",
                    Username,
                    true);
            return;
        }
        ...
        if (m == null) {
            CampaignMain.cm.toUser(
                    "AM:You do not have a unit with ID#" + unitid + ".", Username, true);
            return;
        }

        if (m.isChristmasUnit() && !CampaignMain.cm.getBooleanConfig("Christmas_AllowDonate")) {
            CampaignMain.cm.toUser(
                    "AM:Sorry, you cannot donate Christmas units.  You should play with your new toys.",
                    Username);
            return;
        }
        ...
        if (m.getModelName().startsWith("Error") || m.getModelName().startsWith("OMG")) {
            CampaignMain.cm.toUser(
                    "AM:You tried to donate an Error unit. The unit was auto-scrapped and the staff was alerted.",
                    Username,
                    true);
            CampaignMain.cm.doSendModMail(
                    "NOTE",
                    Username
                            + " tried to donate an OMG. Unit auto-scrapped. Data: "
                            + m.getProducer());
            MWLogger.errLog(
                    Username
                            + " tried to donate an OMG. Unit auto-scrapped. Data: "
        ...
        if (m.getStatus() == Unit.STATUS_FORSALE) {
            CampaignMain.cm.toUser(
                    "AM:Units that are for sale on the Market may not be donated.", Username, true);
            return;
        }

        if (p.getAmountOfTimesUnitExistsInArmies(unitid) > 0
                && p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
            CampaignMain.cm.toUser(
                    "AM:You may not donate units which are in active armies.", Username, true);
            return;
        }
        ...
        for (SArmy currA : p.getArmies()) {
            if (currA.isLocked() && currA.getUnit(unitid) != null) {
                CampaignMain.cm.toUser(
                        "AM:You may not donate units which are in fighting armies.",
                        Username,
                        true);
                return;
            }
        ...
        if (p.getDonationsThisTick() >= donationsAllowed) {
            CampaignMain.cm.toUser(
                    "AM:You may only donate " + donationsAllowed + " unit(s) each tick.",
                    Username,
                    true);
            return;
        }

        if (!Boolean.parseBoolean(house.getConfig("AllowDonatingOfDamagedUnits"))
                && (UnitUtils.hasArmorDamage(m.getEntity())
                        || UnitUtils.hasCriticalDamage(m.getEntity()))) {
        ...
        int moneyToDonate =
                Math.round(
                        p.getMyHouse().getPriceForUnit(m.getWeightclass(), m.getType())
                                * costMulti);
        int infToDonate =
                (int)
                        (p.getMyHouse().getInfluenceForUnit(m.getWeightclass(), m.getType())
                                * costMulti);

        // Allow negative monetary costs (give money back), but don't allow donations to grant flu.
        if (infToDonate < 0) infToDonate = 0;

        // Check to ensure player can afford the scrap
        if (p.getMoney() < moneyToDonate || p.getInfluence() < infToDonate) {
            CampaignMain.cm.toUser(
                    "AM:You cannot afford to donate this unit. You need "
                            + CampaignMain.cm.moneyOrFluMessage(true, true, moneyToDonate)
                            + " and "
                            + CampaignMain.cm.moneyOrFluMessage(false, true, infToDonate)
                            + ".",
                    Username,
                    true);
            return;
        ...
        if (m.getScrappableFor() >= 0) {
            ...
            CampaignMain.cm.toUser(
                    "AM:You donated the "
                            ...
                            + CampaignMain.cm.moneyOrFluMessage(
                                    true, true, m.getScrappableFor() / 2, true)
                            + ", "
                            + CampaignMain.cm.moneyOrFluMessage(false, true, infToDonate / 2, true)
                            + ").",
                    Username,
                    true);
        } else {
            ...
            CampaignMain.cm.toUser(
                    "AM:You donated the "
                            + m.getModelName()
                            + " ("
                            + CampaignMain.cm.moneyOrFluMessage(true, true, -moneyToDonate, true)
                            + ", "
                            + CampaignMain.cm.moneyOrFluMessage(false, true, -infToDonate, true)
                            + ").",
                    Username,
                    true);
        }
        ...
        CampaignMain.cm.doSendHouseMail(
                ...
                p.getName()
                        + " donated "
                        + StringUtils.aOrAn(m.getVerboseModelName(), true)
                        + " to the faction.");
        if (p.mayAcquireWelfareUnits())
            CampaignMain.cm.doSendModMail(
                    "NOTE", Username + " donated a unit and sent himself into welfare.");

        // do the actual remove last, so the checkops show under the donation string. replace pilot
        // before adding to house.
        p.removeUnit(unitid, true);
        ...
        if (Boolean.parseBoolean(house.getConfig("AllowPersonalPilotQueues"))
            ...
            CampaignMain.cm.toUser(
                    "PL|AP2PPQ|"
                            ...
                            + m.getWeightclass()
                            + "|"
                            + oldPilot.toFileFormat("#", true),
                    Username,
                    false);
            CampaignMain.cm.toUser(
                    oldPilot.getName() + " was moved to your barracks.", Username, true);
            p.getPersonalPilotQueue()
                    .checkQueueAndWarn(p.getName(), m.getType(), m.getWeightclass());
