    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) {
            ...
            if (changeMech != null && changeMech.getModelName().startsWith("Error")) {
                CampaignMain.cm.toUser(
                        "AM:Error units may not be added to armies.", Username, true);
                return;
            }

            if (changeMech != null && a.isUnitInArmy(changeMech)) {
                CampaignMain.cm.toUser(
                        "AM:That unit already exits in Army #" + armyid + ".", Username, true);
                return;
            }

            if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
                CampaignMain.cm.toUser(
                        "AM:You may not change your armies while on active duty.", Username, true);
                return;
            }

            if (changeMech != null && changeMech.getStatus() == Unit.STATUS_UNMAINTAINED) {
                CampaignMain.cm.toUser(
                        "AM:You may not assign unmaintained units to combat formations!",
                        Username,
                        true);
                return;
            }
            ...
            if (changeMech != null) {
                int maxAmount =
                        Integer.valueOf(CampaignMain.cm.getConfig("UnitsInMultipleArmiesAmount"))
                                .intValue();
                if (p.getAmountOfTimesUnitExistsInArmies(changeMech.getId()) >= maxAmount) {
                    if (maxAmount == 1)
                        CampaignMain.cm.toUser(
                                "AM:A unit may only be in one army at a time.", Username, true);
                    else
                        CampaignMain.cm.toUser(
                                "AM:A unit may be in a maximum of " + maxAmount + " armies.",
                                Username,
                                true);
                    return;
                }
                ...
                if (oldMech != null) {
                    ...
                    a.removeUnit(oldID);
                    CampaignMain.cm.toUser(
                            "PL|RAU|" + a.getID() + "#" + oldID + "#" + a.getBV(), Username, false);
                    CampaignMain.cm.toUser(
                            "PL|UU|" + oldMech.getId() + "|" + oldMech.toString(true),
                            Username,
                            false);
                ...
                if (position > -1) {
                    ...
                    CampaignMain.cm.toUser(
                            "PL|AAU|" + a.getID() + "#" + changeMech.getId() + "#" + a.getBV(),
                            Username,
                            false);
                }
                ...
                a.checkLegalRatio(Username);
                    ...
                    != null) { // changemech is known to be null from previous if statement
                a.removeUnit(oldMech.getId());
                CampaignMain.cm.toUser(
                        "PL|RAU|" + a.getID() + "#" + oldMech.getId() + "#" + a.getBV(),
                        Username,
                        false);
                a.checkLegalRatio(Username);
                CampaignMain.cm.toUser(
                        "PL|UU|" + oldMech.getId() + "|" + oldMech.toString(true), Username, false);
            }

            // tell the player that his army was changed and inform him of any legal ops changes
            CampaignMain.cm.toUser(
                    "AM:Army #" + a.getID() + " was changed. New BV: " + a.getBV(), Username, true);
