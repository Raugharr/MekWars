    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) {
            ...
            boolean limitsAllowed =
                    Boolean.parseBoolean(CampaignMain.cm.getConfig("AllowLimiters"));
            if (!limitsAllowed) {
            ...
            if (command.hasMoreElements()) {
                ...
                if (p != null) {
                    if (p.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
                        CampaignMain.cm.toUser(
                                "AM:You cannot change limits while active.", Username, true);
                        return;
                    }
                    ...
                    if (army != null) {
                        ...
                        if (limit < Army.NO_LIMIT) { // -1 is NO_LIMIT
                            CampaignMain.cm.toUser(
                                    "AM:You may not set negative limits.", Username, true);
                            return;
                        }
                        ...
                        if (limit < bufferAmt && limit != Army.NO_LIMIT) {
                            CampaignMain.cm.toUser(
                                    "AM:You must set an upper limit of " + bufferAmt + "or more.",
                                    Username,
                                    true);
                            return;
                        }
                        ...
                        if (limit == -1)
                            CampaignMain.cm.toUser(
                                    "AM:Army #" + armyid + "'s upper limit disabled.",
                                    Username,
                                    true);
                        else
                            CampaignMain.cm.toUser(
                                    "AM:Army #" + armyid + "'s upper limit set to " + limit + ".",
                                    Username,
                                    true);
