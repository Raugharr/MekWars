...
public class SimpleRepairCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            int cost =
                    CampaignMain.cm.getTotalRepairCosts(
                            unit.getEntity(), techs, rolls, ATLevel, player.getMyHouse());
            int time = 0;
            ...
            if (player.getDutyStatus() == SPlayer.STATUS_ACTIVE
                ...
                CampaignMain.cm.toUser(
                        "FSM|You man not repair that unit while it is in an active army.",
                        Username,
                        false);
                return;
            }
            ...
            MWServ.getInstance()
                    .getRTT()
                    .getRepairList()
                    .add(RepairTrackingThread.Repair(player, unitID, techs, time, false));
            CampaignMain.cm.toUser(
                    "FSM|Repairs have begone on your "
                            + unit.getModelName()
                            + " <b>At a Cost of "
    ...
    private int setWorkHours(
            Vector<Integer> rolls, Vector<Integer> techs, Entity unit, SHouse house) {
        ...
        for (int location = 0; location < unit.locations(); location++) {
            ...
            for (int slot = 0; slot < unit.getNumberOfCriticals(location); slot++) {
                ...
                if (cs.getType() == CriticalSlot.TYPE_SYSTEM
                    ...
                    totalTime +=
                            calculateTime(unit, location, slot, techType, baseRoll, false, house);
    ...
    private int calculateTime(
        ...
        roll =
                UnitUtils.getTechRoll(unit, location, slot, tech, armor, house.getTechLevel())
                        - baseRoll;
        ...
        for (int count = 0; count < roll; count++) repairTime *= 2;
