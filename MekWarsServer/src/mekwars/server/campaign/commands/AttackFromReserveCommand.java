...
public class AttackFromReserveCommand implements Command {
    ...
    public void process(StringTokenizer command, String username) {
        ...
        if (Boolean.parseBoolean(CampaignMain.cm.getConfig("CampaignLock")) == true) {
            CampaignMain.cm.toUser(
                    "AM:The campaign is currently locked. Attacks are disabled until the campaign"
                        + " is unlocked.",
                    username,
                    true);
            return;
        ...
        if (ap.getDutyStatus() == SPlayer.STATUS_ACTIVE) {
            CampaignMain.cm.toUser(
                    "AM:You are currently active. You must deactivate in order to attack from"
                        + " reserve.)",
                    username,
                    true);
            return;
        ...
        if ((CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForAFR") > -1)
            ...
            CampaignMain.cm.toUser(
                    "AM:You cannot attack from reserve with more than "
                            ...
                            + " negative bays.  How about you share the wealth with your"
                            + " housemates.",
                    username,
                    true);
            return;
        ...
        if (CampaignMain.cm.getBooleanConfig("DisableAFRIfOverHangarLimits")
            ...
            CampaignMain.cm.toUser(
                    "AM: You have exceeded one or more hangar limits.  Activation is disabled until"
                        + " you get under those limits.",
                    username,
                    true);
            return;
        }
