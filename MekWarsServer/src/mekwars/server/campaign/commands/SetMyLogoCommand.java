...
public class SetMyLogoCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (!command.hasMoreTokens()) {
            ...
            // this way for some reason doesn't work anymore so I'm moving everything to SPlayer. --
            // Torren
            if (newLogo.trim().length() < 1) player.setMyLogo(player.getMyHouse().getLogo());
            else player.setMyLogo(newLogo);
        }
        ...
        CampaignMain.cm.toUser(
                "AM:It'll look like this: <img height=\"150\" width=\"150\" src =\""
                        + player.getMyLogo()
                        + "\">",
                Username,
                true);
