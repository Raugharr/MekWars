...
public class RemoveVoteCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (castersVotes.isEmpty()) {
            CampaignMain.cm.toUser(
                    "AM:You have not cast any votes. Removal is impossible.", Username, true);
            return;
        }
        ...
        if (recipientPlayer == null) {
            CampaignMain.cm.toUser(
                    "AM:You can't remove a vote for a player who doesn't exist.", Username, true);
            return;
        }
        ...
        if (!voteRemoved) {
            CampaignMain.cm.toUser(
                    "AM:There was an error removing the vote. Please contact your "
                            + "server admin or file a bug report.",
                    Username,
                    true);
            return;
        }
        ...
        // else
        CampaignMain.cm.toUser(
                "AM:Your vote for " + recipientName + " has been removed.", Username, true);
    }
}
