...
public class FactionLeaderFluffCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Improper command. Try: /c factionleaderfluff#PlayerName#text",
                    Username,
                    true);
            return;
        }

        if (command.hasMoreElements()) fluff = command.nextToken();
        else fluff = null;
        ...
        if (!leader.getMyHouse().getName().equalsIgnoreCase(p.getMyHouse().getName())) {
            CampaignMain.cm.toUser(
                    "AM:You are not in the same faction as "
                            + p.getName()
                            + " therefore you maynot change their fluff!",
                    Username,
                    true);
            return;
        }

        if (fluff == null) p.setFluffText("");
        else {
            if (fluff.indexOf("~") > 0
                    || fluff.indexOf("$") > 0
                    || fluff.indexOf("#") > 0
        ...
        CampaignMain.cm.toUser(
                "AM:New fluff text for " + p.getName() + ": " + fluff, Username, true);
        ...
        CampaignMain.cm.doSendModMail(
                "NOTE", Username + " set " + p.getName() + "'s fluff to '" + fluff + "'.");
        CampaignMain.cm.doSendToAllOnlinePlayers(
                "PI|FT|" + p.getName() + "|" + p.getFluffText(), false);
    }
}
