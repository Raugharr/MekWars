...
public class PromotePlayerCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Invalid Syntax: /promoteplayer Player#NewSubFactionName", Username);
            return;
        }
        ...
        if (!grunt.canBePromoted() && !isMod) {
            CampaignMain.cm.toUser(
                    "AM:" + grunt.getName() + " cannot be promoted at this time.", Username);
            return;
        }
        ...
        if (!grunt.getMyHouse().getName().equalsIgnoreCase(leader.getMyHouse().getName())
            ...
            CampaignMain.cm.toUser(
                    "AM:You can only promote players that within your same faction!", Username);
            return;
        }
        ...
        CampaignMain.cm.doSendToAllOnlinePlayers(
                "PI|SSN|" + grunt.getName() + "|" + subFactionName, false);
        ...
        CampaignMain.cm.toUser(
                "AM:Congratulations you have been promoted to SubFaction " + subFactionName + ".",
                grunt.getName());
        CampaignMain.cm.doSendHouseMail(
                grunt.getMyHouse(),
                "NOTE",
                grunt.getName()
                        + " has been promoted to subfaction "
                        + subFactionName
                        + " by "
                        + leader.getName()
                        + "!");
        ...
        CampaignMain.cm.toUser(
                "AM:You've promoted " + grunt.getName() + " to SubFaction " + subFactionName + ".",
                Username);
