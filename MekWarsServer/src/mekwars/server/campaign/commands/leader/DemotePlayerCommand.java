...
public class DemotePlayerCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        try {
            ...
            CampaignMain.cm.toUser(
                    "AM:Invalid Syntax: /demoteplayer Player#NewSubFactionName[none]", Username);
            return;
        }
        ...
        if (!grunt.getMyHouse().getName().equalsIgnoreCase(leader.getMyHouse().getName())
            ...
            CampaignMain.cm.toUser(
                    "AM:You can only demote players that within your same faction!", Username);
            return;
        }

        if (subFactionName.equalsIgnoreCase("none")) subFaction = new SubFaction();
        else subFaction = grunt.getMyHouse().getSubFactionList().get(subFactionName);

        if (subFaction == null) {
            CampaignMain.cm.toUser(
                    "AM:That SubFaction does not exist for faction "
        ...
        if (grunt.getSubFactionAccess() < Integer.parseInt(subFaction.getConfig("AccessLevel"))) {
            CampaignMain.cm.toUser(
                    "AM:You cannot demote "
                            + grunt.getName()
                            + " to a subfaction with a higher access level",
                    Username);
            return;
        }
        ...
        if (grunt.getExperience() < minEXP || grunt.getRating() < minELO) {
            CampaignMain.cm.toUser(
                    "AM:Sorry but "
                            + grunt.getName()
                            + " is not skilled enough to join that SubFaction.",
                    Username);
            return;
        }
        ...
        CampaignMain.cm.doSendToAllOnlinePlayers(
                "PI|FT|" + grunt.getName() + "|" + grunt.getFluffText(), false);
        ...
        CampaignMain.cm.toUser(
                "AM:You have been demoted to SubFaction " + subFactionName + ".", grunt.getName());
        CampaignMain.cm.doSendHouseMail(
                ...
                grunt.getName()
                        ...
                        + "!");
        CampaignMain.cm.toUser(
                "AM:You demoted " + grunt.getName() + " to SubFaction " + subFactionName + ".",
                Username);
