...
public class DeactivateCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (p == null) {
            CampaignMain.cm.toUser(
                    "AM:Null player. Big Failure. Report to admin immediately.", Username, true);
            return;
        }
        ...
        if (currentStatus == SPlayer.STATUS_FIGHTING) {
            CampaignMain.cm.toUser(
                    "AM:You're currently fighting! You cannot go into the reserve!",
                    Username,
                    true);
            return;
        }
        ...
        p.leechCount = 0;
        CampaignMain.cm.toUser(
                "AM:[*] You've left active duty and are now in reserve.", Username, true);
