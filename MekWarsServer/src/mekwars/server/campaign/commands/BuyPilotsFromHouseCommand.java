...
public class BuyPilotsFromHouseCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) {
            ...
            // ok the faction will allow them to buy pilots even with them in the queue but how
            // many?
            if (p.getPersonalPilotQueue().getPilotQueue(unitType, weightClass).size()
                            + numberOfPilots
            ...
            for (int pilotCount = 0; pilotCount < numberOfPilots; pilotCount++) {
                ...
                String toUser = "";
                String skills =
                        pilot.getSkillString(true, h.getPilotQueues().getBasePilotSkill(unitType))
                                .trim();
                if (unitType == Unit.MEK) {
                    toUser =
                            "AM:You have purchased "
                                    + pilot.getName()
                                    + " ("
                                    + pilot.getGunnery()
                                    + "/"
