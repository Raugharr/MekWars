...
public class PayoutModifier {
    public TreeMap<String, Integer> calculate(
        ...
        if (CampaignMain.cm.getBooleanConfig("ModifyOpPayoutByELO")
            ...
            if (so.getWinners().containsKey(currName)) {
                ...
                hisRating =
                        so.getLosers().get(so.getLosers().keySet().iterator().next()).getRating();
            } else {
                myRating = so.getLosers().get(currName).getRating();
                hisRating =
                        so.getWinners().get(so.getWinners().keySet().iterator().next()).getRating();
            }
    ...
    private int modifyInfluence(int earnedFlu, Double myRating, Double hisRating) {
        boolean modifyHigher =
                CampaignMain.cm.getBooleanConfig("ModifyOpPayoutByELO_Influence_Higher");
        boolean modifyLower =
                CampaignMain.cm.getBooleanConfig("ModifyOpPayoutByELO_Influence_Lower");
