...
public class OperationReporter {
    ...
    public void setWinnersAndLosers(
            TreeMap<String, SPlayer> winners, TreeMap<String, SPlayer> losers) {
        setWinners(winners);
        setLosers(losers);
    }
    ...
    public void commit() {
        ...
        MWLogger.resultsLog(
                "  Winner(s): " + opData.getWinners() + "  --  Loser(s): " + opData.getLosers());
    ...
    public void setUpOperation(
            String operationName, String planetName, String terrainName, String themeName) {
    ...
    public void addAttacker(String playerName, int armyID) {
        attackerMap.put(
                playerName, CampaignMain.cm.getPlayer(playerName).getHouseFightingFor().getName());
        ...
        if (army != null) {
            ...
            if (s.length() == 0) opData.setAttackerName(playerString);
            else opData.setAttackerName(s + ", " + playerString);
        }
    }
    ...
    public void addDefender(String playerName, int armyID) {
        ...
        if (army != null) {
            ...
            if (s.length() == 0) opData.setDefenderName(playerString);
            else opData.setDefenderName(s + ", " + playerString);
        }
    }
    ...
    public void addArmy(boolean attackerArmy, SArmy army) {
        ...
        if (attackerArmy) opData.setAttackerSize(numUnits);
        else opData.setDefenderSize(numUnits);

        for (Unit currU : army.getUnits()) {
            int ID = currU.getId();
            String model = ((SUnit) currU).getModelName();
            if (attackerArmy) attackerUnits.put(ID, model);
            else defenderUnits.put(ID, model);
        }
    }
    ...
    private void addEndingBV(int unitID, int BV) {
        ...
        if (attackerUnits.containsKey(unitID)) opData.addEndingBV(true, BV);
        else if (defenderUnits.containsKey(unitID)) opData.addEndingBV(false, BV);
    }
