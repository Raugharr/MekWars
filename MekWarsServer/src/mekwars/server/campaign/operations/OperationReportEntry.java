...
public class OperationReportEntry {
    ...
    public void setBV(boolean attacker, boolean start, int BV) {
        if (attacker) {
            if (start) attackerStartBV = BV;
            else attackerEndBV = BV;
        } else {
            if (start) defenderStartBV = BV;
            else defenderEndBV = BV;
        }
    }

    public void addEndingBV(boolean attacker, int BV) {
        if (attacker) attackerEndBV += BV;
        else defenderEndBV += BV;
    }

    public void addStartingBV(boolean attacker, int BV) {
        if (attacker) attackerStartBV += BV;
        else defenderStartBV += BV;
    }
