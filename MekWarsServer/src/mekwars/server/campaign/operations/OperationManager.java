...
public class OperationManager extends AbstractOperationManager implements I_OperationManager {
    ...
    /**
     * Construction of the Manager is keystone event for Operations system. Construction triggers
     * attempts to load Ops and ModOps.
     */
    ...
    // METHODS
    /**
     * Method which checks to see whether a player who logged out (quit client, crashed, etc) was
     * involved in a game. If so, a DisconnectionThread is started and a logout timestamp is set.
     *
     * <p>If both players are disconnected, kill game.
     */
    public void doDisconnectCheckOnPlayer(String name) {
        ...
        OpsDisconnectionThread discoThread =
                new OpsDisconnectionThread(so.getShortID(), otherName, p.getName(), timeOffline);

        OpsDisconnectionThread oldDiscoThread =
                disconnectionThreads.put(p.getName().toLowerCase(), discoThread);
    ...
    /**
     * Method which checks to see whether a reconnecting player should have a DisconnectionThread
     * halted and ShortOp SPlayer/Sarmy references refreshed.
     */
    public void doReconnectCheckOnPlayer(String name) {
        ...
        long gracePeriod =
                Long.parseLong(CampaignMain.cm.getConfig("DisconnectionGracePeriod")) * 1000;
    ...
    /**
     * Method which clears all references to a player in the disconenctionTimestamp and
     * disconnectionDuration Trees.
     *
     * <p>ShortResolver should call for all players when a game finishes, and this class should call
     * for all players whenver terminating a game.
     */
    ...
    /**
     * Method which passes information from AttackCommand to the ShortValidator and returns any
     * failures.
     *
     * <p>If the attacker passes the check, the Validator will automatically check active armies
     * from the aa's OLH for matches. Only if a defender is available will the ShortValidator
     * construct a ShortOperation.
     *
     * <p>The ShortOperation's chickenThreads contain all prechecked SArmies and SPlayers, and the
     * DefendCommand will let these SPlayers/SArmies join automatically.
     *
     * <p>In short - validateShortAttack takes care of 90% of the work necessary to get an Attack
     * running. The analagous defender call need only be used for SPlayers who activated after
     * validateShortAttack was called ...
     */
    public String validateShortAttack(
        ...
        ArrayList<Integer> failures =
                this.shortValidator.validateShortAttacker(ap, aa, o, target, longID, joiningAttack);
    ...
    /**
     * Method which clears a defender to participate in a game. Should only be called from
     * DefendCommand, and even then only when an Army or Player has not been pre-cleared to
     * participate in a game.
     *
     * <p>See validateShortAttack() for a detailed explaination of how/why this works.
     */
    ...
    /**
     * Conduit method. Takes ShortOperation, Operation, and a report String from CampaignMain and
     * sends them to the ShortResolver.
     */
    ...
    /**
     * Conduit method. Takes ShortOperation, Operation, a winner and a loser from a
     * DisconnectionThread and sends them to ShortResolver
     */
     ...
     *
     * <p>SPlayer ap and Operation o are sent for convenience. They could be derived from the
     * ShortOperation with ease, but they're already in the validator, so ...
     */
    public void addShortOperation(ShortOperation so, SPlayer ap, Operation o) {
        ...
        String toSend =
                "You are on your way to " + so.getTargetWorld().getName() + " (" + o.getName();
        if (money > 0) {
            ap.addMoney(-money);
    ...
    /**
     * Method which terminates an operation. This returns all player cost, stops all chicken
     * threads, returns all players to ACTIVE status if the so was in RUNNING mode.
     *
     * <p>It is not possible to terminate reporting or finished ShortOperations.
     *
     * <p>It is also not possible to auto-terminate any Operation which is running without the
     * voluntary fail code (ie - if the termination is not initiated by a user via the
     ...
     * @param ignoreStatus - use true to terminate reporting and finished games. should only be
     *     passed from /c hardterminate.
     */
    public void terminateOperation(
        ...
        if (!ignoreStatus) {
            ...
            if (so.getStatus() == ShortOperation.STATUS_INPROGRESS
                    && (termCode != TERM_TERMCOMMAND && termCode != TERM_NO_REMAINING_PLAYERS))
                return;
        }
        ...
        for (String currName : so.getAttackers().keySet()) {
            ...
            if (attflu > 0) {
                ...
                else toPlayer += ", +" + CampaignMain.cm.moneyOrFluMessage(false, true, attflu);
                didReturn = true;
            }
            if (attrp > 0) {
                ...
                else toPlayer += ", +" + attrp + " " + CampaignMain.cm.getConfig("RPShortName");
                didReturn = true;
            }
            if (didReturn) toPlayer += ").";
            else toPlayer += ".";
        ...
        for (String currName : so.getDefenders().keySet()) {
            ...
            if (o.getBooleanValue("AttackerUnitsTakenBeforeFightStarts")) {
                try {
                    SHouse faction =
                            CampaignMain.cm.getHouseForPlayer(so.getDefenders().firstKey());
                    for (SUnit unit : so.preCapturedUnits) faction.addUnit(unit, true);
                } catch (Exception ex) {
                    logger.error(ex);
                }
            }
        ...
        for (String currN : allArmies.keySet()) {
            try {
                ...
                logger.error(
                        currN + " had a null army while terminating. Continuing to next player.");
                continue;
            }
        }
        ...
        if (so.getStatus() == ShortOperation.STATUS_INPROGRESS) {
            ...
            for (String currN : so.getAllPlayerNames()) {
                ...
                if (dt != null && !dt.playerHasReturned()) {
                    dt.playerReturned(
                            false,
                            0); // duplicative, but ensures the thread won't penalize in the millis
                    // before
                    // clear().
                    continue;
                }
                ...
                if (so.isFromReserve()) currP.setFighting(false, true);
                else currP.setFighting(false);
    ...
    /**
     * Method which returns the ID of a specific long operation. It is assumed that hasLongOnPlanet
     * or hasSpecificLongOnPlanet will precede this in order to ensure a useful ID value.
     */
    ...
    /**
     * Method which checks to see if a faction has a long operation of a specific type running on a
     * given world. This is similar to hasLongOnPlanet, but has different uses.
     */
     ...
     *
     * <p>Can be done directly from the ShortOperation since the ID is already known, but this is
     * shorter than loading the ShortOperation if only the Thread is needed.
     */
    private OpsChickenThread getPlayerThreadForAttack(String pname, int opID) {
        TreeMap<String, OpsChickenThread> threads =
                this.getRunningOps().get(opID).getChickenThreads();
        return threads.get(pname.toLowerCase());
    }

    /**
     * Method which removes a player from every attack's possibleDefender lists and turns off all
     * chicken threads which are still running.
     *
     * <p>If removing this player leaves no possible defenders, the attack should be terminated.
     * This will usually happen when a player deactivates or joins another game.
     */
    ...
    /**
     * Method which removes a player from the attacker lists of all operations, except the given
     * operation. If removing the player from an attacker lists means there are no attackers left,
        ...
        for (ShortOperation currO : this.getRunningOps().values()) {
             ...
             */
                ...
                if (verbose) {
                    for (String currN : currO.getAllPlayerNames())
                        CampaignMain.cm.toUser(
                                p.getName() + " left the game for Attack #" + so.getShortID() + ".",
                                currN,
                                true);
                }
                if (currO.getAttackers().size() == 0) toTerminate.add(currO);
    ...
    /**
     * Method which removes a player from the defender lists of all operations, except the given
     * operation. Used to remove player from a set of multiplayer games when another game is changed
    ...
    /**
     * Method which is called from CampaignMain on ticks. Cleans up resolved operations (short and
     * long) and prepares a display string. This also releases the op numbers for future use.
     */
    public String tick() {
    ...
    /**
     * Method which checks whether or not a player is registered as an attacker. Used by
     * AttackCommand to ensure that a player does not multilaunch.
     */
    public int playerIsAnAttacker(SPlayer p) {
        for (ShortOperation currO : this.getRunningOps().values()) {
            if (currO.getAttackers().get(p.getName().toLowerCase()) != null
                    && currO.getStatus() != ShortOperation.STATUS_FINISHED)
     ...
     * stopped w/ stopChicken() it is also removed from the ShortOperation's tree, so we should only
     * check for nulls and may ignore the thread's shouldContinue boolean.
     *
     * <p>Used by ShortValidator to determine whether or not a player is under attack and to stop
    ...
    /**
     * Method which returns th scrap threads. Used by ShortResolvers to add threads and by
     * ScrapCommand to determine whether scraps should stipper payments to player or by player.
     */
    ...
    /**
     * Method which returns the ShortValidator. Should be used only by the OpponentListHelper's
     * constructor. Hacky.
     *
     * <p>SArmies legalOperations also need to be checked with a short validator, checkOperations()
     * should be piped through this class and into the validator.
     */
    ...
    public void loadOperations() {
        ...
        for (int i = 0; i < modNames.length; i++) {
            ...
            while (st.hasMoreTokens()) {
                ...
                if (currOp == null) {
                    logger.error(
                            "Error assigning modop target. Mod: "
                                    + currMod.getName()
                                    + " Target: "
                                    + currTarget);
                } else {
    ...
    /**
     * Lets us know if MUL only ops have been loaded This effects the status of commands like
     * activate attack and defend.
     *
     * @return
     */
