    ...
    public void writeOpList(TreeMap<String, Operation> ops) {
        ...
        try {
            ...
            for (Operation currO : ops.values()) {
                ...
                String hasLong =
                        currO.getTypeIndicator() + "*"; // actually an INT. 0 == short. 1 == long.
                ...
                String minOwnIBD =
                        currO.getValue("MinPlanetOwnershipIgnoredByDefender")
                                + "*"; // Baruk Khazad! - 20151003
                ...
                if (!currO.getValue("LegalDefendFactions").trim().equals("")) {
                    ...
                    StringTokenizer illegalTokenizer =
                            new StringTokenizer(
                                    currO.getValue("IllegalDefendFactions").trim(), "$");
                ...
                } else legalDefenders.append("allFactions*");

                StringBuffer allowPlanetFlags =
                        new StringBuffer(currO.getValue("AllowPlanetFlags"));
                if (allowPlanetFlags.length() < 1) allowPlanetFlags.append("^ ^*");
                else allowPlanetFlags.append("*");

                StringBuffer disallowPlanetFlags =
                        new StringBuffer(currO.getValue("DisallowPlanetFlags"));
                if (disallowPlanetFlags.length() < 1) disallowPlanetFlags.append("^ ^*");
                else disallowPlanetFlags.append("*");
