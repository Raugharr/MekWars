...
public class LongValidator {
    ...
    /**
     * Method which checks a players army vs. an operation type to see if the army can be used to
     * attack/defend.
     *
     * <p>Returns a vector of failure codes.
     */
    public ArrayList<Integer> validateLongOp(SPlayer p, SHouse ah, SPlanet pl, Operation o) {
        ...
        if (requiredMoney > ah.getMoney()) failList.add(LFAILS_HOUSEMONEY);
        // if (requiredActions > ah.getActions())
        // failList.add(new Integer(LFAILS_HOUSEACTIONS));

        if (reqPlayerMoney > p.getMoney()) failList.add(LFAILS_PLAYERMONEY);
        if (reqPlayerFlu > p.getInfluence()) failList.add(LFAILS_PLAYERFLU);
        if (reqPlayerRP > p.getReward()) failList.add(LFAILS_PLAYERREWARD);
        if (reqPlayerXP > p.getExperience()) failList.add(LFAILS_PLAYEREXP);

        return failList;
    }

    /** Method which converts Integer failure list into a human readible failure string. */
    public String failuresToString(ArrayList<Integer> failList) {

        String s = "Launch failed ";
        if (failList == null) return s += " for reasons unknown ...";
    ...
    /**
     * Method (private) which converts a given failure code into a string. Helper for
     * this.failuresToString
     */
