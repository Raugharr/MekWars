...
public class OperationEntity {
        ...
        try {
            ...
            // if vehicle, check salvage flag
                ...
                if (player != null && ID != -1) {
                    SUnit currUnit = player.getUnit(ID);
                    if (currUnit != null
                            && currUnit.getPilot() != null) // auto-assigned artillery throwing NPEs
                    currUnit.setLastCombatPilot(currUnit.getPilot().getPilotId());
                }
                unitFileName = ST.nextToken();
    ...
    public OperationEntity(
        ...
        } // unit is not owned by the player. most likely auto artillary or gun emplacements.
    ...
    public boolean isSalvagable() {
        ...
        if (CampaignMain.cm.getBooleanConfig("UsePartsRepair")
                && (MMUnitType == Unit.MEK
                        || MMUnitType == Unit.QUAD
                        || MMUnitType == Unit.VEHICLE)) return true;

        if (MMUnitType == Unit.MEK || MMUnitType == Unit.QUAD) return (this.getCTint() > 0);
        else if (MMUnitType == Unit.VEHICLE) return isSalvage;
