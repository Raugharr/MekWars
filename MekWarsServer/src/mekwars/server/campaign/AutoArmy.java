...
/**
 * The AutoArmy is a pseudo-army which is constructed on the fly, loading artillery units in
 * proportion to a BV.
 *
 * <p>Takes the given BV, loads server configs and loops through a set of weightclasses generating
 * new artillery units. Units are created, and the BV counter reduced, until there is no longer
 * enough BV to create a tube of any type.
 *
 * <p>NOTE: This uses a slew of static arrays. MUST be adjusted if the static weightclass numbers in
 * Unit.java are changed (curr: Light == 0, Medium == 1, Heavy == 2, Assault == 3). Since there's no
 * sensible reason to ever actually change these, aside from adding support for super-heavyweight
 * tanks sometime down the road, this should not be a major concern. @ urgru 1.4.05
 */
...
public class AutoArmy {
    ...
    public AutoArmy(int i, boolean Guns) {
        if (Guns) theUnits = this.generateGuns(i);
        else theUnits = this.generateAuto(i);
    } // end constructor
    ...
    public SUnit makeNewArtilleryPiece(int i) {
        ...
        if (Unit.LIGHT == i) {
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("LightArtilleryFile"), "$");

            size = list.countTokens();

            if (size == 1) filename = list.nextToken();
            else {
            ...
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("MediumArtilleryFile"), "$");
            ...
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("HeavyArtilleryFile"), "$");
            ...
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("AssaultArtilleryFile"), "$");
    ...
    public Vector<SUnit> makeNewGunEmplacement(int i) {
        ...
        if (Unit.LIGHT == i) {
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("LightGunEmplacementFile"), "$");
            ...
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("MediumGunEmplacementFile"), "$");
            ...
            StringTokenizer list =
                    new StringTokenizer(CampaignMain.cm.getConfig("HeavyGunEmplacementFile"), "$");
            ...
            StringTokenizer list =
                    new StringTokenizer(
                            CampaignMain.cm.getConfig("AssaultGunEmplacementFile"), "$");
     ...
     *
     * <p>Feed it the vector to use as a paramater.
     */
