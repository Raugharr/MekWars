 ...
 *
 * <p>Note, that while you write modifyPilot, you must only add differences between levels. An
 * example:
 *
 * <p>If your Pilot should get a malus to gunnery for each level, you have to add only +1 to gunnery
 * in modifyPilot, since the malus for previous levels are already applied.
 *
...
public abstract class SLevelPilotSkill extends SPilotSkill {
     ...
     *
     * @param level The level to upgrade to. If this is 1, the chance to get the skill at all is
     *     requested.
     */
    ...
    public SLevelPilotSkill(int id, String name, SLevelPilotSkill previous, String abbreviation) {
        ...
        for (SLevelPilotSkill p = previous; p != null; p = p.previous) level++;
    }
    ...
    /**
     * Get the chance by asking of the chance of level upgrades. Do not override, but use
     * getUpgradeChance() instead.
     */
    @Override
    public final int getChance(int unitType, Pilot pilot) {
        int level = getLevel();
        if (level == 1) return getUpgradeChance(1, unitType, pilot); // obtaining new skill
        if (pilot.getSkills().has(previous)) return getUpgradeChance(level + 1, unitType, pilot);
        return 0; // cannot jump over levels
    }

    /**
     * All leveled skills of one category are linked to each other. This defines the previous skill
     * in this category or null, if it is the first skill.
     */
