...
/**
 * @author Torren (Jason Tighe) Server-side holder of Personal Pilot Queue information. The queue is
 *     a collection of pilots, managed by a player, which may be moved between eligible units
 *     (restricted by type and weightclass).
 */
public class SPersonalPilotQueues implements Serializable {
    ...
    /**
     * Add a pilot to the queue. Many different events can trigger an addition, including game
     * resolution, sale via market, the hiring/purchase of a new pilot, and more. The type of unit
     * that the pilot may use is embedded within the Pilot/SPilot that is passed as a param;
     * however, the weight class is not and must be set here.
     *
    ...
    /**
     * Check the specified queue's size and send a warning to the named player if they've exceeded
     * the queue cap. Have to do this in a stand alone method b/c the PPQ has no knowledge of it's
     * owning player and must be sent a name.
     */
    ...
    /**
     * Remove a given pilot from the player's personal queue (as defined by type, weight and
     * position in the LList) and return him to the calling class.
     */
    ...
    /**
     * Convert the pilot queue information into a data string. Although toClient doesn't change this
     * string-out directly, it is needed by SPilot's toString equivalent (SPilot.toFileFormat()).
     * WARNING: This format MAY NOT BE CHANGED. Any restructuring of this data would break servers'
     * player saves.
     *
            ...
            while (this.getPilotQueue(Unit.MEK, weightClass).size() > capSize) {
                this.getPilot(
                        ...
                        CampaignMain.cm.getRandomNumber(
                                this.getPilotQueue(Unit.MEK, weightClass).size()));
            }
        }
        ...
        for (int weightClass = Unit.LIGHT; weightClass <= Unit.ASSAULT; weightClass++) {
            ...
            while (this.getPilotQueue(Unit.PROTOMEK, weightClass).size() > capSize) {
                this.getPilot(
                        ...
                        weightClass,
                        CampaignMain.cm.getRandomNumber(
                                this.getPilotQueue(Unit.PROTOMEK, weightClass).size()));
            }
        }
        ...
        for (int weightClass = Unit.LIGHT; weightClass <= Unit.ASSAULT; weightClass++) {
            ...
            while (this.getPilotQueue(Unit.AERO, weightClass).size() > capSize) {
                this.getPilot(
                        ...
                        weightClass,
                        CampaignMain.cm.getRandomNumber(
                                this.getPilotQueue(Unit.AERO, weightClass).size()));
            }
        }
        /*
