 ...
 *
 * <p>The BuildTable class does NOT construct any units. Factories or direct building (welfare,
 * rewards) are used to create actual SUnits and add them to players.
 *
...
public class BuildTable {
    ...
    /**
     * Method which randomly selects a unit to build (filename), given a producer, unit size, mode
     * (rare, etc) and other necessary data. This is used for all standard factory production.
     */
    public static String getUnitFilename(
            String unitProducer, String size, int type_id, String dir) {
        ...
        while (!fileFound) {
            ...
            if (Filename.indexOf(".") == -1) unitProducer = Filename;
            else fileFound = true;
        }
        return Filename;
    }
    ...
    /**
     * Method which randomly selects a unit to build (filename), given only a filename. This is used
     * only to produce welfare units.
     */
    public static String getUnitFilename(String unitFileName) {
        ...
        while (!fileFound) {
            ...
            int ran = CampaignMain.cm.getRandomNumber(table.size());
            Filename = table.elementAt(ran);
            if (Filename.indexOf(".") == -1) unitFileName = Filename;
            else fileFound = true;
        }
        return Filename;
    }
    ...
    public static String getFileName(String faction, String weightclass, String dir, int Type) {
        ...
        String result =
                "./data/buildtables/" + dir + "/" + faction + "_" + weightclass + addon + ".txt";
        if (!new File(result).exists()) {
            if (!result.trim().toLowerCase().endsWith(".txt"))
                MWLogger.errLog(
                        "Unable to find build table file "
                                + result
     ...
     *
     * <p>1 UnitA.hmp 4 UnitB.hmp 5 UnitC.hmp 10 UnitD.hmp
     *
     * <p>This table would produce 50% D's, 25% C's, 20% B's and 5% A's (Weight/20).
     *
        ...
        try {
            ...
            while (unitHolder.size() > 0) {
                for (String filename : unitHolder.keySet()) {
                    ...
                    if (--count < 1) unitHolder.remove(filename);
                    else unitHolder.put(filename, count);
                }
            }
            result.trimToSize();
        } catch (Exception ex) {
    ...
    public static void saveBuildTableFile(
            String name, String folder, ConcurrentHashMap<String, Integer> unitHolder) {
    ...
    public static void saveBuildTableFile(
            File file, ConcurrentHashMap<String, Integer> unitHolder) {
        try {
