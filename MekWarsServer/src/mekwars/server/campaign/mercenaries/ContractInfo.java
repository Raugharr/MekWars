...
public class ContractInfo implements Serializable {
    ...
    public boolean isLegal() {
        if (CampaignMain.cm.getHouseFromPartialString(this._contractingHouse) == null) return false;
        if (CampaignMain.cm.getPlayer(this._offeringPlayer) == null) return false;
        if (CampaignMain.cm.getPlayer(this._playerName) == null) return false;
        return true;
    }
    ...
    public void fromString(String s) {
        ...
        if (ST.hasMoreElements()) _type = Integer.parseInt(ST.nextToken());
    }
    ...
    public String getInfo(SPlayer player) {
        ...
        info.append(
                performedXP
                        ...
                        + duration
                        + " "
                        + ContractInfo.getContractName(getType())
                        + " performed.<br>");
    ...
    public static int getContractType(String type) {
        ...
        if (type.equalsIgnoreCase("land")) return ContractInfo.CONTRACT_LAND;

        if (type.equalsIgnoreCase("units")) return ContractInfo.CONTRACT_UNITS;

        if (type.equalsIgnoreCase("components")) return ContractInfo.CONTRACT_COMPONENTS;

        if (type.equalsIgnoreCase("delay")) return ContractInfo.CONTRACT_DELAY;
