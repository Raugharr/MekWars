            ...
            while (true) {
                ...
                if (this.tickid % 8 == 0) {
                    ...
                    if (CampaignMain.cm.getBooleanConfig("DiscordEnable")) {
                        MWServ.getInstance()
                                .postToDiscord(Statistics.getReadableHouseRanking(false));
                    }

                    try {
                        FileWriter out =
                                new FileWriter(
                                        myCampaign.getConfig("HouseRankPath"),
                                        true); // opened in APPEND mode; will be controlled by
                        // config setting
                        out.write(
                                Statistics.getReadableHouseRanking(
                                        false)); // dump actual SHouse Ranking data to a permanent
                        // file
                        out.write("\n");
                        out.close();
                    } catch (IOException e) {
