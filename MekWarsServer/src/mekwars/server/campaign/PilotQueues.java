...
public class PilotQueues {
    ...
    public PilotQueues(
            Vector<Integer> baseGunnery,
            Vector<Integer> basePiloting,
            Vector<String> basePilotSkill) {
     ...
     *     <p>This particular add mechanism should only be called from *within* the PilotQueues
     *     class or when captureing a pilot. It is used to add newly generated pilots and captured
     *     enemy pilot to the queue, with their level ups (server gen'ed 3/5 and 4/4 skills, and
     *     other oddball pilots with the WizWom option true) intact.
     */
     ...
     *     <p>Check config to see if pilots should always retain their skills. If not, check for an
     *     elite pilot and downshift his abilities. Green units should not be improved; however, a
     *     4/5 pilot will be set in place of a pilot with 5 gunnery and improved piloting (no 5/2,
     *     5/3). Also check config to see if pilot XP should have a percentage chance to be cleared
     *     w/ skills.
     * @urgru 6/07/04
     */
    public void addPilot(int type, SPilot p) {
        ...
        if (CampaignMain.cm.getBooleanConfig("ReduceSkillsInQue")) {
            ...
            if (p.getGunnery() < this.getBaseGunnery(type)) gunnerAdjust = true;
            if (p.getPiloting() < this.getBasePiloting(type)) pilotAdjust = true;
            ...
            if (!gunnerAdjust && !pilotAdjust) {
                ...
                if (p.getPiloting() < (this.getBasePiloting(type) - 1)
                        && p.getGunnery() > this.getBaseGunnery(type)) {
                    p.setGunnery(this.getBaseGunnery(type));
                    p.setPiloting(this.getBasePiloting(type));
     ...
     * @param type
     * @param p
     *     <p>This is called from SHouse.fromString to load pilots directly from the dat files
    ...
    private SPilot rollNewPilot(int unitType) {
        ...
        int rnd =
                CampaignMain.cm.getRandomNumber(
                        100); // reroll rnd, use to check for improved pilots
        ...
        if (rnd < 10 && allowGreenPilots) {
            if (rnd < 5) gunnery++;
            else if (rnd >= 5 && rnd < 10) piloting++;
            skillChance = 100;
        }

        // Improved Pilots
        if (rnd >= 90 && allowVetPilots) {
            if (rnd >= 90 && rnd < 95) piloting--;
            else if (rnd >= 95) gunnery--;
        }
     ...
     *     <p>A method which should be called immedaitely after a pilot que is contructed, if
     *     faction specific name lists are enabled.
     */
    ...
    /**
     * @return a pilot name
     *     <p>NOTE: this is modelled on getRandomPilotName() from SUnit, but DOES NOT replace it.
     *     SUnit's naming defaults to the common list and is used for newbie faction units and for
     *     units generated with CreateMechCommand.
     */
    public String getRandomPilotName() {
        ...
        try {
            ...
            MWLogger.errLog(
                    "A problem occured while retreiving a name from the "
                            + factionString
