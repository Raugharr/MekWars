...
/**
 * A marketplace, where players may sell their units to one another. Admins may configure the type
 * of auction used, min/max list price, min/max list time, and various other behaviours.
 *
 * @urgru 12.29.05
 */
public class Market2 {
    ...
    // CONSTRUCTOR
    /**
     * Market determines auction type at construction. Changing the declared auction type in the
     * server configuration will NOT immediately change the type. The server must be restarted for
     * this constructor to run and the change to take hold.
     */
    public Market2() {
        /*
    ...
    // METHODS
    /**
     * Add a listing to the market. This method assumes that a player's sale request has
     * successfully passed checks in SellCommand (legal min bid, etc).
     */
    ...
    public void addListing(
            String sellername, SUnit unit, int minbid, int salesticks, boolean tickListing) {

        SPlayer sellingPlayer = null;

        if (!sellername.toLowerCase().startsWith("faction_")
                && CampaignMain.cm.getHouseFromPartialString(sellername, null) == null)
        ...
        if ((!UnitUtils.canStartUp(unit.getEntity())
            ...
            if (sellingPlayer != null)
                CampaignMain.cm.toUser(
                        "You cannot sell damaged units on the Black Market!", sellername, true);
        ...
        if (tickListing) {
            ...
            CampaignMain.cm.doSendToAllOnlinePlayers(
                    "BM|AU|" + newAuction.toString(auctionID, null), false);
        ...
        if (sellingPlayer != null) {
            CampaignMain.cm.toUser(
                    "BM|CU|" + newAuction.toString(auctionID, sellingPlayer), sellername, false);
            CampaignMain.cm.toUser(
                    "PL|SUS|" + unit.getId() + "#" + Unit.STATUS_FORSALE, sellername, false);
        }
    }

    /**
     * Public rmeove listing. Any call from outside of the Market is going to be a simple remove
     * without destruction.
     */
     ...
     *
     * <p>In most cases, will be triggered by a player-issued RecallCommand. May also be called from
     * AdminRecallCommand. Note that any penalty or msg. to player(s) is handled in the command
     * classes.
     *
     * <p>NOTE: removeListing is only called after a FAILED sale. If a unit changes hands, transfer
     * and updates are handled in this.tick().
     *
    ...
    private void removeListing(int auctionID, boolean destroyFactionUnits) {
        ...
        if (!sellerName.equalsIgnoreCase("rare")
                && !sellerName.toLowerCase().startsWith("faction_")) {
            ...
            if (seller == null) seller = CampaignMain.cm.getPlayer(sellerName);
            ...
            if (seller.isHuman()) {
                ...
                CampaignMain.cm.toUser(
                        "PL|SUS|" + auctionU.getId() + "#" + Unit.STATUS_OK, sellerName, false);
                p.setSave();
            } else {
                ...
                if (destroyFactionUnits) {
                    ...
                    CampaignMain.cm.doSendToAllOnlinePlayers(
                            h, "HS|" + h.getHSUnitAdditionString(auctionU), false);
            }
        } else {
    ...
    /**
     * Method which returns a MarketListing which contains a given unique unit ID. Returns null if
     * no matching listing exists.
     *
     * <p>NOTE: When simply checking whether or not a unit is being sold, use SUnit's .getStatus()
     * to see if Unit.STATUS_FORSALE is set.
     */
    public MarketListing getListingForUnit(int unitID) {

        for (MarketListing currList : currentAuctions.values()) {
            if (currList.getListedUnitID() == unitID) return currList; // match.
        }
        return null; // no match.
    }

    /**
     * Method which returns a MarketListing with a given auction ID. Used by BidCommand,
     * RecallCommand and other classes which can identify auctions.
     *
     * <p>Use this instead of getListingForUnit whenever possible, as no iteration is required.
     *
    ...
    /**
     * Method which determines whether or not a player has units on the market. Used by
     * DefectCommand and UnenrollCommand to ensure that a player who is moving/leaving isn't selling
     * anything.
     */
    ...
    /**
     * Method which gets a player's bid on a given auction. Generally used when a player first
     * connects to send him accurate BM info.
     */
    ...
    /**
     * Method which returns all bids made by a player, in a formatted string. This is used by
     * MyBidsCommand.java.
     *
     * <p>One could argue that the command should iterate through the listings and prep the return
     * string itself; however, that would require giving the hash to the command. As is, the hash is
     * touched only in this class, and I'd like to keep it that way.
     *
     * @urgru 12.29.05
     */
     ...
     *
     * <p>See comments in this.getPlayerBidsString for more details.
     */
        ...
        for (Integer currListID : currentAuctions.keySet()) {
            ...
            result +=
                    "#"
                            + currListID
                            + " "
                            + (CampaignMain.cm.getBooleanConfig("HiddenBMUnits")
    ...
    /**
     * Private method which determines the lowest available auction ID. Used by the market when a
     * new listing is added. Lame brute force approach, but it doesn't really matter here.
     */
    private int getFreeID() {
        boolean foundID = false;
        int potentialID = 0;
        while (!foundID) {
            if (currentAuctions.get(potentialID) == null) foundID = true;
            else potentialID++;
        }
        return potentialID;
    }
    ...
    /**
     * Market.tick() decrements counters on unfinished auction and resolves completed auctions.
     * Amounts to be paid and actual winners are determined by the Market's auctionType, but the
     * tick itself removes units from one player, adds to new owner.
     */
    public void tick() {
            ...
            try {
                ...
                if (currList.getSaleTicks() < 0) {
                    ...
                    if (currList.getSellerName().toLowerCase().startsWith("faction_"))
                        sellingActor =
                                CampaignMain.cm.getHouseFromPartialString(
                                        currList.getSellerName().substring(8), null);

                    // check house first.
                    if (sellingActor == null)
                        sellingActor =
                                CampaignMain.cm.getHouseFromPartialString(
                                        currList.getSellerName(), null);
                    if (sellingActor == null)
                    ...
                    if (winningBid == null) {
                        if (sellingActor.isHuman()) {
                            CampaignMain.cm.toUser(
                                    "No one purchased your " + currList.getListedModelName() + ".",
                                    currList.getSellerName(),
                                    true);
                            SPlayer p = (SPlayer) sellingActor;
                            CampaignMain.cm.toUser(
                                    "PL|SUS|" + currAuctionID + "#" + Unit.STATUS_OK,
                                    p.getName(),
                                    false);
                            p.setSave();
                        }
                     ...
                     */
                            ...
                            CampaignMain.cm.toUser(
                                    "The "
                                            ...
                                            + CampaignMain.cm.moneyOrFluMessage(
                                                    true, true, winningBidAmt)
                                            + ". Auction fees were "
                                            + CampaignMain.cm.moneyOrFluMessage(
                                                    true, true, auctionFeeAmt)
                                            + ", leaving "
                                            + CampaignMain.cm.moneyOrFluMessage(
                                                    true, true, winnerMoneyGain)
                                            + " as your take from "
                                            + "the sale.",
                                    currList.getSellerName(),
                                    true);
                        ...
                        if (Boolean.parseBoolean(
                                        CampaignMain.cm.getConfig("AllowPersonalPilotQueues"))
                                && unitForSale.isSinglePilotUnit()) {

                            // If the old owner is human, he keeps the pilot and gets an updated
                            // PPQ. If a
                            ...
                            if (buyingActor.isHuman()) {
                                ...
                                unitForSale.setPilot(
                                        p.getMyHouse().getNewPilot(unitForSale.getType()));
                            } else {
                        ...
                        sellingActor.removeUnit(
                                unitForSale,
                                false); // BM units already removed from the SHouse display
                ...
                MWLogger.errLog(
                        "Error during Market Tick for unit " + currList.getListedModelName());
                MWLogger.errLog(ex);
            }
        ...
        if (rareChance < CampaignMain.cm.getDoubleConfig("RareChance")) {
            ...
            String unitFilename =
                    BuildTable.getUnitFilename(
                            ...
                            Unit.MEK,
                            BuildTable.RARE); // rare units onto BM
            ...
            } else rareUnits.add(new SUnit(unitFluff, unitFilename, weightClass));
            // build the new unit
            for (SUnit rareUnit : rareUnits) {
                ...
                SHouse sellingFaction =
                        CampaignMain.cm.getHouseFromPartialString(
                                CampaignMain.cm.getConfig("NewbieHouseName"), null);
                int priceForUnit =
                        sellingFaction.getPriceForUnit(
                                rareUnit.getWeightclass(), rareUnit.getType());
                ...
                this.addListing(
                        "Faction_" + sellingFaction.getName(),
                        rareUnit,
                        priceForUnit,
                        rareSalesTime,
                        true);
    ...
    /**
     * Method which returns a client-readable string detailing the current contents of the market.
     * Sent when a player first logs in to the
     */
