...
/**
 * Vickrey auction is a modified highest sealed bid auction. Winner determination is the same
 * (highest bid, earliest placement in the event of a tie), but the winner pays 2nd highest bid,
 * plus one, in lieu of the amount he offered.
 *
 * <p>If only one player bids, he pays full fare. If no one bids, winner is "SERVER" and the Market
 * destroys the unit.
 *
 * @author urgru
 */
public final class VickreyAuction implements IAuction {
    ...
    /**
     * Winner is simply the highest offering person who can afford to pay. The amount he pays is
     * adjusted.
     *
     * <p>- If several people bid, winner pays next-highest + 1. - In only one person bids, winner
     * pays the minimum.
     */
    public MarketBid getWinner(MarketListing listing, boolean hiddenBM) {
        ...
        while (i.hasNext()) {
            ...
            if (potentialWinner == null)
                potentialWinner =
                        CampaignMain.cm.getHouseFromPartialString(currBid.getBidderName(), null);
            ...
            if (potentialWinner.isHuman()
                    && !hiddenBM
                    && !((SPlayer) potentialWinner).hasRoomForUnit(unitType, unitWeightClass)) {
                CampaignMain.cm.toUser(
                        "The "
                                + listing.getListedModelName()
            ...
            if (potentialWinner.isHuman()
                    ...
                    && (CampaignMain.cm.getIntegerConfig("MaximumNegativeBaysFromBM") != -1)) {
                SPlayer p = (SPlayer) potentialWinner;
                int baysAvailable =
                        p.getFreeBays()
                                + CampaignMain.cm.getIntegerConfig("MaximumNegativeBaysFromBM");
                int baysNeeded;
                SHouse sellingFaction;
                // SUnit u;
                String sellerName = listing.getSellerName();
                if (sellerName.toLowerCase().startsWith("faction_")
                        || (CampaignMain.cm.getHouseFromPartialString(sellerName) != null)) {
                    // Coming from a house bay
                    sellingFaction =
                            CampaignMain.cm.getHouseFromPartialString(
                                    sellerName.replace("Faction_", ""));
        ...
        if (i.hasNext()) {
            ...
            if (nextBidAmount < winningBid.getAmount()) winningBid.setAmount(nextBidAmount + 1);
            ...
            if (potentialWinner == null)
                potentialWinner =
                        CampaignMain.cm.getHouseFromPartialString(nextBid.getBidderName(), null);
            if (potentialWinner != null && potentialWinner.isHuman() && !hiddenBM) {
                CampaignMain.cm.toUser(
                        "You didn't get the  "
                                + listing.getListedModelName()
                                + " for "
                                + CampaignMain.cm.moneyOrFluMessage(true, true, nextBidAmount)
                                + ". Yours was the "
                                + "second place offer. The winner paid "
                                + CampaignMain.cm.moneyOrFluMessage(
                                        true, true, winningBid.getAmount())
                                + ".",
                        nextBid.getBidderName(),
                        true);
            }
        }
         ...
         */
        else winningBid.setAmount(listing.getMinBid());
            ...
            if (loser != null && loser.isHuman() && !hiddenBM) {
                CampaignMain.cm.toUser(
                        "You didn't get the  "
                                ...
                                + " for "
                                + CampaignMain.cm.moneyOrFluMessage(
                                        true, true, losingBid.getAmount())
                                + ". The "
                                + "winner paid "
                                + CampaignMain.cm.moneyOrFluMessage(
                                        true, true, winningBid.getAmount())
                                + ".",
                        losingBid.getBidderName(),
                        true);
            }
        }
