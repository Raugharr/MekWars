...
/**
 * MarketListing keeps track of all infomation related to a sale and necessary for resolution. Bids
 * and duration are updated while the auction is running. All other info is passed at construction
 * time and may not be changed while the auction is running.
 *
 * @author urgru
 */
public final class MarketListing {
    ...
    /** Add a bid to the treemap. This overwrites any prior bid from a player of the same name. */
    ...
    /** Return all bids. Uused when resolving auction and by RecallCommand.java */
    public TreeMap<String, MarketBid> getAllBids() {
        return bidsReceived;
    }

    /**
     * Return the bid from a given player. Used by Market2.getAutoMarketStatus() and
     * RecallBidCommand.java.
     */
    public int getBidForPlayer(SPlayer p) {
        ...
        for (MarketBid currBid : bidsReceived.values()) {
            if (p.getName().equalsIgnoreCase(currBid.getBidderName())) return currBid.getAmount();
        }
    ...
    /**
     * Return the ID# of the unit being sold. Note that the unit itself is NOT stored anywhere
     * within the market, and must be accessed by using SPlayer's getUnit(int unitID) or equivalent
     * method in SHouse.
     */
    ...
    /**
     * Returns the simulated model name (i.e., "Heavy Mek") for an auction where the units are
     * hidden
     */
    ...
    /**
     * Decrease the remaining sale-ticks by one. This is called by Market.java during a tick. Don't
     * worry about negative values here, as the Market will resolve any auction with a duration of
     * less than 1 before decrementing.
     */
    ...
    /**
     * This is called for units that are placed on the market at the tick due to the tick
     * decrementing the sale after its listed causing units to appear with -1 ticks left.
     */
