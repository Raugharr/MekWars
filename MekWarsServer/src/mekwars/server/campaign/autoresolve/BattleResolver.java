...
public class BattleResolver {
    ...
    public void resolve(ShortOperation so) {
        Battlefield bf =
                new Battlefield(prepareAttackers(so), prepareDefenders(so), new BattleReport());
    ...
    private String buildReportString(Battlefield bf) {
        return GameReport.prepareReport(bf, true, null).toString();
    }
    ...
    private VirtualUnit findRandomEnemy(VirtualUnit unit, Battlefield bf) {
        ...
        if (unit.isAttacker()) {
            target =
                    bf.getDefenders().get(CampaignMain.cm.getR().nextInt(bf.getDefenders().size()));
        } else {
            target =
                    bf.getAttackers().get(CampaignMain.cm.getR().nextInt(bf.getAttackers().size()));
        }
        return target;
    }
    ...
    private void resolveFiring(Battlefield bf) {
        for (VirtualUnit unit : bf.getAllUnits()) {
            ...
            for (Mounted mounted : ent.getWeaponList()) {
                ...
                if (toHit <= 12) {
                    int roll =
                            CampaignMain.cm.getR().nextInt(5)
                                    + 1
                                    + CampaignMain.cm.getR().nextInt(5)
                                    + 1;
                    if (roll >= toHit) {
    ...
    private void resolveMovement(Battlefield bf) {
        ...
        for (VirtualUnit unit : bf.getAllUnits()) {
            ...
            if (ent.getArmorRemainingPercent() > 0.5 && distance > 0) {
                ...
                unit.setMovement(
                        MovementMode.RUNNING, Math.abs(distance - bf.getDistance(unit, target)));
            ...
            bf.getBattleReport()
                    .addMovementEvent(
                            unit,
                            "Moved from a distance of "
                                    + distance
                                    + " to "
                                    + bf.getDistance(unit, target)
                                    + "<br>");
        }
    }
    ...
    private synchronized void damageEntity(
            VirtualUnit unit, int damage, String nameOfPlayer, Battlefield bf) {
        // Owner
        Entity ent = unit.getUnit().getEntity();
