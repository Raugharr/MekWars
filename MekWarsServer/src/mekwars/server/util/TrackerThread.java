 ...
 * @author Torren
 * @version 2.0
 *     <p>A thread which communicates with the tracker.
 *     <p>ChangeLog: Updated to use campaignconfig instead of ServerConfig, so we don't have to
    ...
    public TrackerThread(MWServ s) {
        ...
        if (trackerID.equalsIgnoreCase("0")) {
            ...
            if ((trackerID = serv.getCampaign().getConfig("TrackerUUID")).equalsIgnoreCase("0")) {
                // The tracker ID is not yet set by the server.  Let's set it so this never happens
                // again
                trackerID = UUID.randomUUID().toString();
                serv.getCampaign()
                        .getCampaignOptions()
                        .getConfig()
                        .setProperty("TrackerUUID", trackerID);
                serv.saveConfigs();
            }
        }
