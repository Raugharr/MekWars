 ...
 *     <p>Thread use to track all repairs made by all users on the server.
 */
public class RepairTrackingThread extends Thread {
    ...
    public void checkRepairs() {
        ...
        try {
            ConcurrentLinkedQueue<Repair> tempVector =
                    new ConcurrentLinkedQueue<Repair>(repairList);
            synchronized (tempVector) {
                for (Repair repairOrder : tempVector) {
                    // MWLogger.errLog("Start Time: "+ new
                    // Date(repairOrder.getStartTime()).toString()+" End
                    // Time: "+new Date(repairOrder.getEndTime()).toString());
                    ...
                    if (repairOrder.getEndTime() <= System.currentTimeMillis()) {
                        try {
                            if ((CampaignMain.cm.getPlayer(repairOrder.getUsername()) == null)
                                ...
                                repairList.removeElement(repairOrder);
                                SPlayer player =
                                        CampaignMain.cm.getPlayer(repairOrder.getUsername());
                                player.checkAndUpdateArmies(
                                        player.getUnit(repairOrder.getUnitID()));
                            }
                        } catch (Exception ex) {
                            MWLogger.errLog(
                                    "Unable to finish repair for "
                ...
                if (repairOrder.getUnitID() == unitID) {
                    ...
                    if (repairOrder.getArmor()) {
                        ...
                        if (repairOrder.getSlot() < UnitUtils.LOC_INTERNAL_ARMOR) {
                            ...
                            if (rear) {
                                results +=
                                        "Repairing external armor "
                                                + repairOrder
                                                        .getUnit()
                                                        .getLocationAbbr(armorLocation)
                                                + "(r) ETA: "
                                                + output
                                                + ".";
                            } else {
                        ...
                        CriticalSlot cs =
                                repairOrder
                                        .getUnit()
                                        .getCritical(
                                                repairOrder.getLocation(), repairOrder.getSlot());
                        ...
                        if (cs.getType() == CriticalSlot.TYPE_EQUIPMENT) {
                            ...
                            results +=
                                    "Repairing "
                                            ...
                                            + repairOrder
                                                    .getUnit()
                                                    .getLocationAbbr(repairOrder.getLocation())
                                            + ") ETA: "
                                            + output
                                            + ".";
                        } // end CS type if
                            ...
                            if (repairOrder.getUnit() instanceof Mech) {
                                results +=
                                        "Repairing "
                                                + ((Mech) repairOrder.getUnit())
                                                        .getSystemName(cs.getIndex())
                                                + "("
                                                + repairOrder
                                                        .getUnit()
                                                        .getLocationAbbr(repairOrder.getLocation())
                    ...
                    results +=
                            " <a href=\"MEKWARS/c stoprepairjob#"
                                    + unitID
                                    + "#"
        ...
        if (results.length() > 1) {
            results =
                    "#"
                            ...
                            + " "
                            + unitName
                            + " has the following repair jobs pending:<br>"
                            + results;
            return results;
        }
    ...
    public void stopRepair(int unitID, int location, int slot, boolean armor) {
        ...
        ConcurrentLinkedQueue<Repair> tempRepairList =
                new ConcurrentLinkedQueue<Repair>(getRepairList());
            ...
            for (Repair repairOrder : tempRepairList) {
                if (repairOrder.matches(unitID, location, slot, armor)) {
                    ...
                    CampaignMain.cm.toUser(
                            "FSM|Repair order cancelled.", repairOrder.getUsername(), false);
                    ...
                    CampaignMain.cm.toUser(
                            "PL|UU|" + unitID + "|" + unit.toString(true), player.getName(), false);
                    return;
                }
            }
        }
    ...
    public static Repair Repair(
        ...
        return new Repair(
                player, unitID, armor, location, slot, techType, retries, techWorkMod, salvage);
    }

    public static Repair Repair(
            SPlayer player, int unitID, Vector<Integer> techs, int repairTime, boolean salvage) {
...
class Repair {
    ...
    public Repair(
            SPlayer player,
            int unitID,
            Vector<Integer> techs,
            int repairTime,
            boolean simpleRepair,
            boolean salvage) {
    ...
    public boolean finishRepair() {
        ...
        if (player == null) {
            MWLogger.errLog(
                    "Could not find player " + Username + " removing repair job from queue.");
            return true;
        }
        ...
        try {
            ...
            String critName = "";
            int damagedCrits = 0;
            boolean disableTechAdvancement =
                    CampaignMain.cm.getBooleanConfig("DisableTechAdvancement");
            ...
            if (useCrits) {
                ...
                if (salvage) {
                    if (armor) {
                        ...
                        damagedCrits =
                                UnitUtils.getNumberOfCrits(unit, slot, location)
                                        - UnitUtils.getNumberOfDamagedCrits(
                                                unit, slot, location, armor);
                    }
                } else {
            ...
            if (simpleRepair) {
                ...
                for (int type = UnitUtils.ARMOR; type <= UnitUtils.ENGINES; type++) {
                    ...
                    if ((techType > UnitUtils.TECH_GREEN)
                            && ((die1 + die2)
                                    <= ((9 + techType) / 4 // Regs & Vets retire on 2. Elites on 3.
                                    ))
                            && !pilotIsRepairing
                            && !levelTech
                            && !repairTech
                        ...
                        if (pilotIsRepairing) {
                            ...
                            CampaignMain.cm.toUser(
                                    "FSM|"
                                            ...
                                            + " has graduated to "
                                            + StringUtils.aOrAn(
                                                    UnitUtils.techDescription(techType + 1), true)
                                            + " tech.",
                                    Username,
                                    false);
                        } else if (player.getTotalTechs().elementAt(techType) > 0) {
                            // AvailableTech was already removed so just move the tech to the next
                            // level
                    ...
                    CampaignMain.cm.toUser(
                            "PL|UU|" + unitID + "|" + mek.toString(true), Username, false);
            ...
            CampaignMain.cm.toUser(
                    "FSM|Tech Roll Base: " + roll + " Roll: " + (die1 + die2), Username, false);
            ...
            if ((die1 + die2) < roll) {
                if (armor) {
                    ...
                    if (slot != UnitUtils.LOC_INTERNAL_ARMOR) {
                        ...
                        if (salvage) {
                            ...
                            if (armorToRepair <= 3) {
                                ...
                                armorRepaired =
                                        CampaignMain.cm.getRandomNumber(armorToRepair - 2) + 1;
                            }
                            ...
                            UnitUtils.removeArmorRepair(unit, slot, location);
                            armorToRepair =
                                    unit.getOArmor(location, rear) - unit.getArmor(location, rear);
                            ...
                            unit.setArmor(
                                    (unit.getArmor(location, rear) + armorRepaired),
                                    location,
                                    rear);
                            String points = " points ";
                            if (armorRepaired == 1) {
                            ...
                            if (unit.getArmor(location, rear) == unit.getOArmor(location, rear)) {
                                if (rear) {
                                    CampaignMain.cm.toUser(
                                            "FSM|However the external armor("
                                                    + unit.getLocationAbbr(location)
                        ...
                        if (salvage) {
                            UnitUtils.removeArmorRepair(
                                    unit, UnitUtils.LOC_INTERNAL_ARMOR, location);
                            ...
                            UnitUtils.removeArmorRepair(
                                    unit, UnitUtils.LOC_INTERNAL_ARMOR, location);
                            // if IS is less then 0 then its been destroyed fully. so reset to 0 for
                            // the repair
                            if (unit.getInternal(location) < 0) {
                                unit.setInternal(0, location);
                            }
                            armorToRepair =
                                    unit.getOInternal(location) - unit.getInternal(location);
                            ...
                            unit.setInternal(
                                    (unit.getInternal(location) + armorRepaired), location);
                            String points = " points ";
                            ...
                            if (unit.getOInternal(location) == unit.getInternal(location)) {
                                CampaignMain.cm.toUser(
                                        "FSM|However the internal structure("
                                                + unit.getLocationAbbr(location)
                                                + ") is now fully repaired.",
                                        Username,
                                        false);
                    ...
                    if (salvage) {
                        if (cs.getType() == CriticalSlot.TYPE_EQUIPMENT) {
                            ...
                            if (UnitUtils.isEngineCrit(cs)) {
                                repairMessage =
                                        "The salvage job on the engines for the "
                                                + unit.getShortNameRaw()
                        ...
                        if (cs.getType() == CriticalSlot.TYPE_EQUIPMENT) {
                            ...
                            if (UnitUtils.isEngineCrit(cs)) {
                                repairMessage =
                                        "The repair job on the engines for the "
                                                + unit.getShortNameRaw()
                                                + " was a complete failure.";
                            } else {
                ...
                if ((retries > 0) && (roll < 13)) {
                    ...
                    int cost =
                            CampaignMain.cm.getRepairCost(
                                    unit, location, slot, techType, armor, techWorkMod, salvage);
                    if (player.getAutoReorder()
                            && (player.getPartsAmount(critName) < damagedCrits)) {
                        String newCommand = critName + "#" + damagedCrits;
                        CampaignMain.cm
                                .getServerCommands()
                                .get("BUYPARTS")
                                .process(new StringTokenizer(newCommand, "#"), Username);
                    }
                    ...
                    if ((cost < player.getMoney())
                        ...
                        // set the location back to a rear location number i.e. LOC_CTR, LOC_RTR,
                        // LOC_LTR
                        if (rear) {
                            location += 7;
                        }
                    ...
                    if (salvage) {
                        if (cs.getType() == CriticalSlot.TYPE_EQUIPMENT) {
                            ...
                            if (UnitUtils.isEngineCrit(cs)) {
                                UnitUtils.destroyAllEngineCrits(unit);
                                repairMessage =
                                        "All viable engines parts on the "
                                                + unit.getShortNameRaw()
                ...
                if ((techType > UnitUtils.TECH_GREEN)
                        && ((die1 + die2)
                                <= ((9 + techType) / 4 // Regs and Vets retire on 2 Elites on 3
                                ))
                        && !pilotIsRepairing
                        && !levelTech
                        && !repairTech
                ...
                CampaignMain.cm.toUser(
                        "FSM|Your "
                                ...
                                + CampaignMain.cm.moneyOrFluMessage(
                                        true, true, su.getCurrentRepairCost()),
                        Username,
                        false);
                su.addRepairCost(-1);
            }
            ...
            if (levelTech) {
                if (pilotIsRepairing) {
                    ...
                    player.addTotalTechs(techType + 1, 1);
                    CampaignMain.cm.toUser(
                            "FSM|<font color=#C11B17>One of your "
                                    + UnitUtils.techDescription(techType)
            ...
            if (techDeath) {
                CampaignMain.cm.toUser(
                        "FSM|<font color=#302226>One of your "
                                + UnitUtils.techDescription(techType)
