 ...
 *
 * <p>Integerated autoupdater into MekWarsAutoupdate --Torren
 *
 * <p>$Log: AutoUpdater.java,v $ Revision 1.2 2006/09/04 23:51:15 urgru + Many more refactors,
 * mostly small, involving loops, generics, and other warnings being thrown in Eclipse 3.2 that
 * weren't caught in 3.1.
 *
 * <p>Revision 1.1 2006/05/22 19:42:35 torren + Added new MekWarsAutoUpdate.jar run in stand alone
 * this can be used to update the Client or create a manifest.
 *
 * <p>Revision 1.5 2002/12/04 22:49:24 cactushack76
 *
 * <p>Changed to download new files to temp directory and create a script to copy them over.
 *
 * <p>Changed so that even if a file is not up to date, if the up to date file is in the temporary
 * directory, it won't download it again, but will update the script that will move the files.
 *
 * <p>Revision 1.4 2001/07/17 04:48:33 wurp added capability for autoupdater to update files that it
 * is currently using (e.g. java.exe) by creating an updatefix.bat file to be run after the update
 * is complete. Currently only works for DOS/WIndows platforms, but will be extended...
 *
 * <p>Revision 1.3 2001/07/04 23:08:00 uid51722 Changed to use * instead of ~ for manifest file
 * delimiter Added capability of adding "<CLEANUP>*dir name" lines to the manifest. This will cause
 * all files under the "dir name" directory tree on the client which are not also in the manifest to
 * be deleted.
 *
 * <p>Revision 1.2 2001/07/04 20:18:09 uid51722 Adding lots of new debug logging Added test to try
 * to use log4j_cfg.prop; log at level INFO to update.log otherwise
 *
 * <p>Revision 1.1.1.1 2001/06/26 03:29:50 wurp no message
 *
 * <p>Revision 1.8 2001/06/23 16:23:59 wurp More log4j migration Fixed sr; replacement of $1 type
 * variables was broken.
 *
 * <p>Revision 1.7 2001/06/20 03:36:05 wurp added log4j. debugging character creation issues.
 *
 * <p>Revision 1.6 2001/04/27 06:37:46 wurp Barest beginnings of party/group/guild/organization
 * support. Progress on real character creation (sending stuff over the wire).
 */
package updaters;
...
public class AutoUpdater {
    ...
    public void cleanUpLocalFiles(VersionManifest manifest) {
        ...
        for (String dirName : dirsToCleanUp) {
            ...
            for (File localFile : localFiles) {
                ...
                if (!expectedFileStructure.contains(localFile) && !localFile.isDirectory()) {
    ...
    /**
     * Builds a list of file objects using the correct localDir from the list of expected file
    ...
    public static void usage() {
        System.out.println(
                "AutoUpdater: Update a local application from a " + "remote repository.");
        System.out.println();
    ...
    public static void update(String localDir, String repositoryName, SplashWindow splash)
            throws IOException {
        AutoUpdater updater = new AutoUpdater(localDir, splash);
        // updater.setProgressMonitor(new ProgressMonitor(new JWindow(),"Updating from remote
        // repository","",0,1));
