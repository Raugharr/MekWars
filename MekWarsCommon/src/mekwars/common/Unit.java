...
public class Unit {
    ...
    public int linkToC3Network(Army army, Unit master) {
        ...
        if (this.getC3Level() == C3_SLAVE) {
            ...
            if (master.getC3Level() != C3_MASTER
                    && master.getC3Level()
                            != C3_MMASTER) // master is really a slave or doesn't have C3
            return -1;
            ...
            if (this.hasBeenC3LinkedTo(army)
                    && !this.hasC3SlavesLinkedTo(
                            army)) // other units have linked to this unit so it cannot link to
                // other units
                return -1;
        ...
        if (this.getC3Level() == C3_MMASTER) {
            ...
            for (Integer c3Slave : army.getC3Network().keySet()) {
                ...
                if (c3Master.intValue() == this.getId()) {
                    ...
                    if (tempUnit.getC3Level() == C3_SLAVE) slaveCount++;
                    else masterCount++; // we are going to assume that no C3I's are in a C3M/S
                    // network
                }
            }
