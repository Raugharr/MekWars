...
/**
 * NOTE: This is a unofficial rule. Pilot gets a -1 to-hit bonus on all ballistic weapons (MGs, all
 * ACs, Gaussrifles). @@author Torren (Jason Tighe)
 */
public class GunneryBallisticSkill extends PilotSkill {
    public GunneryBallisticSkill(int id) {
        super(id, "Gunnery/Ballistic", "GB");
        setDescription(
                "NOTE: This is a unofficial rule. Pilot gets a -1 to-hit bonus on all ballistic"
                        + " weapons (MGs, all ACs, Gaussrifles).");
    }
    ...
    public void modifyPilot(Pilot pilot) {
        pilot.addMegamekOption("gunnery_ballistic", true);
    ...
    public int getBVMod(Entity unit, Pilot p) {
        if (CampaignData.cd
                .getCampaignOptions()
                .getBooleanConfig("USEFLATGUNNERYBALLISTICMODIFIER")) {
    ...
    public int getBVModFlat(Entity unit) {
        int numberOfGuns = 0;
        int gunneryBallisticBVBaseMod =
                CampaignData.cd.getCampaignOptions().getIntegerConfig("GunneryBallisticBaseBVMod");
