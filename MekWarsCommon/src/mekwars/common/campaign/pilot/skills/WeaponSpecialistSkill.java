...
/**
 * A pilot who specializes in a particular weapon receives a -2 to hit modifier on all attacks with
 * that weapon. @@author Torren (Jason Tighe)
 */
public class WeaponSpecialistSkill extends PilotSkill {

    public WeaponSpecialistSkill(int id) {
        super(id, "Weapon Specialist", "WS");
        setDescription(
                "A pilot who specializes in a particular weapon receives a -2 to hit modifier on"
                        + " all attacks with that weapon.");
    }
    ...
    public int getBVMod(Entity unit, Pilot pilot) {
        ...
        double bvSkillDelta =
                BVCalculator.bvSkillMultiplier(crew.getGunnery() - 2, crew.getPiloting())
    ...
    public void assignWeapon(Entity entity, Pilot pilot) {
        HashMap<String, Boolean> uniqueWeapons = new HashMap<>();
        String bannedWeapons = CampaignData.cd.getCampaignOptions().getConfig("BannedWSWeapons");
        ...
        if (uniqueWeapons.size() > 1) {
            selectedWeapon = RandomUtils.getRandomNumber(uniqueWeapons.size());
        }

        for (String weaponName : uniqueWeapons.keySet()) {
            if (selectedWeapon == 0) {
